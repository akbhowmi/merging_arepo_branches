""" @package ./examples/fastwave_local_timestepping_2d/create.py
Create initial conditions for 2D fast magnetosonic wave
with Braginskii viscosity.

This test differs from fastwave_2d mainly in the type of timestepping employed.
...
print("examples/fastwave_local_timestepping_2d/create.py: creating ICs in directory " +
...
if high_nu_limit:
    omega = sign*k*np.sqrt(np.complex(vph**2 - (k*nu/6)**2)) - 1j*k**2.0*nu/6
else:
    omega = k*np.sqrt(np.complex(vph**2 - (k*nu/6)**2)) - 1j*k**2.0*nu/6
...
def create_ics(filename='ics.hdf5', writefunc=None):
    ...
    # Load grid points
    dat = np.loadtxt(simulation_directory + '/points_Ly0.2.txt')
    N = dat.shape[0]
    x = dat[:, 0]
    y = dat[:, 1]
    ...
    vel[index, 0] = vx_an(s, t)
    vel[index, 1] = vy_an(s, t)

    bfld[index, 0] = 0.0
    bfld[index, 1] = 0.0
    bfld[index, 2] = Bz_an(s, t)

    dens[index] = rho_an(s, t)
    P = P0 + (dens - rho0)/rho0*cs**2
    u[index] = P / dens / (gamma-1)
