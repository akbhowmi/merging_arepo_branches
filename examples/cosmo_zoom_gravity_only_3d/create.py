#!/usr/bin/env python3
""" @package examples/cosmo_zoom_gravity_only_3d/create.py
Code that creates ics and output list for a cosmological zoom simulation.
...
music_path = os.path.join(sys.path[0], '..', '..', 'music')
if os.path.exists(music_path):
    call(['cp', '-r', music_path, os.path.join(simulation_directory, 'music')])
else:
    print(
        'CREATE: did not find dir {}, trying to clone from Bitbucket.'.format(
            music_path))
    status = call([
...
# adjust Makefile
if 'FFTW_HOME' in os.environ:
    call(['sed', '-Ei',
        r's~^(\s*CPATHS\b[^#]+)~\1 -I$(GSL_HOME)/include -I$(FFTW_HOME)/include~',
        'Makefile'])
if 'GSL_HOME' in os.environ:
    call(['sed', '-Ei',
        r's~^(\s*LPATHS\b[^#]+)~\1 -L$(GSL_HOME)/lib -L$(FFTW_HOME)/lib '
        '-Xlinker -rpath=$(GSL_HOME)/lib:$(FFTW_HOME)/lib~',
        'Makefile'])
# compile MUSIC
status = call(['make', 'HAVEHDF5=no'])
...
if runParent:
    ...
    status = call([
        './MUSIC',
        os.path.join(simulation_directory, 'param_music_parent.txt')
    ])
    if status != 0:
    ...
    directory = os.path.join(cwd, 'examples', 'cosmo_zoom_gravity_only_3d')
    directory_run = os.path.join(cwd, 'run', 'examples',
                                 'cosmo_zoom_gravity_only_3d')
    ConfigFile = os.path.join(directory, 'Config_parent.sh')
    BuildDir = os.path.join(directory_run, 'build_parent')
    ExecFile = os.path.join(directory_run, 'Arepo_parent')
    ParamFile = os.path.join(directory, 'param_parent.txt')

    ## compile Arepo for cosmological volume
    status = call([
        'make', 'CONFIG=' + ConfigFile, 'BUILD_DIR=' + BuildDir,
        'EXEC=' + ExecFile
    ])
    if status != 0:
    ...
    sn0 = h5py.File(
        os.path.join(simulation_directory, 'output_parent', 'snap_000.hdf5'))
    sn6 = h5py.File(
        os.path.join(simulation_directory, 'output_parent', 'snap_006.hdf5'))
    sub6 = h5py.File(
        os.path.join(simulation_directory, 'output_parent',
                     'fof_subhalo_tab_006.hdf5'))
    ...
    i_mass_select = np.where((M200c > massInterval[0])
                             & (M200c < massInterval[1]))[0]
    ...
    pos = GrpPos[i_halo, :]
    print('Number of halos after isolation criterion selection: %d' %
          len(i_isolated))
    print('Selected halo %d with position %g %g %g' %
          (i_halo, pos[0], pos[1], pos[2]))
    ...
    center = [
        0.5 * (xmax + xmin) / 100000.0, 0.5 * (ymax + ymin) / 100000.0,
        0.5 * (zmax + zmin) / 100000.0
    ]
    box = [(xmax - xmin) / 100000. + 0.02, (ymax - ymin) / 100000. + 0.02,
           (zmax - zmin) / 100000. + 0.02]

    ## replace the high-res extent in param_music.txt
    paramFile = open(
        os.path.join('examples', 'cosmo_zoom_gravity_only_3d',
                     'param_music_parent.txt'), 'r')
    paramOptions = []
...
os.chdir(os.path.join(simulation_directory, 'music'))
status = call(['./MUSIC', os.path.join('..', 'param_music.txt')])
if status != 0:
    print('CREATE: ERROR: execution failed!')
    sys.exit(status)
os.chdir(cwd)
