...
#if defined(DEREFINE_GENTLY) || defined(CONDUCTION_SATURATION) ||                                        \
    (defined(COSMIC_RAYS) && defined(SHOCK_FINDER_ON_THE_FLY) || defined(SHOCK_FINDER_BEFORE_OUTPUT)) || \
    defined(SPECIAL_RELATIVITY) || defined(GENERAL_RELATIVITY) || defined(NON_LINEAR_SLOPE_LIMITERS) ||  \
    defined(CALCULATE_QUANTITIES_IN_POSTPROCESS)
...
#endif
#ifdef MHD_DEDNER
              tmpPrimExch[off].Psi = SphP[place].Psi;
#endif
#ifdef MHD_DEDNER_VARIABLE_SPEED
              tmpPrimExch[off].DednerSpeed = get_dedner_speed(place);
#endif
#ifdef MHD_CT
              for(k = 0; k < 3; k++)
                {
...
#if(defined(SHOCK_FINDER_POST_PROCESSING) || defined(SHOCK_FINDER_ON_THE_FLY) || defined(SHOCK_FINDER_BEFORE_OUTPUT)) && \
    defined(USE_SFR)
              ...
              tmpPrimExch[off].SGS_T_Pressure       = SphP[place].SgsTData.Pressure;
              tmpPrimExch[off].SGS_T_SpecificEnergy = SphP[place].SgsTData.SpecificEnergy;
#endif
              ...
              tmpPrimExch[off].Potential = P[place].Potential;
#endif

#ifdef CHIMES_ADVECT_ABUNDANCES
              for(abunIndex = 0; abunIndex < ChimesGlobalVars.totalNumberOfSpecies; abunIndex++)
                tmpPrimExch[off].ChimesAbundances[abunIndex] = SphP[place].ChimesGasVars.abundances[abunIndex];

#ifdef GFM_STELLAR_EVOLUTION
              tmpPrimExch[off].XH = get_hydrogen_abundances_of_local_cell(place);
#else
              tmpPrimExch[off].XH          = HYDROGEN_MASSFRAC;
#endif  // GFM_STELLAR_EVOLUTION
#endif  // CHIMES_ADVECT_ABUNDANCES
#if(defined(BIERMANN_BATTERY) || defined(DURRIVE_BATTERY))
              tmpPrimExch[off].n_elec = SphP[place].n_elec;
#endif
#ifdef BIERMANN_BATTERY
              tmpPrimExch[off].p_elec = SphP[place].p_elec;
#endif
#ifdef DURRIVE_BATTERY
              tmpPrimExch[off].pdot_elec[0] = SphP[place].pdot_elec[0];
              tmpPrimExch[off].pdot_elec[1] = SphP[place].pdot_elec[1];
              tmpPrimExch[off].pdot_elec[2] = SphP[place].pdot_elec[2];
#endif
            }
          listp = ListExports[listp].nextexport;
        }
...
#ifdef TVD_SLOPE_LIMITER
              MPI_Sendrecv(&tmpGradExchUl[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct grad_data), MPI_BYTE,
                           recvTask, TAG_HYDRO_A, &GradExchUl[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * sizeof(struct grad_data), MPI_BYTE, recvTask, TAG_HYDRO_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
#endif
            }
          ...
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              ...
              MPI_Sendrecv(&tmpHessianExch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct hessian_data),
                           MPI_BYTE, recvTask, TAG_HYDRO_A, &HessianExch[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * sizeof(struct hessian_data), MPI_BYTE, recvTask, TAG_HYDRO_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
