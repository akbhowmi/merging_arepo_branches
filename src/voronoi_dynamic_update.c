 ...
 *
 * \file        src/voronoi_dynamic_update.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
...
#if(defined(VORONOI) && defined(VORONOI_DYNAMIC_UPDATE)) || defined(AMR_CONNECTIONS)
...
int voronoi_get_connected_particles(tessellation *T)
{
  ...
  // mpi_printf("VORONOI: get connected particles\n");
                      ...
                      if(!(P[p].Mass == 0 && P[p].ID == 0)) /* skip cells that have been swallowed or dissolved */
                        {
                          ...
                          if(p < 0 || p >= NumGas)
                            terminate("strange p=%d (Ngas=%d) for q=%d Nvc=%d", p, NumGas, q, Nvc);
                              ...
                              if(Ninlist >= MaxNinlist)
                                {
                                  ...
                                  printf("Task=%d: increase memory allocation, MaxNinlist=%d Indi.AllocFacNinlist=%g\n", ThisTask,
                                         MaxNinlist, T->Indi.AllocFacNinlist);
#endif
                                  ListExports = (struct list_export_data *)myrealloc_movable(
                                      ListExports, MaxNinlist * sizeof(struct list_export_data));
                          ...
                          /* we move coordinates around -> do not add mirrored DPs here */
                          image_flags = 1;
#endif
  ...
  ForeignDC =
      (struct foreign_connection *)mymalloc_movable(&ForeignDC, "ForeignDC", count_foreign * sizeof(struct foreign_connection));
  ...
  if(nexport != count_foreign)
    terminate("nexport=%d  count_foreign=%d\n", nexport, count_foreign);

  if(Send_count[ThisTask] != 0)
    terminate("bad");

  ImportedDC = (struct foreign_connection *)mymalloc_movable(&ImportedDC, "ImportedDC", nimport * sizeof(struct foreign_connection));
  ...
  for(i = 0; i < nimport; i++)
    {
...
#endif
                          ListExports =
                              (struct list_export_data *)myrealloc_movable(ListExports, MaxNinlist * sizeof(struct list_export_data));
      ...
      /*
         if((ListExports[List_P[p].currentexport].image_bits & image_flags))
  ...
  /* because we may have dropped some of the points because they were turned into stars we need to redetermine the
   * send-offsets and counts
   */
  ...
  /* at this point, it might make sense to sort the delaunay point again according to peano-hilbert, in an
   * extended region that allows for the ghost regions
   */
...
void voronoi_update_connectivity(tessellation *T)
{
  ...
  // mpi_printf("VORONOI: updating connectivity\n");
              ...
              if(TimeBinSynchronized[P[p_index].TimeBinHydro])
                {
                  ...
                  if(FirstUnusedConnection < 0 || Nvc == MaxNvc)
                    {
                      if(!(FirstUnusedConnection < 0 && Nvc == MaxNvc))
                        terminate("strange: FirstUnusedConnection=%d Nvc=%d MaxNvc=%d\n", FirstUnusedConnection, Nvc, MaxNvc);
