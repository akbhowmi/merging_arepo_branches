...
void subfind(int num)
{
  ...
  int average_load        = (All.TotNumPart + NTask - 1) / NTask;
  int average_load_serial = 0;

  mpi_printf("SUBFIND: average_load=%d\n", average_load);
  ...
  do
    {
      ...
      if(NTask != NprocsCollective)
        average_load_serial = sum_seriallen / (NTask - NprocsCollective);
      else
        average_load_serial = 0;  // We hit the other criterion to repeat anyway

      mpi_printf("SUBFIND: GroupSize=%g, MaxSerialGroupLen=%d, NprocsCollective=%d/%d, Relative average serial load=%g.\n", GroupSize,
                 MaxSerialGroupLen, NprocsCollective, NTask, (double)average_load_serial / average_load);

      GroupSize += 0.05;
    }
  while((NprocsCollective > 0 && NprocsCollective >= NTask - 1) || (average_load_serial > 1.1 * average_load));

  if(GroupSize > 0.65)
    {
      mpi_printf("SUBFIND: Increased GroupSize to %g.\n", GroupSize);
    }
  ...
  ProcAssign = (struct proc_assign_data *)mymalloc_movable(&ProcAssign, "ProcAssign", Ncollective * sizeof(struct proc_assign_data));
  struct proc_assign_data *locProcAssign =
      (struct proc_assign_data *)mymalloc("locProcAssign", ncount * sizeof(struct proc_assign_data));
...
#ifdef ADD_GROUP_PROPERTIES
  subfind_add_grp_props_distribute_catalogue_subfind();
#endif
...
#ifdef DUST_LIVE
  load = All.MaxPartDust;
  MPI_Allreduce(&load, &max_loaddust, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
#endif
