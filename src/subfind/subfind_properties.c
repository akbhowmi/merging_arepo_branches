                                           ...
                                           int parallel_flag, int nsubgroups_cat, int grnr_local)
{
  ...
  double ddxx, vmax, vmaxrad, maxrad;
  ...
  if(parallel_flag)
    {
      ...
      double *minpotlist = (double *)mymalloc("minpotlist", SubNTask * sizeof(double));
  ...
  int nsubs_from_previous_grps = 0;  // on this task for serial subfind

  for(j = 0; j < grnr_local; j++)
    nsubs_from_previous_grps += GroupCatLocal[j].Nsubs;

  for(j = 0; j < 3; j++)
    pos[j] = SubGroupPosLocal[3 * (nsubs_from_previous_grps + subnr) +
                              j];  // pos[j] = SubGroupPos[3 * (GroupCatLocal[grnr_local].FirstSub + subnr) + j];
#endif
  ...
  for(j = 0; j < 3; j++)
    {
      ...
      s[j]  = coord_wrap(All.BoxSize, s[j] + pos[j]);
  ...
  rr_list = (sort_r2list *)mymalloc("rr_list", sizeof(sort_r2list) * (num + 1));
  ...
  if(parallel_flag)
    {
      ...
      double *masslist = (double *)mymalloc("masslist", SubNTask * sizeof(double));
      ...
      sort_r2list *rr_lowlist = (sort_r2list *)mymalloc("rr_lowlist", SubNTask * sizeof(sort_r2list));
      ...
      int *numlist = (int *)mymalloc("numlist", SubNTask * sizeof(int));
      ...
      double *maxlist    = (double *)mymalloc("maxlist", SubNTask * sizeof(double));
      double *maxradlist = (double *)mymalloc("maxradlist", SubNTask * sizeof(double));
  ...
  for(type = 0; type < NTYPES; type++)
    {
      rr_list = (sort_r2list *)mymalloc("rr_list", sizeof(sort_r2list) * (len_type_loc[type] + 1));
      itmp    = 0;
      for(i = 0; i < num; i++)
        {
          p = d[i].index;
  ...
  my_mhd_list = (struct rlist_mhd *)mymalloc("my_mhd_list", sizeof(struct rlist_mhd) * (num + 1));
  ...
  for(int i = 0; i < num; i++) /* loop over particles */
    {
      ...
      double bfld2 = (SphP[PS[p].OldIndex].B[0] * SphP[PS[p].OldIndex].B[0]) +
                     (SphP[PS[p].OldIndex].B[1] * SphP[PS[p].OldIndex].B[1]) + (SphP[PS[p].OldIndex].B[2] * SphP[PS[p].OldIndex].B[2]);
  ...
  if(parallel_flag)
    {
      ...
      double *egylist = (double *)mymalloc("egylist", SubNTask * sizeof(double));
  ...
  for(j = 0; j < 3; j++)
    {
      if(massinrad > 0)
        {
          sinrad[j] /= massinrad;
          sinrad[j] = coord_wrap(All.BoxSize, sinrad[j] + pos[j]);
          vinrad[j] /= massinrad; /* this is comoving (as it should be.) */
        }
      if(massinhalfrad > 0)
        {
          sinhalfrad[j] /= massinhalfrad;
          sinhalfrad[j] = coord_wrap(All.BoxSize, sinhalfrad[j] + pos[j]);
          ...
          if(SphP[PS[p].OldIndex].Sfr > 0)
            {
              ...
              double Temp =
                  MeanWeight / BOLTZMANN * GAMMA_MINUS1 * SphP[PS[p].OldIndex].Utherm * All.UnitEnergy_in_cgs / All.UnitMass_in_g;
              double nh = SphP[PS[p].OldIndex].Density *
                          ((All.UnitMass_in_g / All.HubbleParam) / pow(All.Time, 3) / pow(All.UnitLength_in_cm / All.HubbleParam, 3) *
                           HYDROGEN_MASSFRAC / PROTONMASS);  // FIXME?
  ...
  if(Luminosity_U > 0 && Luminosity_B > 0 && Luminosity_V > 0 && Luminosity_K > 0 && Luminosity_g > 0 && Luminosity_r > 0 &&
     Luminosity_i > 0 && Luminosity_z)
    {
      ...
      for(irad = 1; irad < GFM_STELLAR_PHOTOMETRICS_RADII; irad++)
        {
          if(get_default_softening_of_particletype(4) < max_stellar_rad)
            radii_grid[irad] =
                get_default_softening_of_particletype(4) * pow(max_stellar_rad / get_default_softening_of_particletype(4),
                                                               (double)(irad - 1) / (GFM_STELLAR_PHOTOMETRICS_RADII - 2));
          else
              ...
              if(P[p].Type == 4 && STP(p).BirthTime >= 0)
                {
                  ...
                  rotxy[1] = (-1) * GRAVITY_NEAREST_X(P[p].Pos[0] - pos[0]) * costheta * sinphi +
                             GRAVITY_NEAREST_Y(P[p].Pos[1] - pos[1]) * costheta * cosphi +
                             GRAVITY_NEAREST_Z(P[p].Pos[2] - pos[2]) * sintheta;
                  rxy = sqrt(rotxy[0] * rotxy[0] + rotxy[1] * rotxy[1]);
                  if(rxy > 0 && rxy >= max_stellar_rad)
                    terminate("SUBFIND GFM_STELLAR_PHOTOMETRICS: rxy=%g >= max_stellar_rad=%g idir=%d orig-pos=[%g %g %g]", rxy,
                              max_stellar_rad, idir, GRAVITY_NEAREST_X(P[p].Pos[0] - pos[0]), GRAVITY_NEAREST_Y(P[p].Pos[1] - pos[1]),
                              GRAVITY_NEAREST_Z(P[p].Pos[2] - pos[2]));
      ...
      if(parallel_flag)
        {
          ...
          MPI_Allreduce(&luminosity_grid[0][0], &luminosity_grid_tot[0][0],
                        GFM_STELLAR_PHOTOMETRICS_DIRECTIONS * (GFM_STELLAR_PHOTOMETRICS_RADII - 1), MPI_MYFLOAT, MPI_SUM, SubComm);
          double mass_grid_tot[GFM_STELLAR_PHOTOMETRICS_DIRECTIONS][GFM_STELLAR_PHOTOMETRICS_RADII - 1];
          MPI_Allreduce(&mass_grid[0][0], &mass_grid_tot[0][0],
                        GFM_STELLAR_PHOTOMETRICS_DIRECTIONS * (GFM_STELLAR_PHOTOMETRICS_RADII - 1), MPI_DOUBLE, MPI_SUM, SubComm);
          for(idir = 0; idir < GFM_STELLAR_PHOTOMETRICS_DIRECTIONS; idir++)
            for(irad = 0; irad < GFM_STELLAR_PHOTOMETRICS_RADII - 1; irad++)
              {
          ...
          for(irad = GFM_STELLAR_PHOTOMETRICS_RADII - 2; irad > 0; irad--)
            {
              ...
              tmp_surface_brightness = 5 * log10(3600 * 360 / 10 / 2 / M_PI) - 2.5 * log10(luminosity_grid[idir][irad]) +
                                       ...
                                       2.5 * log10(pow(All.cf_atime * All.UnitLength_in_cm / PARSEC / All.HubbleParam, 2)) -
                                       2.5 * log10(pow(All.cf_atime, 4));
