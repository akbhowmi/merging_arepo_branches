...
static double RootLen, RootBigFac, RootInverseLen, RootCenter[3], RootCorner[3];
 ...
 *  \param[in] ud Pointer to unbind data.
 *
 *  \return void
 */
void subfind_loctree_findExtent(int npart, struct unbind_data *ud)
{
  int i, j, k;
              ...
              if(numnodes >= MaxNodes)
                {
                  ...
                  if(numnodes > MaxNodes)
                    terminate("maximum number %d of tree-nodes reached., for particle %d  %g %g %g", MaxNodes, i, P[i].Pos[0],
                              P[i].Pos[1], P[i].Pos[2]);
                }
            }
        }
