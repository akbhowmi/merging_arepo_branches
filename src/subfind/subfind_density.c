...
#else
      if((PS[i].GrNr >= TotNgroups) && (P[i].Type == 4 || P[i].Type == 5 || P[i].Type == DUST_LIVE))
#endif
        Todo[i] = 0;
...
#else
      if(P[i].Type != 0 && P[i].Type != 1 && P[i].Type != 4 && P[i].Type != 5 && P[i].Type != DUST_LIVE)
#endif
        Todo[i] = 0;
#endif
#ifdef REFINEMENT_HIGH_RES_GAS
      if(P[i].Type == 0)
        if(SphP[i].AllowRefinement == 0 || SphP[i].HighResMass < HIGHRESMASSFAC * P[i].Mass)
          Todo[i] = 0;
#endif
          ...
          if(PS[i].Hsml == 0)
            {
              printf("Hsml=0 task=%d i=%d no=%d Nodes[no].len=%g Nodes[no].u.d.mass=%g P[i].Mass=%g type=%d ID=%llu  pos=(%g|%g|%g)\n",
                     ThisTask, i, no, Nodes[no].len, Nodes[no].u.d.mass, P[i].Mass, P[i].Type, (unsigned long long)P[i].ID,
...
#endif  // #if defined(SUBFIND) || (defined(GFM_WINDS_VARIABLE) && (GFM_WINDS_VARIABLE == 1)) || defined(GFM_WINDS_LOCAL)
