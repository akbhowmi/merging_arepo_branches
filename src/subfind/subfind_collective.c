...
void subfind_process_group_collectively(int nsubgroups_cat)
{
  ...
  int max_length;
  int count, countall;
  int j, k, nr, grindex = 0, nsubs, subnr;
  int tot_count_leaves, master;
  ...
  do
    {
      ...
      int *countlist = (int *)mymalloc("countlist", SubNTask * sizeof(int));
      int *offset    = (int *)mymalloc("offset", SubNTask * sizeof(int));
      ...
      /*
         if(max_length > 0.5 * All.TotNumPart / NTask)
         {
         if(SubThisTask == 0)
         printf("SUBFIND-COLLECTIVE, root-task=%d: too big coll_candidates, I do the rest collectively\n", ThisTask);
         break;
         }
       */
              ...
              if(parent == 0)
                {
                  if(SubThisTask != master)
                    ...
                    {
                      long long p = coll_candidates[k].head;
...
#ifdef TRACER_MC
      restore_tracer_connectivity(); /* restore integrity of the tracer linked list */
#endif
      t1 = second();
      if(SubThisTask == 0)
        {
          ...
          if(len > 0)
            {
              ...
              tt0 = second();
              subfind_determine_sub_halo_properties(ud, LocalLen, &SubGroup[Nsubgroups], GrNr, subnr, 1, nsubgroups_cat, 0);
              tt1 = second();
  ...
  int *count_loc = mymalloc("count_loc", sizeof(int) * GroupCatLocal[0].Nsubs);
  int *count_max = mymalloc("count_max", sizeof(int) * GroupCatLocal[0].Nsubs);
  int *count_sum = mymalloc("count_sum", sizeof(int) * GroupCatLocal[0].Nsubs);

  for(int i = 0; i < GroupCatLocal[0].Nsubs; i++)
    count_loc[i] = 0;

  for(int i = 0; i < NumPart; i++)
    {
      int subnr = (P[i].OriginalSubNr - GroupCatLocal[0].FirstSub);
      if(subnr >= 0 && subnr < GroupCatLocal[0].Nsubs)
        count_loc[subnr]++;
    }

  MPI_Allreduce(count_loc, count_max, GroupCatLocal[0].Nsubs, MPI_INT, MPI_MAX, SubComm);
  MPI_Allreduce(count_loc, count_sum, GroupCatLocal[0].Nsubs, MPI_INT, MPI_SUM, SubComm);

  ud = (struct unbind_data *)mymalloc_movable(&ud, "ud", NumPartGroup * sizeof(struct unbind_data));

  subfind_loctree_treeallocate(All.TreeAllocFactor * NumPart, NumPart);

  Group[0].Nsubs = GroupCatLocal[0].Nsubs;

  for(int subnr = 0; subnr < GroupCatLocal[0].Nsubs; subnr++)
    {
      LocalLen = 0;
      int LocalNonGasLen;
      ...
      for(int i = 0; i < NumPart; i++)
        {
          if(P[i].OriginalSubNr == GroupCatLocal[0].FirstSub + subnr)
            ud[LocalLen++].index = i;
        }

      if(count_sum[subnr] != count_max[subnr]) /* parallel distributed case */
        {
          subfind_col_unbind(ud, LocalLen, &LocalNonGasLen);
        }
      else
        {
          if(LocalLen > 0)
            subfind_unbind(ud, LocalLen, &LocalNonGasLen);
        }
      ...
      if(SubThisTask == 0)
        {
          ...
          if(subnr == 0)
            {
              for(int j = 0; j < 3; j++)
                Group[0].Pos[j] = SubGroup[Nsubgroups].Pos[j];
            }
...
void subfind_fof_calc_am_collective(int snapnr, int ngroups_cat)
{
  ...
  if(SubThisTask == 0)
    {
      // gr_mass = Group[grindex].Mass;
      // gr_mass_dm = Group[grindex].MassType[1];
      // gr_mass_gas = Group[grindex].MassType[0]; // (incl. winds)
      // gr_mass_stars = Group[grindex].MassType[4];
  ...
  // MPI_Bcast(&gr_mass, 1, MPI_DOUBLE, 0, SubComm);
  // MPI_Bcast(&gr_mass_dm, 1, MPI_DOUBLE, 0, SubComm);
  // MPI_Bcast(&gr_mass_gas, 1, MPI_DOUBLE, 0, SubComm);
  // MPI_Bcast(&gr_mass_stars, 1, MPI_DOUBLE, 0, SubComm);
  ...
  for(k = 0; k < len; k++)
    {
      ...
      for(i = 0; i < 3; i++)
        Pos_pbc[i] = fof_periodic_nearest(Pos_pbc[i]);
 ...
 *
 *  \param[in] cnt_cand Number of subgroup candidates.
 *
 *  \return void
 */
void subfind_unbind_independent_ones(int cnt_cand)
{
  ...
  qsort(coll_candidates, cnt_cand, sizeof(struct coll_cand_dat), subfind_compare_coll_candidates_nsubs);

  for(k = 0, i = 0; k < cnt_cand; k++)
