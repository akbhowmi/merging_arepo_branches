 ...
 *
 * \file        src/MRT/RT_voronoi_gradients_lsf.c
 * \date        06/2018
 * \author
 * \brief
 * \details
 ...
 *
 * Calculates the least square fit gradients for the RT variables only.
 */
...
#if defined(MRT) && (defined(VORONOI_DYNAMIC_UPDATE) || defined(AMR_CONNECTIONS))

static double *minvalues, *maxvalues;

static void limit_gradients_RT(void);
...
void calculate_gradients_RT(void)
{
  ...
  TIMER_STOP(CPU_RT_GRADIENTS);
}

void limit_gradients_RT(void)
{
  mpi_printf_rt(0, "RT: Limiting gradients...\n");
