 ...
 *
 * - This file contains all the necessary functions required to calculate the
 * RT riemann fluxes
 */
...
{
  TIMER_START(CPU_RT_FLUX);
  mpi_printf_rt(0, "RT: computing interface fluxes....\n");
  int num1, num2, num3;
  ...
  for(i = 0; i < T->Nvf; i++)
    ...
    defined(BOUNDARY_REFL_SOLIDSIDE_MAXID)
      ...
      /*            double X = DP[VF[i].p2].x - DP[VF[i].p1].x ;
      double Y = DP[VF[i].p2].y - DP[VF[i].p1].y ;
      double Z = DP[VF[i].p2].z - DP[VF[i].p1].z ;

      double deltax = sqrt(X*X + Y*Y + Z*Z) ;*/
      ...
      if(idR >= BOUNDARY_REFL_FLUIDSIDE_MINID && idR < BOUNDARY_REFL_FLUIDSIDE_MAXID && idL >= BOUNDARY_REFL_SOLIDSIDE_MINID &&
         idL < BOUNDARY_REFL_SOLIDSIDE_MAXID)
        {
          ...
          for(nn1 = 0; nn1 < MRT_BINS; nn1++)
            {
              state_R.DensPhot[nn1] = state_center_R.DensPhot[nn1];
              state_R.RT_F[nn1][1]  = state_center_R.RT_F[nn1][1];
              state_R.RT_F[nn1][0]  = state_center_R.RT_F[nn1][0];
              state_R.RT_F[nn1][2]  = state_center_R.RT_F[nn1][2];
              // state_R.modFN[nn1] = 0.9999999*c_internal_units ;

              state_L.DensPhot[nn1] = (F - state_R.RT_F[nn1][1] + c_internal_units * state_R.DensPhot[nn1]) / c_internal_units;
              state_L.RT_F[nn1][1]  = F;
              // state_L.DensPhot[nn1] = state_R.DensPhot[nn1] ;
              // state_L.RT_F[nn1][1] = -state_R.RT_F[nn1][1] ;
              state_L.RT_F[nn1][0] = 0.0;
              state_L.RT_F[nn1][2] = 0.0;
              // state_L.modFN[nn1] = 0.9999999*c_internal_units ;

              //	      state_R.RT_F[nn1][1] = MINDENSPHOT ;
              // state_R.RT_F[nn1][0] = 0.0 ; //state_center_L.RT_F[nn1][0] ;
              // state_R.RT_F[nn1][2] = 0.0 ; //state_center_R.RT_F[nn1][2] ;

              //      state_L.DensPhot[nn1] = state_R.DensPhot[nn1]*(1.0 - deltax/All.BoxSize) ;

              //  state_L.RT_F[nn1][0] = state_R.RT_F[nn1][0]*(1.0 - deltax/All.BoxSize) ;
              // state_L.RT_F[nn1][1] = state_R.RT_F[nn1][1]*(1.0 - deltax/All.BoxSize) ;
              // state_L.RT_F[nn1][2] = state_R.RT_F[nn1][2]*(1.0 - deltax/All.BoxSize) ;

              state_L.OldCons_DensPhot[nn1] = state_L.DensPhot[nn1] * state_L.volume;
            }
        }
              ...
              idR < BOUNDARY_REFL_SOLIDSIDE_MAXID)
        {
          ...
          for(nn1 = 0; nn1 < MRT_BINS; nn1++)
            {
              state_L.DensPhot[nn1] = state_center_L.DensPhot[nn1];
              state_L.RT_F[nn1][1]  = state_center_L.RT_F[nn1][1];
              state_L.RT_F[nn1][0]  = state_center_L.RT_F[nn1][0];
              state_L.RT_F[nn1][2]  = state_center_L.RT_F[nn1][2];
              //	      state_L.modFN[nn1] = 0.9999999*c_internal_units ;

              state_R.DensPhot[nn1] = (F - state_L.RT_F[nn1][1] + c_internal_units * state_L.DensPhot[nn1]) / c_internal_units;
              state_R.RT_F[nn1][1]  = F;
              // state_R.DensPhot[nn1] = state_L.DensPhot[nn1] ;
              // state_R.RT_F[nn1][1] = -state_L.RT_F[nn1][1] ;
              state_R.RT_F[nn1][0] =
                  0.0;  //-state_L.RT_F[nn1][0] ;
                        ////0.9999999*sqrt(fabs(c_internal_units*state_L.DensPhot[nn1]*c_internal_units*state_L.DensPhot[nn1] - F*F)) ;
              state_R.RT_F[nn1][2] = 0.0;

              //	      state_L.RT_F[nn1][1] = MINDENSPHOT ;
              // state_L.RT_F[nn1][0] = 0.0 ; //state_center_L.RT_F[nn1][0] ;
              // state_L.RT_F[nn1][2] = 0.0 ; //state_center_L.RT_F[nn1][2] ;

              // state_R.modFN[nn1] = 0.9999999*c_internal_units ;
              // state_R.DensPhot[nn1] = state_L.DensPhot[nn1]*(1.0 - deltax/All.BoxSize) ;

              // state_R.RT_F[nn1][0] = state_L.RT_F[nn1][0]*(1.0 - deltax/All.BoxSize) ;
              // state_R.RT_F[nn1][1] = state_L.RT_F[nn1][1]*(1.0 - deltax/All.BoxSize) ;
              // state_R.RT_F[nn1][2] = state_L.RT_F[nn1][2]*(1.0 - deltax/All.BoxSize) ;

              state_R.OldCons_DensPhot[nn1] = state_R.DensPhot[nn1] * state_R.volume;
            }
        }
        //      else if(idR >= BOUNDARY_REFL_SOLIDSIDE_MINID && idR < BOUNDARY_REFL_SOLIDSIDE_MAXID)
        // continue ;
        //      else if(idL >= BOUNDARY_REFL_SOLIDSIDE_MINID && idL < BOUNDARY_REFL_SOLIDSIDE_MAXID)
        // continue ;
          ...
          for(nn1=0;nn1<MRT_BINS;nn1++)
            {
              ...
              state_L.DensPhot[nn1] = state_R.DensPhot[nn1];
              state_L.RT_F[nn1][0] = state_R.RT_F[nn1][0] ;
              state_L.RT_F[nn1][1] = state_R.RT_F[nn1][1] ;
              state_L.RT_F[nn1][2] = state_R.RT_F[nn1][2] ;
              state_L.OldCons_DensPhot[nn1] = state_R.OldCons_DensPhot[nn1] ;
            }
        }
...
BOUNDARY_INFLOWOUTFLOW_MINID && idR < BOUNDARY_INFLOWOUTFLOW_MAXID)
          ...
          for(nn1=0;nn1<MRT_BINS;nn1++)
            {
              ...
              state_R.OldCons_DensPhot[nn1] = state_L.OldCons_DensPhot[nn1] ;
            }
      ...
      */
      ...
      /*rosunov type riemann solver - face velocity taken into the eigen value calculation*/
#ifdef MRT_RIEMANN_ROSUNOV
      double press2 = godunov_flux_3d_rosunov_RT(&state_L, &state_R, &state_face, &fluxes, vel_face_turned[0]);
#endif
      /*HLLE solver - face velocity taken into the eigen value calculation*/
#ifdef MRT_RIEMANN_HLLE
      double press2 = godunov_flux_3d_HLLE_RT(&state_L, &state_R, &state_face, &fluxes, vel_face_turned[0]);
#endif
      /*rosunov type riemann solver - face velocity taken into account using an advection step*/
#ifdef MRT_RIEMANN_ROSUNOV_NEW
      double press2 = godunov_flux_3d_rosunov_RT_new(&state_L, &state_R, &state_face, &fluxes, vel_face_turned[0]);
#endif
      /*HLLE solver - face velocity taken into account using an advection step*/
#ifdef MRT_RIEMANN_HLLE_NEW
      double press2 = godunov_flux_3d_HLLE_RT_new(&state_L, &state_R, &state_face, &fluxes, vel_face_turned[0]);
#endif
      ...
      /*do the advection step to account for face velocity*/
#if defined(MRT_RIEMANN_ROSUNOV_NEW) || defined(MRT_RIEMANN_HLLE_NEW)
      flux_convert_to_lab_frame_RT_new(&state_face, vel_face_turned, &fluxes);
#endif
...
#if !defined(RUNGE_KUTTA_FULL_UPDATE) && !defined(MRT_SUBCYCLE)
          fac *= 0.5;
#else
          fac *= 1.;
#endif
              ...
              if(DP[q].task == ThisTask)
                {
                  ...
                  //		  if(P[p].ID ==44)
                  // printf("Before = %d %d %g %g %g %g || dir = %g %g \n", state_L.ID, state_R.ID, SphP[p].Cons_DensPhot[0],
                  // SphP[p].DensPhot[0], dir*fluxes.DensPhot[0],  SphP[p].Cons_DensPhot[0]/SphP[p].Volume, dir, face_dt) ;
                  ...
                  // if(P[p].ID ==44)
                  // printf("After = %d %d %g %g %g %g || dir = %g %g\n", state_L.ID, state_R.ID, SphP[p].Cons_DensPhot[0],
                  // SphP[p].DensPhot[0], dir*fluxes.DensPhot[0],  SphP[p].Cons_DensPhot[0]/SphP[p].Volume, dir, face_dt) ;
                }
              else
                {
  ...
  if(ThisTask == 0)
    {
      ...
      mpi_printf_rt(0, "RT FLUX: exchanged fluxes over %g faces, with %g reduced (fraction %g), cumulative fraction %g\n", tot_count,
                    tot_count_reduced, tot_count_reduced / (tot_count + 1.0e-30),
                    All.TotCountReducedFluxes / (All.TotCountFluxes + 1.0e-30));
      All.TotCountReducedFluxes += tot_count_reduced;
      All.TotCountFluxes += tot_count;
    }

  TIMER_STOP(CPU_RT_FLUX);
}
...
#else
      delta->RT_F[num1][0] = -dt_half * c_internal_units * c_internal_units *
                             (rtgrad->dPT[num1][0][0][0] + rtgrad->dPT[num1][1][0][1] + rtgrad->dPT[num1][2][0][2]);
      delta->RT_F[num1][1] = -dt_half * c_internal_units * c_internal_units *
                             (rtgrad->dPT[num1][0][1][0] + rtgrad->dPT[num1][1][1][1] + rtgrad->dPT[num1][2][1][2]);
      delta->RT_F[num1][2] = -dt_half * c_internal_units * c_internal_units *
                             (rtgrad->dPT[num1][0][2][0] + rtgrad->dPT[num1][1][2][1] + rtgrad->dPT[num1][2][2][2]);
#endif
...
void face_do_spatial_extrapolation_RT(struct state *delta, struct state *st, struct state *st_other)
{
  ...
  for(num1 = 0; num1 < MRT_BINS; num1++)
    {
      face_do_spatial_extrapolation_single_quantity(&delta->DensPhot[num1], st->DensPhot[num1], st_other->DensPhot[num1],
                                                    rtgrad->dDensPhot[num1], dx, r);
      face_do_spatial_extrapolation_single_quantity(&delta->modFN[num1], st->modFN[num1], st_other->modFN[num1], rtgrad->dmodFN[num1],
                                                    dx, r);
      face_do_spatial_extrapolation_single_quantity(&delta->RT_F[num1][0], st->RT_F[num1][0] / st->DensPhot[num1],
                                                    st_other->RT_F[num1][0] / st_other->DensPhot[num1], rtgrad->dFN[num1][0], dx, r);
      face_do_spatial_extrapolation_single_quantity(&delta->RT_F[num1][1], st->RT_F[num1][1] / st->DensPhot[num1],
                                                    st_other->RT_F[num1][1] / st_other->DensPhot[num1], rtgrad->dFN[num1][1], dx, r);
      face_do_spatial_extrapolation_single_quantity(&delta->RT_F[num1][2], st->RT_F[num1][2] / st->DensPhot[num1],
                                                    st_other->RT_F[num1][2] / st_other->DensPhot[num1], rtgrad->dFN[num1][2], dx, r);
    }
}
          ...
          if(num1 < UV_BINS || num1 >= (UV_BINS + IR_BINS))
            {
              ...
              double oldmodFN =
                  sqrt(st_face->RT_F[num1][0] * st_face->RT_F[num1][0] + st_face->RT_F[num1][1] * st_face->RT_F[num1][1] +
                       st_face->RT_F[num1][2] * st_face->RT_F[num1][2]) /
              ...
              if(isnan(st_face->RT_F[num1][0]))
                terminate("TIME, Initial F is a nan %g %g %g \t%g \t %g \t %g \t %g \t %d\n", st_face->RT_F[num1][0],
                          st_face->RT_F[num1][1], st_face->RT_F[num1][2], Fx_face, oldmodFN, newmodFN, st_face->DensPhot[num1],
                          st_face->ID);
      ...
      for(num1 = 0; num1 < MRT_BINS; num1++)
        {
          ...
          if(isnan(Fx_face))
            terminate(" Fx is a nan %g \t %g \t %g||||| Nface = %g \t Ncell = %g \n", Fx_face, st_face->RT_F[num1][0],
                      delta->RT_F[num1][0], Nface, Ncell);

          if(FN_face > c_internal_units)
            FN_face = 0.9999999 * c_internal_units;
  ...
  for(num1 = 0; num1 < MRT_BINS; num1++)
    {
      ...
      if(st->ID >= BOUNDARY_REFL_SOLIDSIDE_MINID && st->ID < BOUNDARY_REFL_SOLIDSIDE_MAXID)
        {
          f    = 1.0;
          modF = fabs(st->RT_F[num1][0]);
        }
#endif

      if(f > 1.000000001)
        terminate("%g\t%g\t%g\t%g\t%g\t%g\t%g\nAgain!!!! Before = %g %g %g\t ID = %d\n", f, st->RT_F[num1][0], st->RT_F[num1][1],
                  st->RT_F[num1][2], modF, st->DensPhot[num1], c_internal_units, Fx, Fy, Fz, st->ID);
          ...
          for(k = 0; k < 3; k++)
            {
              ...
              if(isnan(st->PT[num1][j][k]))
                {
                  terminate("\nWHAT!!!!\n %g\t%g\t%g\t%g\n%g\t%g\t%g\t%g\t%g\t%g\nID=%d\n", st->DensPhot[num1], modF,
                            st->RT_F[num1][0], st->RT_F[num1][1], st->RT_F[num1][2], f, chi, n[0], n[1], n[2], st->ID);
                }
...
void flux_convert_to_lab_frame_RT_new(const struct state_face *st, double *vel_face, struct fluxes *flux)
{
  ...
  for(num1 = 0; num1 < MRT_BINS; num1++)
    {
      flux->DensPhot[num1] -= st->DensPhot[num1] * vel_face[0];

      flux->RT_F[num1][0] -= st->RT_F[num1][0] * vel_face[0];
      flux->RT_F[num1][1] -= st->RT_F[num1][1] * vel_face[0];
      flux->RT_F[num1][2] -= st->RT_F[num1][2] * vel_face[0];
    }
}
#endif
  ...
  *count = *count + 1.0;
  ...
  for(int num1 = 0; num1 < MRT_BINS; num1++)
    ...
    defined(BOUNDARY_REFL_SOLIDSIDE_MAXID)
          ...
          if((idR >= BOUNDARY_REFL_FLUIDSIDE_MINID && idR < BOUNDARY_REFL_FLUIDSIDE_MAXID && idL >= BOUNDARY_REFL_SOLIDSIDE_MINID &&
              idL < BOUNDARY_REFL_SOLIDSIDE_MAXID) ||
             (idL >= BOUNDARY_REFL_FLUIDSIDE_MINID && idL < BOUNDARY_REFL_FLUIDSIDE_MAXID && idR >= BOUNDARY_REFL_SOLIDSIDE_MINID &&
              idR < BOUNDARY_REFL_SOLIDSIDE_MAXID))
            {
              reduc_fac = 1.0;
            }
