...
void reset_turb_temp(void)
{
      ...
      if(P[i].Type == 0)
        {
          ...
          SphP[i].Energy =
              P[i].Mass * (SphP[i].Utherm + 0.5 * (P[i].Vel[0] * P[i].Vel[0] + P[i].Vel[1] * P[i].Vel[1] + P[i].Vel[2] * P[i].Vel[2]) +
                           get_turb_pot(P[i].Pos[0], P[i].Pos[1], P[i].Pos[2]));

#ifdef MHD
          SphP[i].Energy +=
              0.5 * (SphP[i].B[0] * SphP[i].B[0] + SphP[i].B[1] * SphP[i].B[1] + SphP[i].B[2] * SphP[i].B[2]) * SphP[i].Volume;
#endif
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
          ...
          for(l = 0; l < Nof_base_functions; l++)  // loop over base functions
            {
              SphP[i].Weights[l][W_E] +=
                  (0.5 * (w_cell[W_PX] * w_cell[W_PX] + w_cell[W_PY] * w_cell[W_PY] + w_cell[W_PZ] * w_cell[W_PZ]) / w_cell[W_RHO] +
                   All.RefEntropy * pow(w_cell[W_RHO], GAMMA) / GAMMA_MINUS1) *
                  GET_Inner_base_values(j, l) * GET_Inner_quad_points_weights(j);

              SphP[i].WeightsDiss[l] +=
                  (w_cell[W_E] -
                   0.5 * (w_cell[W_PX] * w_cell[W_PX] + w_cell[W_PY] * w_cell[W_PY] + w_cell[W_PZ] * w_cell[W_PZ]) / w_cell[W_RHO] -
                   All.RefEntropy * pow(w_cell[W_RHO], GAMMA) / GAMMA_MINUS1) *
                  GET_Inner_base_values(j, l) * GET_Inner_quad_points_weights(j) / DG_PROJ_NORM;
            }
        }
      ...
      SphP[i].Utherm = SphP[i].Weights[0][W_E] / SphP[i].Weights[0][W_RHO] -
                       0.5 *
                           (SphP[i].Weights[0][W_PX] * SphP[i].Weights[0][W_PX] + SphP[i].Weights[0][W_PY] * SphP[i].Weights[0][W_PY] +
                            SphP[i].Weights[0][W_PZ] * SphP[i].Weights[0][W_PZ]) /
                           (SphP[i].Weights[0][W_RHO] * SphP[i].Weights[0][W_RHO]);
  ...
  double(*a)[NOF_BASE_FUNCTIONS][5] =
      (double(*)[NOF_BASE_FUNCTIONS][5])mymalloc("a", NumGas * Nof_base_functions * 5 * sizeof(double));
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      tend = All.Ti_begstep[P[i].TimeBinHydro]; /* end of step (Note: All.Ti_begstep[] has already been advanced for the next step at
                                                   this point)   */
...
void log_turb_temp(void)
{
  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      ekin += 0.5 * P[i].Mass *
              ((P[i].Vel[0] - vmean_global[0]) * (P[i].Vel[0] - vmean_global[0]) +
               (P[i].Vel[1] - vmean_global[1]) * (P[i].Vel[1] - vmean_global[1]) +
  ...
  if(ThisTask == 0)
    {
      fprintf(FdTurb, "%g %g %g %g %g %g %g\n", All.Time, mach, glob_etot / glob_mass, glob_dudt_drive / glob_mass,
