      ...
      for(quadp = 0; quadp < Nof_outer_quad_points; quadp++)  // loop over quadrature points
        {
          ...
          // printf("Left state: rho=%e, p=%e, vx=%e, vy=%e, vz=%e\n", state_L.rho, state_L.press, state_L.velx, state_L.vely,
          // state_L.velz); printf("Right state: rho=%e, p=%e, vx=%e, vy=%e, vz=%e\n", state_R.rho, state_R.press, state_R.velx,
          // state_R.vely, state_R.velz);
          ...
          if(press < 0)
            {
              ...
              printf("Left state: rho=%e, p=%e, vx=%e, vy=%e, vz=%e\n", state_L.rho, state_L.press, state_L.velx, state_L.vely,
                     state_L.velz);
              printf("Right state: rho=%e, p=%e, vx=%e, vy=%e, vz=%e\n", state_R.rho, state_R.press, state_R.velx, state_R.vely,
                     state_R.velz);
              terminate("press < 0");
            }
...
#if defined(RIEMANN_HLLC) || defined(RIEMANN_ROSUNOV) || defined(RIEMANN_HLLD) ||                                                     \
    defined(RIEMANN_HLL) /* for non-exact Riemann solver, fluxes are already computed in the local frame, so convert to lab frame and \
                            turn momentum fluxes to the lab orientation  */
                  ...
                  if(DP[q].task == ThisTask)
                    {
                      ...
                      for(l = 0; l < Nof_base_functions; l++)
                        {
                          // a2 = a2 - c * dt * R_outer(a1)
                          a2[p][l][0] = a2[p][l][0] - c * face_dt * norm_fac * VF[i].area / SphP[p].Volume * dir * fluxes.mass *
                                                          GET_Outer_base_values(interface, quadp, l) *
                                                          GET_Outer_quad_points_weights(interface, quadp);
                          a2[p][l][1] = a2[p][l][1] - c * face_dt * norm_fac * VF[i].area / SphP[p].Volume * dir * fluxes.momentum[0] *
                                                          GET_Outer_base_values(interface, quadp, l) *
                                                          GET_Outer_quad_points_weights(interface, quadp);
                          a2[p][l][2] = a2[p][l][2] - c * face_dt * norm_fac * VF[i].area / SphP[p].Volume * dir * fluxes.momentum[1] *
                                                          GET_Outer_base_values(interface, quadp, l) *
                                                          GET_Outer_quad_points_weights(interface, quadp);
                          a2[p][l][3] = a2[p][l][3] - c * face_dt * norm_fac * VF[i].area / SphP[p].Volume * dir * fluxes.momentum[2] *
                                                          GET_Outer_base_values(interface, quadp, l) *
                                                          GET_Outer_quad_points_weights(interface, quadp);
                          a2[p][l][4] = a2[p][l][4] - c * face_dt * norm_fac * VF[i].area / SphP[p].Volume * dir * fluxes.energy *
                                                          GET_Outer_base_values(interface, quadp, l) *
                                                          GET_Outer_quad_points_weights(interface, quadp);
                        }
      ...
      if(pv[PRES] < 0 || pv[RHO] < 0)
        {
          /*
             ...
             double w2[5];
             double pv2[5];
             calc_state_at_pos(a[particle], GET_Outer_quad_points_x(interface, q), GET_Outer_quad_points_y(interface, q),
             GET_Outer_quad_points_z(interface, q), w2); w_to_primvars(w2,pv2);
          ...
          printf(
              "particle index: %d, level=%d, interface: %d, quadpoint: %d, p1:(%f, %f, %f), p2:(%f, %f, %f), pressure=%e, "
              "density=%e\n",
              particle, Mesh.DP[particle].level, interface, q, Mesh.DP[VF[i].p1].x, Mesh.DP[VF[i].p1].y, Mesh.DP[VF[i].p1].z,
              Mesh.DP[VF[i].p2].x, Mesh.DP[VF[i].p2].y, Mesh.DP[VF[i].p2].z, pv[PRES], pv[RHO]);
      ...
      if(pv[PRES] < 0 || pv[RHO] < 0)
        {
          printf("particle index: %d, interface: %d, quadpoint: %d, p1:(%f, %f, %f), p2:(%f, %f, %f), pressure=%f, density=%f\n",
                 particle, interface, q, Mesh.DP[VF[i].p1].x, Mesh.DP[VF[i].p1].y, Mesh.DP[VF[i].p1].z, Mesh.DP[VF[i].p2].x,
                 Mesh.DP[VF[i].p2].y, Mesh.DP[VF[i].p2].z, pv[PRES], pv[RHO]);
          terminate("Bad values at quadrature point!\n");
        }
      ...
      for(quadp = 0; quadp < Nof_outer_quad_points; quadp++)
        {
          for(l = 0; l < Nof_base_functions; l++)
            {
              a[p][l][0] = a[p][l][0] - FluxListGet[i].dM[quadp] / SphP[p].Volume *
                                            GET_Outer_base_values(FluxListGet[i].interface, quadp, l) *
                                            GET_Outer_quad_points_weights(FluxListGet[i].interface, quadp);
              a[p][l][1] = a[p][l][1] - FluxListGet[i].dP[quadp][0] / SphP[p].Volume *
                                            GET_Outer_base_values(FluxListGet[i].interface, quadp, l) *
                                            GET_Outer_quad_points_weights(FluxListGet[i].interface, quadp);
              a[p][l][2] = a[p][l][2] - FluxListGet[i].dP[quadp][1] / SphP[p].Volume *
                                            GET_Outer_base_values(FluxListGet[i].interface, quadp, l) *
                                            GET_Outer_quad_points_weights(FluxListGet[i].interface, quadp);
              a[p][l][3] = a[p][l][3] - FluxListGet[i].dP[quadp][2] / SphP[p].Volume *
                                            GET_Outer_base_values(FluxListGet[i].interface, quadp, l) *
                                            GET_Outer_quad_points_weights(FluxListGet[i].interface, quadp);
              a[p][l][4] = a[p][l][4] - FluxListGet[i].dEnergy[quadp] / SphP[p].Volume *
                                            GET_Outer_base_values(FluxListGet[i].interface, quadp, l) *
                                            GET_Outer_quad_points_weights(FluxListGet[i].interface, quadp);
            }
        }
