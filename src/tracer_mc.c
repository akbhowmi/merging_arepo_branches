...
int exchange_tracer_flux_list(void)
{
  ...
  struct tracer_flux_list_data *TracerFluxListSend =
      (struct tracer_flux_list_data *)mymalloc("TracerFluxListSend", nexport * sizeof(struct tracer_flux_list_data));
      ...
      if(recvTask < NTask)
        {
          if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&TracerFluxListSend[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct tracer_flux_list_data),
                           MPI_BYTE, recvTask, TAG_DENS_A, &TracerFluxListGet[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct tracer_flux_list_data), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
      ...
      if(P[p].Type == 0)
        {
          TracerLinkedList[itr].fluid_quantities[TracerMCExchangeDistanceErrorIndex] +=
              get_cell_radius(p) * (sqrt(TracerLinkedList[itr].fluid_quantities[TracerMCExchangeCounterIndex]) -
                                    sqrt(TracerLinkedList[itr].fluid_quantities[TracerMCExchangeCounterIndex] - 1));
        }
#endif
#endif
...
void start_MC_tracer(int nexport_tracer)
{
  ...
  /* export buffer */
  TracerFluxListIn =
      (struct tracer_flux_list_data *)mymalloc("TracerFluxListIn", nexport_tracer * sizeof(struct tracer_flux_list_data));
}
  ...
  for(ngrp = 0; ngrp < (1 << PTask); ngrp++)
    {
      ...
      MPI_Sendrecv(&tracer_cellids[0],
                   ...
                   MPI_BYTE, recvTask, TAG_DENS_A, &ParentID_Recv[0], Recv_count[recvTask] * sizeof(MyIDType), MPI_BYTE, recvTask,
      ...
      MPI_Sendrecv(&ParentIndex_Remote[0], Recv_count[recvTask] * sizeof(int), MPI_BYTE, recvTask, TAG_DENS_A, &ParentIndex_Recv[0],
...
int get_number_of_tracers(int i)
{
  ...
  if(count != P[i].NumberOfTracers)
    terminate("Mismatch found. i=%d type=%d count=%d P[i].TracerHead=%d P[i].NumberOfTracers=%d", i, P[i].Type, count, P[i].TracerHead,
              P[i].NumberOfTracers);
          ...
          if(P[p].Type == 0)
            {
              TracerLinkedList[move].fluid_quantities[TracerMCExchangeDistanceErrorIndex] +=
                  get_cell_radius(p) * (sqrt(TracerLinkedList[move].fluid_quantities[TracerMCExchangeCounterIndex]) -
...
void remove_tracer_from_parent(int p, int itracer)
{
  ...
  if(P[p].ID != TracerLinkedList[itracer].ParentID)
    terminate("P[p=%d].ID=%lld != TracerLinkedList[itracer=%d].ParentID=%lld\n", p, (long long)P[p].ID, itracer,
              (long long)TracerLinkedList[itracer].ParentID);
#endif
  ...
  if(P[p_from].Type == 0)
    {
      TracerLinkedList[itracer].fluid_quantities[TracerMCExchangeDistanceErrorIndex] +=
          get_cell_radius(p_from) * (sqrt(TracerLinkedList[itracer].fluid_quantities[TracerMCExchangeCounterIndex]) -
                                     sqrt(TracerLinkedList[itracer].fluid_quantities[TracerMCExchangeCounterIndex] - 1));
    }
  if(P[p_to].Type == 0)
    {
      TracerLinkedList[itracer].fluid_quantities[TracerMCExchangeDistanceErrorIndex] +=
          get_cell_radius(p_to) * (sqrt(TracerLinkedList[itracer].fluid_quantities[TracerMCExchangeCounterIndex]) -
                                   sqrt(TracerLinkedList[itracer].fluid_quantities[TracerMCExchangeCounterIndex] - 1));
    }
#endif
  ...
  while(next >= 0) /* traverse forwards to end */
    {
      if(P[p_id].ID != TracerLinkedList[next].ParentID)
        terminate(
            "p_id=%d P[p_id].Type=%d  P[p_id].NumberOfTracers=%d   P[p_id].ID=%lld  next=%d  TracerLinkedList[next].ParentID=%lld   "
            "parent ID mismatch",
            p_id, P[p_id].Type, P[p_id].NumberOfTracers, (long long)P[p_id].ID, next, (long long)TracerLinkedList[next].ParentID);
...
void check_tracer_lists(void)
{
  ...
  if(All.N_alltracer_global != get_total_number_of_tracers(-1))
    terminate("TRACER_MC: N_alltracer_global is not conserved! N_alltracer_global=%lld but current number of tracers is %lld",
