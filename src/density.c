 ...
 * \details     This file contains the "first SPH loop", where the SPH densities and some
 ...
 * number of weighted neighbors in the current smoothing radius, and the divergence
 ...
 * computation is called again.  Note that the smoothing length is
              ...
              if(NumNgb[i] < (desnumngb - All.MaxNumNgbDeviation) || NumNgb[i] > (desnumngb + All.MaxNumNgbDeviation))
                {
                  ...
                  if(NumNgb[i] < (desnumngb - All.MaxNumNgbDeviation))
                    Left[i] = fmax(SphP[i].Hsml, Left[i]);
                  else
                    {
                      if(Right[i] != 0)
                        {
 ...
 */
int density_isactive(const int n)
{
