...
#if(defined(VORONOI) || defined(AMR)) && defined(ONEDIMS)

#ifdef OUTPUT_DIVVEL
static void compute_divvel(void);
#endif
      ...
      for(k = 0; k < N_Grad; k++)
        {
          ...
          // double grad = ((Pos - PosL) * (Value - ValueL) + (Pos - PosR) * (Value - ValueR)) / ((Pos - PosL) * (Pos - PosL) + (Pos -
          // PosR) * (Pos - PosR));
          ...
          double ValueMin = fmin(ValueL, ValueR);
          double ValueMax = fmax(ValueL, ValueR);
