...
void cooling_and_starformation(void)
{
...
#ifdef CHIMES
      /* When CHIMES is included, we first loop through all
       * active particles, evolving the chemistry and cooling
       * for those that are not above the EoS density and
       * colder than the EoS. */
#ifdef CHIMES_PTHREADS
  chimes_cooling_for_starformation_pthreads();
#else
  chimes_cooling_for_starformation();
#endif
#endif
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      if(dens * All.cf_a3inv >= eos_dens_threshold)
#ifdef CHIMES
        if(SphP[i].Utherm <= egyeff || (SphP[i].Sfr > 0 && All.TimeStep == 0) ||
           chimes_convert_ucgs_to_temp(SphP[i].Utherm * All.UnitPressure_in_cgs / All.UnitDensity_in_cgs, i) <= All.TemperatureThresh)
#else
        if(SphP[i].Utherm <= egyeff || (SphP[i].Sfr > 0 && All.TimeStep == 0) ||
           u_to_temp_fac * SphP[i].Utherm <= All.TemperatureThresh)
#endif
          flag = 0;
  ...
  TIMER_START(CPU_COOLSFRIMBAL);
  MPI_Barrier(MPI_COMM_WORLD);
  TIMER_STOP(CPU_COOLSFRIMBAL);
#else
  TIMER_STOP(CPU_COOLINGSFR);
#endif
}
...
double get_starformation_rate(int i)
{
  ...
  if(SphP[i].Density * All.cf_a3inv >= eos_dens_threshold)
    {
#ifdef CHIMES
      egyeff = chimes_calc_egyeff(i, SphP[i].Density * All.cf_a3inv, &x, &tsfr, &factorEVP);
#else
      ne = SphP[i].Ne;
...
#ifdef CHIMES
      terminate(
          "ERROR: init_clouds() is incompatible with CHIMES. When CHIMES is switched on, you cannot set PhysDensThresh to 0 in the "
          "parameter file.\n");
#else
      A0 = All.FactorEVP;
      ...
      while(u >= u4)
        {
          ...
          double mbh =
              sqrt(u * mgas * pow(GAMMA * GAMMA_MINUS1 * u, 1.5) /
                   (dens * tcool * All.BlackHoleFeedbackFactor * All.BlackHoleAccretionFactor * All.BlackHoleRadiativeEfficiency * 4 *
                    M_PI * pow(CLIGHT / All.UnitVelocity_in_cm_per_s, 2) * All.G * All.G));
...
double calc_egyeff(int i, double gasdens, double *ne, double *x, double *tsfr, double *factorEVP)
{
...
#else
#ifdef OTVET_COOLING_HEATING
      tcool = otvet_GetCoolingTime(i, egyhot, rho, ne);
#else /* MRT_COOLING_HEATING */
      // tcool = mrt_GetCoolingTime(i, egyhot, rho, ne); TODO: implement this properly at some point
      tcool = GetCoolingTime(egyhot, rho, ne);
#endif
#endif
    }
  ...
  *x = 1 + 1 / (2 * y) - sqrt(1 / y + 1 / (4 * y * y));
  ...
  if(gasdens < All.PhysDensThresh)
    egyeff = egyeff * (1.0 + (All.FactorUthermAtThresh - 1.0) * (gasdens - All.PhysDensThresh * All.FactorDensThresh) /
                                 (All.PhysDensThresh * (1.0 - All.FactorDensThresh)));
  else if(gasdens < All.JoinDens)
    egyeff = All.FactorUthermAtThresh * All.UthermAtThresh *
             (1.0 + (All.FactorUthermJoin / All.FactorUthermAtThresh - 1.0) * (gasdens - All.PhysDensThresh) /
                        (All.JoinDens - All.PhysDensThresh));
...
double invert_effective_eos(double egyeff_old)
{
  ...
  if(egyeff_old < All.UthermAtThresh)
    terminate("Impossible to invert EOS. egyeff=%g is less than %g the effective energy at the star formation density threshold",
              egyeff_old, All.UthermAtThresh);
