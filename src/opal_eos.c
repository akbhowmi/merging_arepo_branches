...
int get_opal_result(struct opal_eos_table *opal_table, double x, double rho, double t, int mode, struct opal_eos_result *result);
int get_ideal_result(struct opal_eos_table *opal_table, double x, double rho, double t, int mode, struct opal_eos_result *result);
int check_limits(double rho, double t, double *F);
void blend_results(struct opal_eos_result *result, struct opal_eos_result *result_tmp, double F);
          ...
          for(it = 0; it < opal_table->numt[irho]; it++)
            {
              ...
              n = sscanf(line, "%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf", &opal_table->t[it], &dum, &dum, &opal_table->p[it][irho][ix],
                         &opal_table->e[it][irho][ix], &opal_table->s[it][irho][ix], &opal_table->dedrho[it][irho][ix],
                         &opal_table->cv[it][irho][ix], &opal_table->chir[it][irho][ix], &opal_table->chit[it][irho][ix],
                         &opal_table->gamma1[it][irho][ix], &opal_table->gamma2[it][irho][ix]);
              /* consistency checks */
              if(n < 12)
                {
#ifdef OPAL_DEBUG
 ...
 */
void opaleos_deinit_global(void) { opaleos_deinit_local(opal_table); }
 ...
 * @return <0 if error occured, 0 otherwise
 */
int opaleos_all_egiven_local(struct opal_eos_table *opal_table, double x, double rho, double e, double *tguess,
 ...
 * @return <0 if error occured, 0 otherwise
 */
int opaleos_egiven_local(struct opal_eos_table *opal_table, double x, double rho, double e, double *tguess, int mode,
                         struct opal_eos_result *result)
{
      ...
      if(err < 0) /* error in EOS */
        {
          ...
          fprintf(stderr, "Error in iteration %d for X=%e, rho=%e, e=%e, t=%e, mint=%e, maxt=%e\n", i, x, rho, e, t,
                  opal_table->cache->mint, opal_table->maxt);
//          return -1;
#endif
        }
 ...
 * @return <0 if error occured, 0 otherwise
 */
int opaleos_all_pgiven_local(struct opal_eos_table *opal_table, double x, double rho, double p, double *tguess,
                             struct opal_eos_result *result)
{
 ...
 * @return <0 if error occured, 0 otherwise
 */
int opaleos_pgiven_local(struct opal_eos_table *opal_table, double x, double rho, double p, double *tguess, int mode,
                         struct opal_eos_result *result)
{
  double t, told; /* temperature */
  ...
  for(i = 0; i < OPAL_NEWTON_MAX; i++)
    {
      ...
      fprintf(stderr, "Iteration %d for X=%e, rho=%e, p=%e => t=%e, dt=%e, p_n=%e, dp=%e, chit=%e\n", i, x, rho, p, t,
              dp / (result->p / t * result->chit), result->p, dp, result->chit);
#endif
    }
...
int get_eostable_value(struct opal_eos_table *opal_table, eostable var, double x, double rho, double t, double *result)
{
  ...
  for(i = 0; i < 2; i++)
    for(j = 0; j < 2; j++)
      varrt[i][j] = interpolate_linear_slope(opal_table->cache->slopex,
                                             var[opal_table->cache->it + i][opal_table->cache->irho + j] + opal_table->cache->ix);
  ...
  *result = interpolate_linear_slope(opal_table->cache->slopet, vart);
  ...
  int imaxt1 = imin(opal_table->numt[opal_table->cache->irho],
                    imin(opal_table->numt[opal_table->cache->irho + 1], opal_table->numt[opal_table->cache->irho + 2])) -
               1;
  int imaxt2 = imin(opal_table->numt[opal_table->cache->irho + 1],
                    imin(opal_table->numt[opal_table->cache->irho + 2], opal_table->numt[opal_table->cache->irho + 3])) -
               1;
          ...
          for(j = 0; j < 4; j++)
            {
              ...
              varrt[i][j] =
                  interpolate_quadratic_coeff(opal_table->cache->Ax1, opal_table->cache->Bx1, opal_table->cache->Cx1,
                                              var[opal_table->cache->it + i][opal_table->cache->irho + j] + opal_table->cache->ix);
            }
        }
    }
          ...
          for(j = 0; j < 4; j++)
            {
              ...
              varrt[i][j] =
                  interpolate_quadratic_coeff(opal_table->cache->Ax2, opal_table->cache->Bx2, opal_table->cache->Cx2,
                                              var[opal_table->cache->it + i][opal_table->cache->irho + j] + opal_table->cache->ix + 1);
            }
        }
    }
  /* in the middle */
      ...
      for(i = 0; i < 4; i++)
        {
          ...
          if(i == 4 && !bottomleft)
            continue;
          vart1[i] =
              interpolate_quadratic_coeff(opal_table->cache->Arho1, opal_table->cache->Brho1, opal_table->cache->Crho1, varrt[i]);
        }
    }
  /* right border */
      ...
      for(i = 0; i < 4; i++)
        {
          ...
          if(i == 4 && !bottomright)
            continue;
          vart2[i] =
              interpolate_quadratic_coeff(opal_table->cache->Arho2, opal_table->cache->Brho2, opal_table->cache->Crho2, varrt[i] + 1);
        }
    }
  /* in the middle */
  else
 ...
 */
double interpolate_linear(double x, double *xs, double *ys) { return ys[0] + (x - xs[0]) * (ys[1] - ys[0]) / (xs[1] - xs[0]); }
 ...
 * @return interpolated value
 */
double interpolate_linear_slope(double slope, double *ys) { return ys[0] + slope * (ys[1] - ys[0]); }
 ...
 * @return interpolated value
 */
double interpolate_quadratic_coeff(double A, double B, double C, double *ys) { return A * ys[0] + B * ys[1] + C * ys[2]; }
...
int checkmode(int mode)
{
  ...
  if(mode & EOS_RADIATION)
    {
      if((mode & EOS_GAMMA2) && !(mode & EOS_GAMMA1))
        mode = mode | EOS_GAMMA1;
      if((mode & EOS_GAMMA1) && !(mode & EOS_GAMMA3))
        mode = mode | EOS_GAMMA3;
      if((mode & EOS_GAMMA3) && (!(mode & EOS_CHIT) || !(mode & EOS_CHIR) || !(mode & EOS_CV)))
        mode = mode | EOS_CHIT | EOS_CHIR | EOS_CV;
      if((mode & EOS_CHIT) && !(mode & EOS_PRESSURE))
        mode = mode | EOS_PRESSURE;
      if((mode & EOS_CHIR) && !(mode & EOS_PRESSURE))
        mode = mode | EOS_PRESSURE;
    }
  return mode;
}
...
double mean_molecular_weight(double xh, double z)
{
  ...
  /**< number of protons */
  const double mui[6] = {1.0079, 4.0026, 12.011, 14.0067, 15.9994, 20.179};        /**< molecular weight */
  const double xi0[6] = {0.0, 0.0, 0.247137766, 0.0620782, 0.52837118, 0.1624188}; /**< initial mass fraction of metals */
...
int update_cache(struct opal_eos_table *opal_table, double x, double rho, double t)
{
  ...
  if(fabs(opal_table->cache->x - x) < 1e-15 && fabs(opal_table->cache->rho - rho) < 1e-15 && fabs(opal_table->cache->t - t) < 1e-15 &&
  ...
  double mint = fmax(opal_table->mint[opal_table->cache->mrho], opal_table->mint[opal_table->cache->mrho + 1]);
...
#ifdef OPAL_DEBUG
          fprintf(stderr,
                  "Error: T out of range. Value: %e, limits: %e to %e \
for x=%e, rho=%e.\n",
      ...
      int minnum = imin(opal_table->numt[opal_table->cache->mrho], opal_table->numt[opal_table->cache->mrho + 1]);
  ...
  /* set slopes */
  if(fabs(opal_table->cache->x - x) > 1e-15)
    opal_table->cache->slopex =
        (x - opal_table->x[opal_table->cache->ix]) / (opal_table->x[opal_table->cache->ix + 1] - opal_table->x[opal_table->cache->ix]);
  if(fabs(opal_table->cache->rho - rho) > 1e-15)
    opal_table->cache->sloperho = (rho - opal_table->rho[opal_table->cache->irho]) /
                                  (opal_table->rho[opal_table->cache->irho + 1] - opal_table->rho[opal_table->cache->irho]);
  if((fabs(opal_table->cache->t - t) > 1e-15) || (fabs(opal_table->cache->mint - mint) > 1e-15))
    opal_table->cache->slopet =
        (t - opal_table->t[opal_table->cache->it]) / (opal_table->t[opal_table->cache->it + 1] - opal_table->t[opal_table->cache->it]);
#elif INTERPOLATION_METHOD == IP_QUADRATIC
  /* update coefficients only if values change */
  if(fabs(opal_table->cache->x - x) > 1e-15)
    {
      /* set A, B and C coefficients */
      compute_quadratic_coeff(x, opal_table->x + opal_table->cache->ix, &opal_table->cache->Ax1, &opal_table->cache->Bx1,
                              &opal_table->cache->Cx1);
      compute_quadratic_coeff(x, opal_table->x + opal_table->cache->ix + 1, &opal_table->cache->Ax2, &opal_table->cache->Bx2,
                              &opal_table->cache->Cx2);
      /* set slopes for overlapping quadratics */
      opal_table->cache->slopex = (opal_table->x[opal_table->cache->ix + 2] - x) /
                                  (opal_table->x[opal_table->cache->ix + 2] - opal_table->x[opal_table->cache->ix + 1]);
    }
  if(fabs(opal_table->cache->rho - rho) > 1e-15)
    {
      /* set A, B and C coefficients */
      compute_quadratic_coeff(rho, opal_table->rho + opal_table->cache->irho, &opal_table->cache->Arho1, &opal_table->cache->Brho1,
                              &opal_table->cache->Crho1);
      compute_quadratic_coeff(rho, opal_table->rho + opal_table->cache->irho + 1, &opal_table->cache->Arho2, &opal_table->cache->Brho2,
                              &opal_table->cache->Crho2);
      /* set slopes for overlapping quadratics */
      opal_table->cache->sloperho = (opal_table->rho[opal_table->cache->irho + 2] - rho) /
                                    (opal_table->rho[opal_table->cache->irho + 2] - opal_table->rho[opal_table->cache->irho + 1]);
    }
  if((fabs(opal_table->cache->t - t) > 1e-15) || (fabs(opal_table->cache->mint - mint) > 1e-15))
    {
      /* set A, B and C coefficients */
      compute_quadratic_coeff(t, opal_table->t + opal_table->cache->it, &opal_table->cache->At1, &opal_table->cache->Bt1,
                              &opal_table->cache->Ct1);
      compute_quadratic_coeff(t, opal_table->t + opal_table->cache->it + 1, &opal_table->cache->At2, &opal_table->cache->Bt2,
                              &opal_table->cache->Ct2);
      /* set slopes for overlapping quadratics */
      opal_table->cache->slopet = (opal_table->t[opal_table->cache->it + 2] - t) /
                                  (opal_table->t[opal_table->cache->it + 2] - opal_table->t[opal_table->cache->it + 1]);
    }
#endif
