...
int voronoi_proj_setup(int argc, char **argv, struct projection_data *pdata)
{
  ...
  double nn =
      sqrt(pdata->normal1[0] * pdata->normal1[0] + pdata->normal1[1] * pdata->normal1[1] + pdata->normal1[2] * pdata->normal1[2]);
  for(k = 0; k < 3; k++)
    pdata->normal1[k] /= nn;
...
int voronoi_proj_init(struct projection_data *pdata, int pflag)
{
...
#endif
#ifdef MRT
  memset(pdata->photon, 0, pdata->npixels * sizeof(double));
#endif
        ...
        for(k = 0; k < 3; k++)
          {
            if(!pdata->pflag)
              {
                pdata->Ray[pdata->Nray].pos[k] = pdata->center[k] - 0.5 * pdata->boxz * pdata->dir[k] +
                                                 (-0.5 * pdata->boxx + (x + 0.5) * pdata->dx) * pdata->normal1[k] +
                                                 (-0.5 * pdata->boxy + (y + 0.5) * pdata->dy) * pdata->normal2[k];

                pdata->Ray[pdata->Nray].dir[k] = pdata->dir[k];
              }
            else
              {
                pdata->Ray[pdata->Nray].pos[k] = pdata->center[k] - 0.5 * pdata->boxx * pdata->normal1[k] +
                                                 (-0.5 * pdata->boxy + (x + 0.5) * pdata->dx) * pdata->normal2[k] +
                                                 (-0.5 * pdata->boxz + (y + 0.5) * pdata->dy) * pdata->dir[k];
  ...
  // rays
  searchdata = (mesh_search_data *)mymalloc("searchdata", pdata->Nray * sizeof(mesh_search_data));
  ...
  mpi_distribute_items_from_search(searchdata, pdata->Ray, &pdata->Nray, &pdata->MaxNray, sizeof(ray_data), TAG_DENS_A,
                                   offsetof(ray_data, task), offsetof(ray_data, index));
...
int voronoi_proj_run(struct projection_data *pdata)
{
...
#ifdef VORONOI_PROJ_TAU
  double *tauex = 0;
  tauex         = (float *)mymalloc("tauex", pdata->npixels * sizeof(double));
  double *lenex = 0;
  lenex         = (float *)mymalloc("lenex", pdata->npixels * sizeof(double));
#endif
  ...
  if(ThisTask == 0)
    {
      rho = (double *)mymalloc("rho", pdata->npixels * sizeof(double));
#ifdef MHD
      bsqr = (double *)mymalloc("bsqr", pdata->npixels * sizeof(double));
#endif
      ptot = (double *)mymalloc("ptot", pdata->npixels * sizeof(double));
#ifdef GFM_DUST
      metal_rho = (double *)mymalloc("metal_rho", pdata->npixels * sizeof(double));
      dust_rho  = (double *)mymalloc("dust_rho", pdata->npixels * sizeof(double));
      dust_lum  = (double *)mymalloc("dust_lum", pdata->npixels * sizeof(double));
#endif
#ifdef VORONOI_PROJ_TAU
      tau   = (double *)mymalloc("tau", pdata->npixels * sizeof(double));
      tps   = (double *)mymalloc("tps", pdata->npixels * sizeof(double));
      rhops = (double *)mymalloc("rhops", pdata->npixels * sizeof(double));
      len   = (double *)mymalloc("len", pdata->npixels * sizeof(double));
#endif
#ifdef MRT
      photon = (double *)mymalloc("photon", pdata->npixels * sizeof(double));
#endif
    }
...
#endif
#ifdef MRT
  MPI_Reduce(pdata->photon, photon, pdata->npixels, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
...
#endif
#ifdef MRT
      voronoi_proj_write_file("photon", pdata, photon);
#endif
...
void voronoi_proj_write_file(char *name, struct projection_data *pdata, double *data)
{
  ...
  if(!pdata->pflag)
    file_path_sprintf(fname, "%s/vproj_%s_%03d", All.OutputDir, name, RestartSnapNum);
  else
    file_path_sprintf(fname, "%s/vprojp_%s_%03d", All.OutputDir, name, RestartSnapNum);
...
void voronoi_proj_update_column_integral(int cell, double len, int pix, double delta_center[3], struct projection_data *pdata)
{
  ...
  FILE *fd;
#endif
  // SphP[sph_idx].Grad.drho[0] * delta_center[0] + SphP[sph_idx].Grad.drho[1] * delta_center[1] + SphP[sph_idx].Grad.drho[2] *
  // delta_center[2];
#ifdef MHD
  ...
  double phot_sum = 0.0;
  int kk;
  for(kk = 0; kk < MRT_BINS; kk++)
    phot_sum += SphP[cell].DensPhot[kk];
  pdata->photon[pix] += len * phot_sum;
#endif
  ...
  double temp_in_K =
      (SphP[cell].GammaE - 1.0) * SphP[cell].Utherm / BOLTZMANN * All.UnitEnergy_in_cgs / All.UnitMass_in_g * mu * PROTONMASS;
#else
  ...
  double vol   = SphP[cell].Volume * All.UnitLength_in_cm * All.UnitLength_in_cm * All.UnitLength_in_cm /
  ...
  if(!pdata->finished[pix])
    {
      double rho = SphP[cell].Density + SphP[cell].Grad.drho[0] * delta_center[0] + SphP[cell].Grad.drho[1] * delta_center[1] +
                   SphP[cell].Grad.drho[2] * delta_center[2];
      rho = fmax(1e-14, rho);
      /* use cell value */
      // double t = SphP[cell].Temperature;
      /* use T gradient */
      double t = SphP[cell].Temperature + SphP[cell].Grad.dtemp[0] * delta_center[0] + SphP[cell].Grad.dtemp[1] * delta_center[1] +
                 SphP[cell].Grad.dtemp[2] * delta_center[2];
      t             = fmax(1e3, t);
      ...
      if(pdata->tau[pix] > 1.0)
        {
          ...
          pdata->tps[pix] = SphP[cell].Temperature + SphP[cell].Grad.dtemp[0] * delta_center[0] + SphP[cell].Grad.dtemp[1] *
          delta_center[1] + SphP[cell].Grad.dtemp[2] * delta_center[2]; pdata->rhops[pix] = SphP[cell].Density +
          SphP[cell].Grad.drho[0] * delta_center[0] + SphP[cell].Grad.drho[1] * delta_center[1] + SphP[cell].Grad.drho[2] *
          delta_center[2];
          */
        }
#ifdef PROJ_WRITE_RAYS
      if(!pdata->pflag)
        file_path_sprintf(fname, "ray_%06d", pix);
      else
        file_path_sprintf(fname, "rayp_%06d", pix);
  ...
  for(i = 0; i < pdata->Nray; i++)
    {
      ...
      int next_edge = find_next_voronoi_cell(&Mesh, cell, pdata->Ray[i].pos, pdata->Ray[i].dir, pdata->Ray[i].prev, &len);
      ...
      periodic_wrap_point_MyDouble(pdata->Ray[i].pos, &Mesh.DP[DC[next_edge].dp_index].x);
    }
  return nleft;
#endif
}
