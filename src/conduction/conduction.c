      ...
      for(i1 = 0; i1 <= Noc; i1++)
        {
          ...
          zycoeff2 =
              area_kl[i1] * mdls_ltnk[i1] * a * u[j1] / mdls_ysigma[j1] + area_kl[i1] * mdls_ltnk[i1] * b * u[k1] / mdls_ysigma[k1];
#else
          ...
          if(VF[face_id[i1]].area > 1e-10 * SphP[i].SurfaceArea)
            {
              ...
              if((zycoeff1 * (u_K - datatetra[j1].u1) < 0.0) || (zycoeff2 * (u_K - datatetra[j1].u2) < 0.0) ||
                 (zycoeff3 * (u_K - datatetra[j1].u3) < 0.0))
                terminate("|||| Fatal  Error one of a,b,c < 0\n\n");
