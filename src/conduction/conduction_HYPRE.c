...
Implicit Time Integration - Non-linear system ( M(U)U = F(U) ) solved using  Picard iterations with  GMRES and multigrid preconditioner
(using HYPRE library) for the linear system.
 */
...
void init_conductivity(void)
{
  ...
  All.ElectronFreePathFactor = 8 * pow(3.0, 1.5) * pow(GAMMA_MINUS1, 2) / pow(3 + 5 * HYDROGEN_MASSFRAC, 2) / (1 + HYDROGEN_MASSFRAC) /
                               sqrt(M_PI) / coulomb_log * pow(PROTONMASS, 3) / pow(ELECTRONCHARGE, 4) /
          ...
          if(VF[face_id[i1]].area > 1e-10 * SphP[i].SurfaceArea)
            {
              ...
              /*
                 ...
                 printf("particle_id = %d \t datatetra j1 p1 = %d \t p2  = %d \t p3 = %d \n", particle_id[i1], datatetra[j1].p1Noc,
                 datatetra[j1].p2Noc, datatetra[j1].p3Noc) ;
                 }
               */
              int vfidx = face_id[i1];
              int to    = VF[vfidx].p1;
      ...
      if((error / error_init > epsilon_non_max) && (u_error > epsilon_u_err))
        {
          ...
          mpi_printf("CONDUCTION: NON LINEAR ITERATION = %d \t relative error = %le\t initial error = %le\t u_error = %le\n", niter,
                     error / error_init, error_init, u_error);
        }
      else
        break;
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&tmpUExch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct uexch), MPI_BYTE, recvTask,
                           TAG_DENS_A, &UExch[Mesh_Recv_offset[recvTask]], Mesh_Recv_count[recvTask] * sizeof(struct uexch), MPI_BYTE,
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&tmpF2Exch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct f2exch), MPI_BYTE,
                           recvTask, TAG_DENS_A, &F2Exch[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * sizeof(struct f2exch), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
