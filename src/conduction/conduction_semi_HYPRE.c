 ...
 *
 * \file        src/conduction/conduction_semi_HYPRE.c
 * \date        MM/YYYY
 * \author      Rahul Kannan
 * \brief
 * \details
...
/*! \file conduction_semi_HYPRE.c
 *  \brief main driver for an anisotropic/isotropic diffusion solver
 *
 *  This file contains the code for a extremum preserving anisotrpic diffusion solver. This implementation is based on the method
 outlined in Gao & Wu (2013) - Journal of Computational Physics 10/2013; 250:308-331. DOI: 10.1016/j.jcp.2013.05.013 .

 Semi implicit time integration - Split the Non-linear system ( M(U)U = F(U) ) into explicit and implicit parts as in Sharma & Hammett
 2011, Journal of Computational Physics 10.1016/j.jcp.2011.03.009 solved using  GMRES and multigrid preconditioner (using HYPRE
 library).
 */
...
static struct inexch
{
  ...
  double B[DIMS];
#endif
#ifdef NON_LINEAR_SLOPE_LIMITERS
  double Volume, vel[DIMS], gradU[DIMS];
#endif
} * InExch;
  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      double mod_dU = sqrt(SphP[i].Grad.dutherm[0] * SphP[i].Grad.dutherm[0] + SphP[i].Grad.dutherm[1] * SphP[i].Grad.dutherm[1] +
...
void init_conductivity(void)
{
  ...
  All.ElectronFreePathFactor = pow(3.0, 1.5) * GAMMA_MINUS1 * GAMMA_MINUS1 * meanweight_cgs * meanweight_cgs * meanweight_cgs *
                               (3 + HYDROGEN_MASSFRAC) / (8.0 * sqrt(M_PI) * pow(ELECTRONCHARGE, 4) * coulomb_log);

  All.ElectronFreePathFactor /= erg * erg * cm * cm / g / g / g;

  //      All.ElectronFreePathFactor = 8 * pow(3.0, 1.5) * pow(GAMMA_MINUS1, 2) / pow(3 + 5 * HYDROGEN_MASSFRAC, 2) / (1 +
  //      HYDROGEN_MASSFRAC) / sqrt(M_PI) / coulomb_log * pow(PROTONMASS, 3) / pow(ELECTRONCHARGE, 4)  / (All.UnitDensity_in_cgs *
  //      All.HubbleParam * All.HubbleParam) * pow(All.UnitPressure_in_cgs / All.UnitDensity_in_cgs, 2);
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
  double volume_K, volume_L, gradU_K[DIMS], gradU_L[DIMS], vel_L[DIMS], vel_K[DIMS];
#endif
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
      volume_K   = SphP[i].Volume;
      gradU_K[0] = SphP[i].Grad.dutherm[0];
      gradU_K[1] = SphP[i].Grad.dutherm[0];
      vel_K[0]   = P[i].Vel[0];
      vel_K[1]   = P[i].Vel[1];
#endif
      ...
      while(q >= 0)
        {
          ...
          if(Noc >= MaxNoc)
            terminate("Reached Maximum number of allowed neighbours. Increase MaxNoc parameter!");
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
              volume_L   = SphP[particle].Volume;
              gradU_L[0] = SphP[particle].Grad.dutherm[0];
              gradU_L[1] = SphP[particle].Grad.dutherm[1];
              vel_L[0]   = P[particle].Vel[0];
              vel_L[1]   = P[particle].Vel[1];
#endif
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
              volume_L   = InExch[particle].Volume;
              gradU_L[0] = InExch[particle].gradU[0];
              gradU_L[1] = InExch[particle].gradU[1];
              vel_L[0]   = InExch[particle].vel[0];
              vel_L[1]   = InExch[particle].vel[1];
#endif
          ...
          double Lambda_L[DIMS][DIMS], Lambda_star[DIMS][DIMS];

          double LG_star[DIMS];

          int dim1, dim2;

          double cs_K  = sqrt(GAMMA * GAMMA_MINUS1 * u_K);
          double cs_L  = sqrt(GAMMA * GAMMA_MINUS1 * u_L);
          double cstar = (cs_K + cs_L) / 2.0;

          double det_Lambda_star = 0.0;
          for(dim1 = 0; dim1 < DIMS; dim1++)
            {
              for(dim2 = 0; dim2 < DIMS; dim2++)
                {
                  Lambda_L[dim1][dim2]    = Kappa_L * B_L[dim1] * B_L[dim2];
                  Lambda_star[dim1][dim2] = (Lambda_K[dim1][dim2] / volume_K + Lambda_L[dim1][dim2] / volume_L) / 2.0;
                  det_Lambda_star += square(Lambda_star[dim1][dim2]);
                }
            }

          det_Lambda_star = sqrt(det_Lambda_star);

          double gradU_star[DIMS], vstar[DIMS];

          gradU_star[0] = (gradU_K[0] + gradU_L[0]) / 2.0;
          gradU_star[1] = (gradU_K[1] + gradU_L[1]) / 2.0;
          vstar[0]      = (-vel_L[0] + vel_K[0]) / 2.0;
          vstar[1]      = (-vel_L[1] + vel_K[1]) / 2.0;
          ...
          for(dim1 = 0; dim1 < DIMS; dim1++)
            {
              LG_star[0] += Lambda_star[0][dim1] * gradU_star[dim1];
              LG_star[1] += Lambda_star[1][dim1] * gradU_star[dim1];
            }

          // double det_Lambda_star  = Lambda_star[0][0]*Lambda_star[1][1] - Lambda_star[0][1]*Lambda_star[1][0] ;
          double det_gradU_star = sqrt(square(gradU_star[0]) + square(gradU_star[1]));
          double det_LG_star    = sqrt(square(LG_star[0]) + square(LG_star[1]));
          double det_vstar      = sqrt(square(vstar[0]) + square(vstar[1])) / 2.0;
          double rfac           = (det_vstar + cstar) * mdls / det_Lambda_star;
          double alpha_fac;
          if(det_gradU_star == 0.0)
            alpha_fac = ((0.2 + rfac) / (0.2 + rfac + rfac * rfac));
          else
            alpha_fac = (det_LG_star / det_Lambda_star / det_gradU_star) * ((0.2 + rfac) / (0.2 + rfac + rfac * rfac));

          if(alpha_fac != alpha_fac)
            {
              mpi_printf("volume_L = %lf \t volume_K = %lf \n", volume_L, volume_K);
              mpi_printf("vel_L = %lf %lf \t vel_K = %lf %lf \n", vel_L[0], vel_L[1], vel_K[0], vel_K[1]);
              mpi_printf("rfac = %lf \t alpha_fac = %lf \t det_vstar = %lf \t cstar = %lf \t mdls = %lf \t det_Lambda_star = %lf \n",
                         rfac, alpha_fac, det_vstar, cstar, mdls, det_Lambda_star);
              terminate("alpha_fac is NAN\n");
            }

          VF[vf].alpha_fac = alpha_fac;
#endif
  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      volume_K   = SphP[i].Volume;
      gradU_K[0] = SphP[i].Grad.dutherm[0];
      gradU_K[1] = SphP[i].Grad.dutherm[1];
      gradU_K[2] = SphP[i].Grad.dutherm[2];
      vel_K[0]   = P[i].Vel[0];
      vel_K[1]   = P[i].Vel[1];
      vel_K[2]   = P[i].Vel[2];
#endif
      ...
      while(q >= 0)
        {
          ...
          if(DP[dp].task == ThisTask)
            {
              ...
              gradU_L[0] = SphP[particle].Grad.dutherm[0];
              gradU_L[1] = SphP[particle].Grad.dutherm[1];
              gradU_L[2] = SphP[particle].Grad.dutherm[2];
              vel_L[0]   = P[particle].Vel[0];
              vel_L[1]   = P[particle].Vel[1];
              vel_L[2]   = P[particle].Vel[2];
#endif
          ...
          else
            {
              ...
              gradU_L[0] = InExch[particle].gradU[0];
              gradU_L[1] = InExch[particle].gradU[1];
              gradU_L[2] = InExch[particle].gradU[2];
              vel_L[0]   = InExch[particle].vel[0];
              vel_L[1]   = InExch[particle].vel[1];
              vel_L[2]   = InExch[particle].vel[2];
#endif
          ...
          double Lambda_L[DIMS][DIMS][DIMS], Lambda_star[DIMS][DIMS][DIMS];
          ...
          for(dim1 = 0; dim1 < DIMS; dim1++)
            {
              for(dim2 = 0; dim2 < DIMS; dim2++)
                {
                  for(dim3; dim3 < DIMS; dim3++)
                    {
                      Lambda_L[dim1][dim2][dim3] = Kappa_L * B_L[dim1] * B_L[dim2] * B_L[dim3];
                      Lambda_star[dim1][dim2] = (Lambda_K[dim1][dim2][dim3] / volume_K + Lambda_L[dim1][dim2][dim3] / volume_L) / 2.0;
                      det_Lambda_star += square(Lambda_star[dim1][dim2][dim3]);
                    }
                }
              ...
              gradU_star[0] = (gradU_K[0] + gradU_L[0]) / 2.0;
              gradU_star[1] = (gradU_K[1] + gradU_L[1]) / 2.0;
              gradU_star[2] = (gradU_K[2] + gradU_L[2]) / 2.0;
              vstar[0]      = (-vel_L[0] + vel_K[0]) / 2.0;
              vstar[1]      = (-vel_L[1] + vel_K[1]) / 2.0;
              vstar[2]      = (-vel_L[2] + vel_K[2]) / 2.0;
              ...
              for(dim1 = 0; dim1 < DIMS; dim1++)
                {
                  LG_star[0] += Lambda_star[0][dim1] * gradU_star[dim1];
                  LG_star[1] += Lambda_star[1][dim1] * gradU_star[dim1];
                  LG_star[2] += Lambda_star[2][dim1] * gradU_star[dim1];
                }

              // double det_Lambda_star  = Lambda_star[0][0]*Lambda_star[1][1] - Lambda_star[0][1]*Lambda_star[1][0] ;
              double det_gradU_star = sqrt(square(gradU_star[0]) + square(gradU_star[1]) + square(gradU_star[2]));
              double det_LG_star    = sqrt(square(LG_star[0]) + square(LG_star[1]) + square(LG_star[2]));
              double det_vstar      = sqrt(square(vstar[0]) + square(vstar[1]) + square(vstar[2])) / 2.0;
              double rfac           = (det_vstar + cstar) * mdls / det_Lambda_star;
              double alpha_fac;
              if(det_gradU_star == 0.0)
                alpha_fac = ((0.2 + rfac) / (0.2 + rfac + rfac * rfac));
              ...
              if(alpha_fac != alpha_fac)
                {
                  mpi_printf("density_L = %lf \t density_K = %lf \n", density_L, density_K);
                  mpi_printf("vel_L = %lf %lf \t vel_K = %lf %lf \n", vel_L[0], vel_L[1], vel_K[0], vel_K[1]);
                  mpi_printf(
                      "rfac = %lf \t alpha_fac = %lf \t det_vstar = %lf \t cstar = %lf \t mdls = %lf \t det_Lambda_star = %lf \n",
                      rfac, alpha_fac, det_vstar, cstar, mdls, det_Lambda_star);
                  terminate("alpha_fac is NAN\n");
                }
          ...
          if(VF[vf].doface == 1)
            {
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
              double alpha_fac = Mesh.VF[vf].alpha_fac;
#endif
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
              double mu_K = (mod(F2_L) + epsilon) / (mod(F2_K) + mod(F2_L) + 2.0 * epsilon);
#endif
              if(index == P[i].ID)
                {
              ...
              double F_real = mu_K * F2_K - (1.0 - mu_K) * F2_L;

              double F_slope = beta * F2_K;
              double F_num   = alpha_fac * (F_real - F_slope);

              double F_mod = minmod(1.1 * F_real, F_real + F_num);
              double F_final;
              if((sign(F_real) != sign(F_mod)) && (mod(F_real) > 0.5 * mod(F_mod)))
                F_final = 0.0;
              else
                F_final = F_mod;
...
void linear_iterations(double dt)
{
  ...
  HYPRE_IJMatrixCreate(MPI_COMM_WORLD, fld_offsets[ThisTask], fld_offsets[ThisTask + 1] - 1, fld_offsets[ThisTask],
                       fld_offsets[ThisTask + 1] - 1, &A);
              ...
              if((task = ListExports[listp].origin) != ThisTask)
                {
                  ...
                  tmpInExch[off].vel[0] = P[place].Vel[0];
                  tmpInExch[off].vel[1] = P[place].Vel[1];
#ifndef TWODIMS
                  tmpInExch[off].vel[2] = P[place].Vel[2];
#endif
                  tmpInExch[off].gradU[0] = SphP[place].Grad.dutherm[0];
                  tmpInExch[off].gradU[1] = SphP[place].Grad.dutherm[1];
#ifndef TWODIMS
                  tmpInExch[off].gradU[2] = SphP[place].Grad.dutherm[2];
#endif
#endif
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&tmpInExch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct inexch), MPI_BYTE,
                           recvTask, TAG_DENS_A, &InExch[Mesh_Recv_offset[recvTask]],
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
                      tmpFluxExch[off].alpha_fac[init] = 0.0;
#endif
                    }
...
#ifdef NON_LINEAR_SLOPE_LIMITERS
                              tmpFluxExch[off].alpha_fac[Noc] = VF[vf].alpha_fac;
#endif
      ...
      if(recvTask < NTask)
        {
          if(Mesh_Send_count[recvTask] > 0 || Mesh_Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&tmpFluxExch[Mesh_Send_offset[recvTask]], Mesh_Send_count[recvTask] * sizeof(struct fluxexch), MPI_BYTE,
                           recvTask, TAG_DENS_A, &FluxExch[Mesh_Recv_offset[recvTask]],
                           Mesh_Recv_count[recvTask] * sizeof(struct fluxexch), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
...
double MC(double a, double b) { return minmod(2.0 * minmod(a, b), (a + b) / 2.0); }
