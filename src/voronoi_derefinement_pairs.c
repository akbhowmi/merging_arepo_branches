...
int do_derefinements(void)
{
  ...
  for(idx = 0; idx < NActiveParticles; idx++)
    {
      ...
      if(SphP[i].DerefPartnerId > 0)
        {
          /* check partner index */
          if(SphP[i].DerefPartnerIndex >= NumGas || SphP[i].DerefPartnerIndex < 0 ||
             P[SphP[i].DerefPartnerIndex].ID != SphP[i].DerefPartnerId)
      ...
      if(SphP[i].DerefPartnerId > 0 && SphP[i].DerefPartnerIndex >= 0)
        {
          ...
          printf("Task: %d, ID=%d, PID=%d, dist=%g, thres=%g\n", ThisTask, P[i].ID, SphP[i].DerefPartnerId, dist,
                 0.01 * (get_cell_radius(i) + get_cell_radius(j)));
  ...
  mpi_printf(
      "DEREFINE: Number of cells we de-refined: %d, Number of pairs left: %d (lost: %d), failed to find partner for %d particles.\n",
  ...
  while(q >= 0)
    {
      ...
      /* discard cells on other tasks and mirrored cells */
      if(DC[q].task != ThisTask || DC[q].image_flags != 1 || particle >= NumGas || particle < 0 ||
         SphP[particle].DerefPartnerId != 0 || !TimeBinSynchronized[P[particle].TimeBinHydro])
        {
          if(q == SphP[i].last_connection)
            break;
...
void voronoi_derefinement_merge_pair(int i, int j)
{
  ...
  for(s = 0; s < N_Scalar; s++)
    *(MyFloat *)(((char *)(&SphP[keep])) + scalar_elements[s].offset_mass) +=
        *(MyFloat *)(((char *)(&SphP[del])) + scalar_elements[s].offset_mass);
#endif
#ifdef TRACER_FIELD
