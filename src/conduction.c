...
void conduction(void)
{
  ...
  mpi_printf("CONDUCTION: Start thermal conduction...\n");
  ...
  mpi_printf("CONDUCTION: dt=%g   All.ConductionCoeff=%g\n", dt, All.ConductionCoeff);
  ...
  while(iter < MAX_COND_ITER && glob_max_rel_change > COND_ITER_ACCURACY && delta1 > 0)
    {
      ...
      mpi_printf("CONDUCTION: conduction iter=%d  delta1=%g delta1/delta0=%g  max-rel-change=%g\n", iter, delta1, delta1 / delta0,
                 glob_max_rel_change);
    }
  ...
  mpi_printf("\nCONDUCTION: conduction finished. energy_before=%g erg, energy_after=%g erg, rel-change=%g rel-transfer=%g\n\n",
...
#ifdef CONDUCTION_ANISOTROPIC
void conduction_PCG_anisotropic(double *in, double *out) { terminate("here you can insert a more complicated preconditioner\n"); }
#endif /* CONDUCTION_ANISOTROPIC */
...
void conduction_matrix_multiply_anisotropic(double *in, double *out, double *diag, double theta)
{
  ...
  double *res = (double *)mymalloc("res", NumGas * sizeof(double));
