...
int do_refinements(void)
{
  ...
  ref_SphP_dp_index = (int *)mymalloc_movable(&ref_SphP_dp_index, "ref_SphP_dp_index", NumGas * sizeof(int));
  ...
  if(countall)
    {
      ...
      int *list = (int *)mymalloc("list", NTask * sizeof(int));
      ...
      ngb       = (struct ngb_data *)mymalloc("ngbs", 2 * Mesh.Nvf * sizeof(struct ngb_data));
          ...
          if(should_this_cell_be_split(i))
            {
...
#ifdef CHIMES
              SphP[j].ChimesGasVars.index = j;
              chimes_set_pointers(j);
              chimes_set_pointers(i);
              for(abunIndex = 0; abunIndex < ChimesGlobalVars.totalNumberOfSpecies; abunIndex++)
                SphP[j].ChimesGasVars.abundances[abunIndex] = SphP[i].ChimesGasVars.abundances[abunIndex];
#endif  // CHIMES

              rad = get_cell_radius(i);

              double dir[3];
#ifdef REFINEMENT_SPLIT_MOST_DISTANCE_NEIGHBOUR
              double maxdist2 = 0;

              k = first_ngb[ref_SphP_dp_index[i]];
              while(k >= 0)
                {
                  int q = ngb[k].index;

                  double dx    = Mesh.DP[q].x - P[i].Pos[0];
                  double dy    = Mesh.DP[q].y - P[i].Pos[1];
                  double dz    = Mesh.DP[q].z - P[i].Pos[2];
...
#endif
                      DeRefMesh.DP -= 5;
                      DeRefMesh.DP = (point *)myrealloc_movable(DeRefMesh.DP, (DeRefMesh.MaxNdp + 5) * sizeof(point));
                      DeRefMesh.DP += 5;
                    }
              ...
              double *Volume = (double *)mymalloc("Volume", DeRefMesh.Ndp * sizeof(double));
              ...
              if(fmax(voli, volj) > 0.95 * (voli + volj))
                {
                  /*printf( "REFINEMENT: Task=%d, abandoning refining cell %lld (vol=%g) into cells with voli=%g, volj=%g, ratio:
                    %g:1\n", ThisTask, (long long)P[i].ID, SphP[i].Volume, voli, volj, fmax(voli, volj) / fmin(voli, volj) );*/

                  /* mark this cell as derefined -> we still need to increase count to increase NumGas in case this was the last cell
                   * to be added! */
                  P[j].ID   = 0;
                  P[j].Mass = 0;
                  for(k = 0; k < 3; k++)
                    SphP[j].VelVertex[k] = 0;
...
//#endif
              ...
              for(k = 0; k < 3; k++)
                {
                  ...
                  SphP[j].B[k] = SphP[j].BConserved[k] / (voli + volj);
                  /*
                  SphP[j].B[k] =
                      SphP[i].B[k] + SphP[i].Grad.dB[k][0] * (P[j].Pos[0] - P[i].Pos[0]) +
                      SphP[i].Grad.dB[k][1] * (P[j].Pos[1] - P[i].Pos[1]) +
                      SphP[i].Grad.dB[k][2] * (P[j].Pos[2] - P[i].Pos[2]); */
                  /* extrapolate B to the position of the new cell */
...
#ifdef MHD_DEDNER
              SphP[i].PsiConserved *= faci;
              SphP[j].PsiConserved *= facj;
#endif
#endif
              ...
              for(k = 0; k < 3; k++)
                {
                  ...
                  SphP[j].A[k] =
                      SphP[i].A[k] + SphP[i].Grad.dA[k][0] * (P[j].Pos[0] - P[i].Pos[0]) +
                      SphP[i].Grad.dA[k][1] * (P[j].Pos[1] - P[i].Pos[1]) +
                      SphP[i].Grad.dA[k][2] * (P[j].Pos[2] - P[i].Pos[2]); /* extrapolate A to the position of the new cell */
      ...
      int count_cancelled_all;
      MPI_Allreduce(&count_cancelled, &count_cancelled_all, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
      mpi_printf("REFINE: cancelled refinement of %d cells\n", count_cancelled_all);
    }
