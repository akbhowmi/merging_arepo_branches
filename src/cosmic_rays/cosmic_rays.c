  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      double dt_cell = (P[i].TimeBinHydro ? (((integertime)1) << P[i].TimeBinHydro) : 0) * All.Timebase_interval /
                       All.cf_hubble_a; /* timestep of the cell in Arepo units */
      double rho = SphP[i].Density * All.UnitDensity_in_cgs * All.cf_a3inv * All.HubbleParam *
                   All.HubbleParam; /* physical mass density in cgs */
      double Gamma_heating = 4.02e-16 * rho / PROTONMASS * (1. - 0.5 * (1. - HYDROGEN_MASSFRAC)) *
                             All.UnitTime_in_s; /* collisional heating rate in Arepo units */
      double Gamma = 1.022e-15 * rho / PROTONMASS * (1. - 0.5 * (1. - HYDROGEN_MASSFRAC)) *
                     All.UnitTime_in_s; /* total CR cooling rate in Arepo units */
      ...
      if(Gamma > 0.)  // don't heat if rho == 0
        SphP[i].Energy += SphP[i].CR_Energy * Gamma_heating / Gamma * (1. - exp(-Gamma * dt_cell)) * All.cf_atime;
      SphP[i].CR_Energy *= exp(-Gamma * dt_cell);
      SphP[i].CR_Energy = fmax(SphP[i].CR_Energy, All.MinimumCREnergyDensity * SphP[i].Volume);
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      double prod = (SphP[i].B[0] * SphP[i].Grad.dcrPressure[0] + SphP[i].B[1] * SphP[i].Grad.dcrPressure[1] +
                     SphP[i].B[2] * SphP[i].Grad.dcrPressure[2]) /
                    b;

      double dCR_Energy = fmax(0., fmin(dt_cell * vel_Alfven * fabs(prod) * SphP[i].Volume / sqrt(All.cf_atime),
