...
static void compute_quantities_at_corners(void);
...
static void diffusion_explicit(void);
static void diffusion_implicit(void);
...
void compute_quantities_at_corners(void)
{
  int icorner, k;
  for(icorner = 0; icorner < Mesh.Ndt; icorner++)
    {
...
void compute_diffusion_coefficients(void)
{
      ...
      if(recvTask < NTask)
        {
          if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
            {
              MPI_Sendrecv(&CRFluxList[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct crflux_list_data), MPI_BYTE,
                           recvTask, TAG_DENS_A, &CRFluxListGet[Recv_offset[recvTask]],
...
void diffusion_implicit(void)
{
      ...
      if(failed)
        {
          if(fullNormalGradients)
            {
              mpi_printf(
                  "COSMIC_RAYS: DIFFUSION: Changing to simple normal gradients and repeating implicit step with multigrid "
                  "preconditioner.\n");
  ...
  mpi_printf(
      "COSMIC_RAYS: DIFFUSION: Total CR Energy (min/max density) of all participating cells changed from %g (%g,%g) to %g (%g,%g)\n",
  ...
  if(ThisTask == out.rank && residual_index != -1)
    {
      ...
      printf(
          "COSMIC_RAYS: DIFFUSION: Particle %d on task %d has the highest residual res=%g, b=%g. eCR=%g (before=%g), Volume=%g, "
