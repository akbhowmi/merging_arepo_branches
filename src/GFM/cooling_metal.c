...
void allocate_cooling_tables(void)
{
  ...
  coolingMetalTable.HydrogenNumberDensity_bins =
      (MyFloat *)mymalloc("hydrogenNumberDensity_bins", *n_hydrogenNumberDensity * sizeof(MyFloat));
      ...
      for(j = 0; j < *n_bolFlux; j++)
        {
          ...
          for(m = 0; m < *n_metallicityInSolar; m++)
            {
              coolingMetalTable.NetCoolingRate[i][j][m] =
                  (MyFloat **)mymalloc("netCoolingRate", *n_hydrogenNumberDensity * sizeof(MyFloat *));
              for(k = 0; k < *n_hydrogenNumberDensity; k++)
                {
                  coolingMetalTable.NetCoolingRate[i][j][m][k] =
                      (MyFloat *)mymalloc("netCoolingRate", *n_temperature * sizeof(MyFloat));
                }
            }
        }
    }
      ...
      for(j = 0; j < *n_Phios; j++)
        {
          ...
          for(n = 0; n < *n_Phins; n++)
            {
              coolingMetalTable.NetCoolingRate[i][j][n] =
                  (float **)mymalloc("netCoolingRate", *n_hydrogenNumberDensity * sizeof(float *));
              coolingMetalTable.NetHeatingRate[i][j][n] =
                  (float **)mymalloc("netHeatingRate", *n_hydrogenNumberDensity * sizeof(float *));
              for(k = 0; k < *n_hydrogenNumberDensity; k++)
                {
      ...
      for(j = 0; j < *n_Phios_lz; j++)
        {
          ...
          for(n = 0; n < *n_Phins_lz; n++)
            {
              coolingMetalTable.NetCoolingRateLowZ[i][j][n] =
                  (float *****)mymalloc("netCoolingRateLowZ", *n_PhiT6 * sizeof(float ****));
              coolingMetalTable.NetHeatingRateLowZ[i][j][n] =
                  (float *****)mymalloc("netHeatingRateLowZ", *n_PhiT6 * sizeof(float ****));
              for(idx6 = 0; idx6 < *n_PhiT6; idx6++)
                {
                  coolingMetalTable.NetCoolingRateLowZ[i][j][n][idx6] =
                      (float ****)mymalloc("netCoolingRateLowZ", *n_PhiT7 * sizeof(float ***));
                  coolingMetalTable.NetHeatingRateLowZ[i][j][n][idx6] =
                      (float ****)mymalloc("netHeatingRateLowZ", *n_PhiT7 * sizeof(float ***));
                  for(idx7 = 0; idx7 < *n_PhiT7; idx7++)
                    {
                      coolingMetalTable.NetCoolingRateLowZ[i][j][n][idx6][idx7] =
                          (float ***)mymalloc("netCoolingRateLowZ", *n_PhiT8 * sizeof(float **));
                      coolingMetalTable.NetHeatingRateLowZ[i][j][n][idx6][idx7] =
                          (float ***)mymalloc("netHeatingRateLowZ", *n_PhiT8 * sizeof(float **));
                      for(idx8 = 0; idx8 < *n_PhiT8; idx8++)
                        {
                          coolingMetalTable.NetCoolingRateLowZ[i][j][n][idx6][idx7][idx8] =
                              (float **)mymalloc("netCoolingRateLowZ", *n_hydrogenNumberDensity * sizeof(float *));
                          coolingMetalTable.NetHeatingRateLowZ[i][j][n][idx6][idx7][idx8] =
                              (float **)mymalloc("netHeatingRateLowZ", *n_hydrogenNumberDensity * sizeof(float *));
                          for(k = 0; k < *n_hydrogenNumberDensity; k++)
                            {
                              coolingMetalTable.NetCoolingRateLowZ[i][j][n][idx6][idx7][idx8][k] =
                                  (float *)mymalloc("netCoolingRateLowZ", *n_temperature * sizeof(float));
                              coolingMetalTable.NetHeatingRateLowZ[i][j][n][idx6][idx7][idx8][k] =
                                  (float *)mymalloc("netHeatingRateLowZ", *n_temperature * sizeof(float));
                            }
                        }
                    }
                }
  ...
  for(i = 0; i < 2; i++)
    {
      ...
      for(m = 0; m < *n_metallicityInSolar; m++)
        {
          coolingMetalTable.NetCoolingRate[i][m] =
              (MyFloat **)mymalloc("netCoolingRate", *n_hydrogenNumberDensity * sizeof(MyFloat *));
...
void read_cooling_tables_init(void)
{
  ...
  dummyvar = 0; /* this redshift initialization is just added to fix a strange linking problem on Mac OSX - without it, the file
                   cooling_metal_vars.c is somehow not linked in */
  ...
  mpi_printf("GFM_COOLING_METAL: reading %010llu/%010d Bytes\n", (unsigned long long)H5Dget_storage_size(dataset), tmpdbllen);
  ...
  hsize_t offset[5] = {(hsize_t)cur_redshift_low_index, 0, 0, 0, 0};
...
#ifdef RADCOOL_HOTHALO
  tmpfllen_lz =
      2 * *n_Phios_lz * *n_Phins_lz * *n_PhiT6 * *n_PhiT7 * *n_PhiT8 * *n_hydrogenNumberDensity * *n_temperature * sizeof(float);
  hsize_t offset_lz[8] = {(hsize_t)cur_redshift_low_index_lz, 0, 0, 0, 0, 0, 0, 0};
  ...
  hsize_t offset[4] = {(hsize_t)cur_redshift_low_index, 0, 0, 0};
...
#ifdef GFM_AGN_RADIATION
      file_path_sprintf(buf, "%s/cooling_metal_bins_AGN_Compton.txt", All.OutputDir);
#else
#ifdef RADCOOL
      file_path_sprintf(buf, "%s/cooling_metal_bins_RADCOOL.txt", All.OutputDir);
#ifdef RADCOOL_HOTHALO
      file_path_sprintf(buf, "%s/cooling_metal_bins_RADCOOL_HOTHALO.txt", All.OutputDir);
#endif
#else
      file_path_sprintf(buf, "%s/cooling_metal_bins_UVB.txt", All.OutputDir);
#endif
#endif
      ...
      if(cur_redshift_low_index_lz > 0 && All.cf_redshift < redshift_bins_lz[cur_redshift_low_index_lz])
        {
          ...
          dummyvar = 0; /* this redshift initialization is just added to fix a strange linking problem on Mac OSX - without it, the
                           file cooling_metal_vars.c is somehow not linked in */

          tmpfllen = 2 * *n_Phios_lz * *n_Phins_lz * *n_PhiT6 * *n_PhiT7 * *n_PhiT8 * *n_hydrogenNumberDensity * *n_temperature *
                     sizeof(float);
...
*/
#ifdef GFM_AGN_RADIATION
MyFloat get_CoolingMetalRate(MyFloat log_BolFlux, MyFloat log_MetallicityInSolar, MyFloat log_HydrogenNumberDensity,
                             MyFloat log_Temperature)
#else
#ifdef RADCOOL
...
MyFloat get_CoolingMetalRate(MyFloat log_MetallicityInSolar, MyFloat log_HydrogenNumberDensity, MyFloat log_Temperature,
                             MyFloat *cool_val, MyFloat *lower_cool, MyFloat *upper_cool, MyFloat *lower_logT, MyFloat *upper_logT)
#else
MyFloat get_CoolingMetalRate(MyFloat log_MetallicityInSolar, MyFloat log_HydrogenNumberDensity, MyFloat log_Temperature)
#endif
#endif
...
#ifdef UVB_OFF
  i1  = 0;
  d_i = 1.0;  // i.e. use earliest redshift bin available
#else
  if(All.cf_redshift > redshift_bins[0])
    {
      ...
      if(i1 != 0)
        terminate("GFM_COOLING_METAL: i1 != 0: i1=%d i2=%d cur_redshift_low_index=%d redshift_bins[i1+cur_redshift_low_index]=%g", i1,
                  i2, cur_redshift_low_index, redshift_bins[i1 + cur_redshift_low_index]);
      if(i2 != 1)
        terminate("GFM_COOLING_METAL: i2 != 1: i1=%d i2=%d cur_redshift_low_index=%d redshift_bins[i2+cur_redshift_low_index]=%g", i1,
                  i2, cur_redshift_low_index, redshift_bins[i2 + cur_redshift_low_index]);
      ...
      if(All.cf_redshift > redshift_bins_lz[0])
        {
          ...
          if(i1 != 0)
            terminate(
                "GFM_COOLING_METAL: i1 != 0: i1=%d i2=%d cur_redshift_low_index=%d redshift_bins_lz[i1+cur_redshift_low_index]=%g", i1,
                i2, cur_redshift_low_index_lz, redshift_bins_lz[i1 + cur_redshift_low_index_lz]);
          if(i2 != 1)
            terminate(
                "GFM_COOLING_METAL: i2 != 1: i1=%d i2=%d cur_redshift_low_index=%d redshift_bins_lz[i2+cur_redshift_low_index]=%g", i1,
                i2, cur_redshift_low_index_lz, redshift_bins_lz[i2 + cur_redshift_low_index_lz]);
      ...
      if(All.cf_redshift > redshift_bins[0])
        {
          ...
          i2 -= cur_redshift_low_index;
          if(i1 != 0)
            terminate("GFM_COOLING_METAL: i1 != 0: i1=%d i2=%d cur_redshift_low_index=%d redshift_bins[i1+cur_redshift_low_index]=%g",
                      i1, i2, cur_redshift_low_index, redshift_bins[i1 + cur_redshift_low_index]);
          if(i2 != 1)
            terminate("GFM_COOLING_METAL: i2 != 1: i1=%d i2=%d cur_redshift_low_index=%d redshift_bins[i2+cur_redshift_low_index]=%g",
                      i1, i2, cur_redshift_low_index, redshift_bins[i2 + cur_redshift_low_index]);
          if(All.cf_redshift >= redshift_bins[0] && All.cf_redshift <= redshift_bins[*n_redshift - 1])
            d_i_local = All.cf_redshift - redshift_bins[i1 + cur_redshift_low_index];
          else
            d_i_local = 0;
  ...
  if(log_MetallicityInSolar > metallicityInSolar_bins[0])
    {
      ...
      if(log_MetallicityInSolar >= metallicityInSolar_bins[0] &&
         log_MetallicityInSolar <= metallicityInSolar_bins[*n_metallicityInSolar - 1])
  ...
  if(log_HydrogenNumberDensity > hydrogenNumberDensity_bins[0])
    {
      ...
      if(log_HydrogenNumberDensity >= hydrogenNumberDensity_bins[0] &&
         log_HydrogenNumberDensity <= hydrogenNumberDensity_bins[*n_hydrogenNumberDensity - 1])
  ...
  if(log_Temperature > temperature_bins[0])
    {
      ...
      if(l2 >= *n_temperature)
        l2 = *n_temperature - 1;
#ifdef MRT_METAL_COOLING
      *upper_logT = temperature_bins[l2];
      *lower_logT = temperature_bins[l1];
      //  printf("%g %g %d %d\n", *lower_logT, *upper_logT, l1, l2) ;
#endif
...
#ifdef MRT_METAL_COOLING
  *cool_val        = coolingMetalRate;
  *lower_cool      = interpolate_4d(netCoolingRate, i1, m1, k1, l1, d_i, d_m, d_k, 0.0);
  *upper_cool      = interpolate_4d(netCoolingRate, i1, m1, k1, l2, d_i, d_m, d_k, 0.0);
  // printf("%g %g %g %g %g %d %d\n", *cool_val, *lower_cool, *upper_cool, *lower_logT, *upper_logT, l1, l2) ;
#endif
...
void init_cooling_metal(void) { read_cooling_tables_init(); }

void dump_cooling_table(void)
{
  int i, k, l;
  char buf[MAXLEN_PATH];

  file_path_sprintf(buf, "%s/%s", All.OutputDir, "coolingtable.txt");
