...
static double get_agn_luminosity(MyFloat bh_mdot, MyFloat bh_mass)
{
  ...
  double meddington = (4 * M_PI * GRAVITY * CLIGHT * PROTONMASS / (All.BlackHoleRadiativeEfficiency * CLIGHT * CLIGHT * THOMPSON)) *
                      bh_mass * All.UnitTime_in_s / All.HubbleParam;
  ...
  /* ADAF efficiency (Narayan & Yi 1994) with continuous transition to All.BlackHoleRadiativeEfficiency */
    ...
    {
      Lbol = ADAFparam1 * ADAFparam2 * mdot_in_edd / (1.0 + ADAFparam2 * mdot_in_edd) * All.BlackHoleRadiativeEfficiency *
             mdot_in_cgs * CLIGHT * CLIGHT;
  ...
  if(!gsl_finite(Lbol))
    terminate("GFM_AGN_RADIATION: Lbol=%g mdot_in_edd=%g mdot_in_cgs=%g bh_mdot=%g meddington=%g bh_mass=%g", Lbol, mdot_in_edd,
              mdot_in_cgs, bh_mdot, meddington, bh_mass);
...
void assign_agn_radiation(void)
{
  ...
  rhocrit *=
      (All.Omega0 * pow(1 + All.cf_redshift, 3) + (1 - All.Omega0 - All.OmegaLambda) * pow(1 + All.cf_redshift, 2) + All.OmegaLambda);
  ...
  nH_mean =
      HYDROGEN_MASSFRAC * All.OmegaBaryon * 3 * All.Hubble * All.Hubble / (8 * M_PI * All.G) / PROTONMASS * All.UnitDensity_in_cgs;
  ...
  if(All.HighestSynchronizedTimeBin >= All.HighestOccupiedTimeBin)
    {
      ...
      if(totFlagActive > 0)
        mpi_printf(
            "GFM_AGN_RADIATION: FlagActive=%d  RadiativeEfficient=%d (%g%)  RadiativeInefficient=%d (%g%)  MinMdotInEddington=%g  "
            "MaxMdotInEddington=%g\n",
