...
void do_winds(void)
{
  ...
  WindParticle = (struct wind_particle *)mymalloc("WindParticle", MaxNwind * sizeof(struct wind_particle));
  ...
  mpi_printf(
      "GFM_WINDS: energy statistics (step)       EnergyIs[internal units]=%e EnergyShould[internal units]=%e Ratio=%g EnergyIs/formed "
      "stellar mass[erg/M_sun]=%e\n",
      WindEnergy_Is, WindEnergy_Should, (WindEnergy_Should > 0) ? (WindEnergy_Is / WindEnergy_Should) : (0),
      (global_stellar_mass > 0) ? (global_SNII_energy / global_stellar_mass) : (0));

  mpi_printf("GFM_WINDS: energy statistics (cumulative) EnergyIs[internal units]=%e EnergyShould[internal units]=%e Ratio=%g\n",
             All.CumWindEnergy_Is, All.CumWindEnergy_Should,
             (All.CumWindEnergy_Should > 0) ? (All.CumWindEnergy_Is / All.CumWindEnergy_Should) : (0));
#endif
  ...
  for(i = 0; i < Nwind; i++)
    {
      ...
      if((WindParticle[i].Density * All.cf_a3inv < All.WindFreeTravelDensFac * All.PhysDensThresh) ||
         (STP(WindParticle[i].index).BirthTime >= 0))
        STP(WindParticle[i].index).BirthTime = 0;

#if !defined(GFM_WINDS_THERMAL) && !defined(GFM_WINDS_THERMAL_NEWDEF) && !defined(CHIMES)
      /* now do the cooling */
      ...
      update_gas_state(WindParticle[i].Density * All.cf_a3inv, HYDROGEN_MASSFRAC, STP(WindParticle[i].index).Metallicity);
#else
      update_gas_state(WindParticle[i].Density * All.cf_a3inv,
                       STP(WindParticle[i].index).MassMetals[element_index_Hydrogen] / P[WindParticle[i].index].Mass,
                       STP(WindParticle[i].index).Metallicity);
#endif
#endif
      double ne = 1.0;
      double unew =
          DoCooling(fmax(All.MinEgySpec, STP(WindParticle[i].index).Utherm), WindParticle[i].Density * All.cf_a3inv, dtime, &ne);
...
#ifndef STATICHQ
  double HostHaloMass = SphP[i].w.HostHaloMass;
  double DMVelDisp    = SphP[i].w.DMVelDisp;
#else
  double HostHaloMass = HQ_M200;
  double DMVelDisp    = 0.0;
#endif
  if(HostHaloMass > 0 && stellar_mass > 0)
    {
      wind_parameter wp;
      gfm_calc_variable_wind_parameters(stellar_mass, HostHaloMass, DMVelDisp, SphP[i].Metallicity, &wp);
...
#error "GFM_VARIABLE_IMF mode is not ok"
  ...
  *v_wind       = sqrt(2 * All.WindEnergyFactor * FactorSN * WindEgySpecSN / (1 - FactorSN) / All.WindEfficiency);
  ...
  SphP[i].MassLoading = (p > 0.0 ? *p_wind / p : 0.0);
  SphP[i].WindVel     = *v_wind;
#endif
}
#endif
...
void gfm_add_wind(int i, double v, double utherm)
{
...
#else
  dir[0]        = (GravAccel[1] - SphP[i].GroupGravAcc[1]) * ((SphP[i].Momentum[2] / P[i].Mass) - SphP[i].GroupVel[2]) -
           (GravAccel[2] - SphP[i].GroupGravAcc[2]) * ((SphP[i].Momentum[1] / P[i].Mass) - SphP[i].GroupVel[1]);
  dir[1] = (GravAccel[2] - SphP[i].GroupGravAcc[2]) * ((SphP[i].Momentum[0] / P[i].Mass) - SphP[i].GroupVel[0]) -
           (GravAccel[0] - SphP[i].GroupGravAcc[0]) * ((SphP[i].Momentum[2] / P[i].Mass) - SphP[i].GroupVel[2]);
  dir[2] = (GravAccel[0] - SphP[i].GroupGravAcc[0]) * ((SphP[i].Momentum[1] / P[i].Mass) - SphP[i].GroupVel[1]) -
           (GravAccel[1] - SphP[i].GroupGravAcc[1]) * ((SphP[i].Momentum[0] / P[i].Mass) - SphP[i].GroupVel[0]);
#endif
#endif
#else
  ...
  STP(i).HighResMassCGM = SphP[i].HighResMassCGM;
#endif
}
...
void gfm_spawn_wind_from_cell(int igas, double v, double utherm, int istar, MyFloat mass_of_wind)
{
  ...
  dir[0] = (GravAccel[1] - SphP[igas].GroupGravAcc[1]) * ((SphP[igas].Momentum[2] / P[igas].Mass) - SphP[igas].GroupVel[2]) -
           (GravAccel[2] - SphP[igas].GroupGravAcc[2]) * ((SphP[igas].Momentum[1] / P[igas].Mass) - SphP[igas].GroupVel[1]);
  dir[1] = (GravAccel[2] - SphP[igas].GroupGravAcc[2]) * ((SphP[igas].Momentum[0] / P[igas].Mass) - SphP[igas].GroupVel[0]) -
           (GravAccel[0] - SphP[igas].GroupGravAcc[0]) * ((SphP[igas].Momentum[2] / P[igas].Mass) - SphP[igas].GroupVel[2]);
  dir[2] = (GravAccel[0] - SphP[igas].GroupGravAcc[0]) * ((SphP[igas].Momentum[1] / P[igas].Mass) - SphP[igas].GroupVel[1]) -
           (GravAccel[1] - SphP[igas].GroupGravAcc[1]) * ((SphP[igas].Momentum[0] / P[igas].Mass) - SphP[igas].GroupVel[0]);
#endif
#endif
#else
  theta                = acos(2 * get_random_number() - 1);
...
#endif
#ifdef REFINEMENT_CGM
  STP(istar).HighResMassCGM = SphP[igas].HighResMassCGM * (1 - fac);
#endif
