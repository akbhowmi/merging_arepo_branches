 ...
 * http://www.nscl.msu.edu/~nero/db/docs/part_frdm.asc one file containing the reaction rates in the REACLIB 2 format (described in
 * http://www.nscl.msu.edu/~nero/db/docs/reaclibFormat.pdf) [rates.txt] you can download it e.g. from
 * http://www.nscl.msu.edu/~nero/db/library.php?action=download one file containing the binding energies of the nuclei [masses.txt] you
 * can download it e.g. from http://www.nndc.bnl.gov/masses/ | http://www.nndc.bnl.gov/masses/mass.mas03 one file containing weak
 * reactions e.g. by langanke 2001
 */
...
void screening_apply(double rho, double temp, double ye, double yz, const struct network_data *nd, struct network_workspace *nw);
static void compute_constant_screening_factors(struct network_rate *rate, const struct network_nucdata *nuc1,
                                               const struct network_nucdata *nuc2, const struct network_data *nd);
static network_var compute_screening(const struct network_rate *rate, network_var itemp13, network_var Gamma_prime,
                                     network_var lGamma_prime, network_var Gamma_prime14, network_var H_w_common);
...
static __inline __attribute__((unused)) network_var network_var_const_add(const network_var a, const double b);
...
static __inline __attribute__((unused)) network_var network_var_div(const network_var a, const network_var b);
...
static __inline __attribute__((unused)) network_var network_var_log(const network_var a);
static __inline __attribute__((unused)) network_var network_var_pow_int(const network_var a, int n);
...
static int network_init_internal(char *speciesfile, char *ratesfile, char *partfile, char *massesfile, char *weakratesfile,
                                 struct network_data *nd, int onlyweak);
...
int network_init_onlyweak(char *speciesfile, char *ratesfile, char *partfile, char *massesfile, char *weakratesfile,
                                 ...
                                 struct network_data *nd, int onlyweak)
{
  ...
  if(!(fd = fopen(speciesfile, "r")))
    terminate("can't open file `%s' for reading species information.\n", speciesfile);
  ...
  nd->nucdata = (struct network_nucdata *)malloc(nd->nuc_count * sizeof(struct network_nucdata));
  ...
  masses = (char *)calloc(nd->nuc_count, sizeof(char));

  if(!(fd = fopen(massesfile, "r")))
    terminate("can't open file `%s' for reading mass excess.\n", massesfile);
  ...
  for(i = 0; i < nd->nuc_count; i++)
    {
      ...
      nd->nucdata[i].m =
          ...
          nd->nucdata[i].exm * conv / GSL_CONST_NUM_AVOGADRO / (GSL_CONST_CGS_SPEED_OF_LIGHT * GSL_CONST_CGS_SPEED_OF_LIGHT);
    }
  ...
  if(ThisTask == 0)
    {
      myprintf("Loaded nuclei:\n");
      for(i = 0; i < nd->nuc_count; i++)
        myprintf("%s A=%d, Z=%d, exm=%g\n", nd->nucdata[i].name, nd->nucdata[i].na, nd->nucdata[i].nz, nd->nucdata[i].exm);
    }
  ...
  spins = (char *)calloc(nd->nuc_count, sizeof(char));

  if(!(fd = fopen(partfile, "r")))
    {
      terminate("can't open file `%s' for reading partition functions.\n", partfile);
    }
                  ...
                  for(k = 0; k < 8; k++)
                    {
                      ...
                      if(tmp == next_field)
                        {
                          terminate("Error reading partition function data for element %s\n", nd->nucdata[i].name);
                        }
                      next_field                     = tmp;
  ...
  nd->weakrate_count = 0;
  nallocated         = 1024;
  nd->weakrates      = (struct network_weakrate *)malloc(nallocated * sizeof(struct network_weakrate));

  if(!(fd = fopen(weakratesfile, "r")))
    {
      terminate("can't open file `%s' for reading weak rate data.\n", weakratesfile);
    }
      ...
      if(nucEductId >= 0 && nucProductId >= 0)
        {
          ...
          if(rc == nallocated)
            {
              nallocated += 1024;
              nd->weakrates = (struct network_weakrate *)realloc(nd->weakrates, nallocated * sizeof(struct network_weakrate));
            }
  ...
  nd->weakrates = (struct network_weakrate *)realloc(nd->weakrates, nd->weakrate_count * sizeof(struct network_weakrate));
  ...
  count = (int *)malloc(nd->nuc_count * sizeof(int));

  nd->rate_count = 0;
  nallocated     = 1024;
  nd->rates      = (struct network_rate *)malloc(nallocated * sizeof(struct network_rate));

  if(!(fd = fopen(ratesfile, "r")))
    {
      terminate("can't open file `%s' for reading rate data.\n", ratesfile);
    }
      ...
      if(needed)
        {
          ...
          if(rc == nallocated)
            {
              nallocated += 1024;
              nd->rates = (struct network_rate *)realloc(nd->rates, nallocated * sizeof(struct network_rate));
            }
  ...
  /* free memory that is not needed */
  nd->rates = (struct network_rate *)realloc(nd->rates, nd->rate_count * sizeof(struct network_rate));

  fclose(fd);

  for(i = 0; i < nd->nuc_count; i++)
    {
      nd->nucdata[i].rates  = (struct network_rate *)malloc(nd->nucdata[i].nrates * sizeof(struct network_rate));
      nd->nucdata[i].prates = (struct network_rate **)malloc(nd->nucdata[i].nrates * sizeof(struct network_rate *));
      nd->nucdata[i].irates = (size_t *)malloc(nd->nucdata[i].nrates * sizeof(size_t));
      nd->nucdata[i].w      = (double *)malloc(nd->nucdata[i].nrates * sizeof(double));

      nd->nucdata[i].weakrates  = (struct network_weakrate *)malloc(nd->nucdata[i].nweakrates * sizeof(struct network_weakrate));
      nd->nucdata[i].pweakrates = (struct network_weakrate **)malloc(nd->nucdata[i].nweakrates * sizeof(struct network_weakrate *));
      nd->nucdata[i].iweakrates = (size_t *)malloc(nd->nucdata[i].nweakrates * sizeof(size_t));
      nd->nucdata[i].wweak      = (double *)malloc(nd->nucdata[i].nweakrates * sizeof(double));
    }

  ratecount = (int *)calloc(nd->nuc_count, sizeof(int));
              ...
              if(rc == nd->nucdata[j].nrates)
                {
                  ...
                  terminate("rate %d, %s: ratecount too low (%d <> %zu), stopping.\n", i, nd->nucdata[j].name, rc,
                            nd->nucdata[j].nrates);
                }
  ...
  nd->electronCapture_rates = (size_t *)malloc(nd->electronCapture_count * sizeof(size_t));
  nd->electronCapture_count = 0;
  for(i = 0; i < nd->rate_count; i++)
    {
      if(nd->rates[i].isElectronCapture)
  ...
  ratecount            = (int *)calloc(nd->rate_count, sizeof(int));
  nd->screened_count   = 0;
  ...
  nd->screened_rates = (struct network_rate **)malloc(nd->screened_count * sizeof(struct network_rate *));
  nd->screened_count = 0;
  for(i = 0; i < nd->rate_count; i++)
    {
      if(ratecount[i])
...
int network_workspace_init(const struct network_data *nd, struct network_workspace *nw)
{
  ...
  nw->y      = (double *)malloc(nd->n_matrix * sizeof(double));
  nw->gg     = (network_var *)malloc(nd->nuc_count * sizeof(network_var));
  nw->matrix = (double *)malloc(nd->n_matrix * nd->n_matrix * sizeof(double));

  nw->old_temp = -1.0;
  nw->old_rho  = -1.0;
  nw->old_ye   = -1.0;

  nw->baserate        = (network_var *)malloc(nd->rate_count * sizeof(network_var));
  nw->unscreened_rate = (network_var *)malloc(nd->rate_count * sizeof(network_var));
  nw->rates           = (network_var *)malloc(nd->rate_count * sizeof(network_var));
  nw->weakrates       = (network_var *)malloc(nd->weakrate_count * sizeof(network_var));
  nw->yrates          = (network_var *)malloc(nd->rate_count * sizeof(network_var));
  nw->yweakrates      = (network_var *)malloc(nd->weakrate_count * sizeof(network_var));

  nw->prates     = (network_var ***)malloc(nd->nuc_count * sizeof(network_var **));
  nw->pweakrates = (network_var ***)malloc(nd->nuc_count * sizeof(network_var **));

  for(i = 0; i < nd->nuc_count; i++)
    {
      nw->prates[i] = (network_var **)malloc(nd->nucdata[i].nrates * sizeof(network_var *));
      for(j = 0; j < nd->nucdata[i].nrates; j++)
        {
          nw->prates[i][j] = &nw->yrates[nd->nucdata[i].irates[j]];
        }
      nw->pweakrates[i] = (network_var **)malloc(nd->nucdata[i].nweakrates * sizeof(network_var *));
  ...
  nw->dTdYi   = (network_var *)malloc(nd->nuc_count * sizeof(network_var));
  nw->drhodYi = (network_var *)malloc(nd->nuc_count * sizeof(network_var));
...
int network_getrhs(double rho, double temp, const double y[], int compute_derivs, const struct network_data *nd,
                   struct network_workspace *nw, double *rhs, network_var *deriv)
{
  int j;
  double ne, nn, ye, yz;
  ...
  for(yrate = nw->yrates, ratedata = nd->rates, rateval = nw->rates; ratedata != rateend; yrate++, ratedata++, rateval++)
    {
  ...
  for(yrate = nw->yweakrates, weakratedata = nd->weakrates, rateval = nw->weakrates; weakratedata != weakrateend;
      yrate++, weakratedata++, rateval++)
    {
      /* The derivative w.r.t. y[i] is treated separately. */
  ...
  nucend = &nd->nucdata[nd->nuc_count];
  for(nucdata = nd->nucdata, rhsiter = deriv, pratelist = nw->prates, pweakratelist = nw->pweakrates; nucdata != nucend;
      nucdata++, rhsiter++, pratelist++, pweakratelist++)
    {
      network_var a = {0};
      network_var **prate;
      double *w, *wend;
  ...
  {
    int i;
    network_var dedT_var = {0}, dedtime = {0};
    network_var dpdrho = {0}, dpdT = {0}, dedrho = {0};
#ifndef NETWORK_NEGLECT_DTDY_TERMS
    network_var dedA = {0}, dedZ = {0}, dpdA = {0}, dpdZ = {0};
...
#error "unknown mode"
        ...
        for(i = 0; i < nd->nuc_count; i++)
          {
            network_var dZdy = {.v = abar.v * (nd->nucdata[i].nz - zbar.v), .dabar = nd->nucdata[i].nz - zbar.v, .dzbar = -abar.v};
...
int network_getjacob(const double y[], const network_var *rhs, const struct network_data *nd, struct network_workspace *nw,
                     jacob_t *jacob)
{
  double abar, zbar, ye;
  double dabardyi;
  ...
  if(nw->old_ye != ye || changed)
    {
      ...
      for(i = 0; i < nd->electronCapture_count; i++)
        {
          nw->unscreened_rate[nd->electronCapture_rates[i]] = n_m(nw->unscreened_rate[nd->electronCapture_rates[i]], ye_v);
        }
  ...
  if(changed)
    {
      ...
      if(nd->weakrate_count > 0)
        {
          network_var logrhoye = {.v = log10(rho * ye), .drho = 1.0 / (rho * log(10)), .dYe = 1.0 / (ye * log(10))};
          network_var dt = {0}, drhoye = {0};
...
int network_part(double temp, const struct network_data *nd, struct network_workspace *nw)
{
  ...
  static const double network_parttemp[24] = {1.0e8, 1.5e8, 2.0e8, 3.0e8, 4.0e8, 5.0e8, 6.0e8, 7.0e8, 8.0e8, 9.0e8, 1.0e9, 1.5e9,
                                              2.0e9, 2.5e9, 3.0e9, 3.5e9, 4.0e9, 4.5e9, 5.0e9, 6.0e9, 7.0e9, 8.0e9, 9.0e9, 1.0e10};
                                               ...
                                               const struct network_nucdata *nuc2, const struct network_data *nd)
{
  ...
  rate->tau_fac = pow(27. / 2. * gsl_pow_2(M_PI) * (a1 * a2) / (a1 + a2) * gsl_pow_2(z1 * z2) * gsl_pow_4(ELECTRON_CHARGE_ESU) /
                          (GSL_CONST_NUM_AVOGADRO * GSL_CONST_CGS_BOLTZMANN * gsl_pow_2(GSL_CONST_CGS_PLANCKS_CONSTANT_HBAR)),
                      1. / 3.);
   ...
   */
  rate->H_w_fac =
      gsl_pow_3(ELECTRON_CHARGE_ESU) * sqrt(4.0 * M_PI * GSL_CONST_NUM_AVOGADRO / gsl_pow_3(GSL_CONST_CGS_BOLTZMANN)) * z1 * z2;
...
void screening_apply(double rho, double temp, double ye, double yz, const struct network_data *nd, struct network_workspace *nw)
{
  ...
  const network_var temp_var = {.v = temp, .dT = 1.0}, rho_var = {.v = rho, .drho = 1.0}, ye_var = {.v = ye, .dYe = 1.0},
                    yz_var = {.v = yz, .dYz = 1.0};
  ...
  Gamma_prime   = n_cm(n_d(n_cbrt(ne), temp_var), gsl_pow_2(ELECTRON_CHARGE_ESU) / GSL_CONST_CGS_BOLTZMANN * cbrt(4. / 3. * M_PI));
  ...
  if(nd->tripleAlpha_rate != NULL)
    {
      ...
      H = n_a(compute_screening(&nd->alpha_alpha_rate, itemp13, Gamma_prime, lGamma_prime, Gamma_prime14, H_w_common),
              compute_screening(&nd->Be8_alpha_rate, itemp13, Gamma_prime, lGamma_prime, Gamma_prime14, H_w_common));
  ...
  if(Gamma.v >= 0.3)
    {
      ...
      H_s = n_a(C, n_a(n_a(n_a(n_m(n_powi(b, 3), n_cm(tau, -5. / (32. * 3.))),
                               n_m(n_powi(b, 4), n_s(n_cm(tau, 0.014 / 3.), n_cm(Gamma, 0.0055)))),
                           n_m(n_powi(b, 5), n_a(n_cm(tau, 0.0128 / 3.), n_cm(Gamma, 0.0098)))),
                       n_m(n_powi(b, 6), n_cm(Gamma, -0.0048))));
    }
