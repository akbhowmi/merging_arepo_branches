...
#ifdef DVR_RENDER
static short int *SaveTimeBin;
static int SaveTimeBinActive[TIMEBINS];
#endif
...
void create_mesh(void)
{
...
#ifdef FREE_DC_BEFORE_FULL_MESH_CONSTRUCTION
  if(All.HighestActiveTimeBin == All.HighestOccupiedTimeBin)
    {
      /* we need to add all cell anyway, so let's empty DC before */
      MaxNvc = 0;
      Nvc    = 0;
      ...
      DC = (connection *)myrealloc_movable(DC, MaxNvc * sizeof(connection));
      mpi_printf("VORONOI: Emptied DC because we construct the full mesh anyway.\n");
    }
#endif
#endif

  MaxNinlist  = Mesh.Indi.AllocFacNinlist;
  ListExports = (struct list_export_data *)mymalloc_movable(&ListExports, "ListExports", MaxNinlist * sizeof(struct list_export_data));

  NumGasInMesh = 0;
  List_InMesh  = (int *)mymalloc_movable(&List_InMesh, "List_InMesh", NumGas * sizeof(int));

  List_P = (struct list_P_data *)mymalloc_movable(&List_P, "List_P", NumGas * sizeof(struct list_P_data));
  ...
  Mesh.DTC = (tetra_center *)mymalloc_movable(&Mesh.DTC, "DTC", Mesh.MaxNdt * sizeof(tetra_center));
  Mesh.DTF = (char *)mymalloc_movable(&Mesh.DTF, "DTF", Mesh.MaxNdt * sizeof(char));
  for(int i = 0; i < Mesh.Ndt; i++)
    Mesh.DTF[i] = 0;
  ...
  for(int i = 0; i < Mesh.Ndp; i++)
    {
#if defined(REFLECTIVE_X) && (REFLECTIVE_X == 2)
      Mesh.DP[i].image_flags |= OUTFLOW_X;
#endif
#if defined(REFLECTIVE_Y) && (REFLECTIVE_Y == 2)
      Mesh.DP[i].image_flags |= OUTFLOW_Y;
#endif
#if defined(REFLECTIVE_Z) && (REFLECTIVE_Z == 2)
...
#ifdef AXISYMMETRY
  compute_axisymmetric_geometry_factors(&Mesh);
#endif
...
/*! \brief Routine that makes sure there is enough space for new delauny points
 *
 */

void voronoi_check_and_increase_dp_array(tessellation *T, int new_points)
{
  if(T->Ndp + new_points >= T->MaxNdp)
    {
...
#endif
              ListExports = (struct list_export_data *)myrealloc_movable(ListExports, MaxNinlist * sizeof(struct list_export_data));
...
#endif
              Mesh.DP -= 5;
              Mesh.DP = (point *)myrealloc_movable(Mesh.DP, (Mesh.MaxNdp + 5) * sizeof(point));
              Mesh.DP += 5;
...
#ifdef VORONOI_MESH_KEEP_DT_AND_DTC
...
#if defined(VORONOI_DYNAMIC_UPDATE) && defined(DOUBLE_STENCIL)
...
void compute_voronoi_faces_and_volumes(void)
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
...
#ifdef ACTIVE_CELL_SPIN
      {
        ...
        for(m = 0; m < 3; m++)
          for(o = 0; o < 3; o++)
            SphP[i].MomentOfInertia[m][o] = 0.;
      }
#endif
    }

  Edge_visited = (unsigned char *)mymalloc_movable(&Edge_visited, "Edge_visited", Mesh.Ndt * sizeof(unsigned char));
  ...
  Narea    = 0;
  AreaList = (struct area_list_data *)mymalloc_movable(&AreaList, "AreaList", MaxNarea * sizeof(struct area_list_data));
...
#ifdef ACTIVE_CELL_SPIN
#ifdef TWODIMS
      /* prevent maxtrix from becoming singular */
      SphP[i].MomentOfInertia[0][0] = 1.0;
      SphP[i].MomentOfInertia[1][1] = 1.0;
      /* moment of inertia is already relative to Center */
#else
      /* 3D -> we have to move the tensor from the mesh-generating point to the center of mass of the cell */
      {
        double dx = NEAREST_X(SphP[i].Center[0] - P[i].Pos[0]);
        double dy = NEAREST_Y(SphP[i].Center[1] - P[i].Pos[1]);
        double dz = NEAREST_Z(SphP[i].Center[2] - P[i].Pos[2]);

        SphP[i].MomentOfInertia[0][0] -= SphP[i].Volume * (dy * dy + dz * dz);
        SphP[i].MomentOfInertia[1][1] -= SphP[i].Volume * (dx * dx + dz * dz);
        SphP[i].MomentOfInertia[2][2] -= SphP[i].Volume * (dx * dx + dy * dy);
        SphP[i].MomentOfInertia[0][1] += SphP[i].Volume * dx * dy;
        SphP[i].MomentOfInertia[1][0] += SphP[i].Volume * dx * dy;
        SphP[i].MomentOfInertia[0][2] += SphP[i].Volume * dx * dz;
        SphP[i].MomentOfInertia[2][0] += SphP[i].Volume * dx * dz;
        SphP[i].MomentOfInertia[1][2] += SphP[i].Volume * dy * dz;
        SphP[i].MomentOfInertia[2][1] += SphP[i].Volume * dy * dz;
      }
#endif
#endif
    }
...
void derefine_refine_compute_volumes(double *vol)
{
  ...
  Edge_visited = (unsigned char *)mymalloc_movable(&Edge_visited, "Edge_visited", DeRefMesh.Ndt * sizeof(unsigned char));
...
double get_cell_radius_from_volume(double Volume)
{
  double cellrad;
#ifdef ONEDIMS
  terminate("This does not work.");
#endif
#ifdef TWODIMS
  cellrad = sqrt(Volume / M_PI);
#else
  cellrad = pow(Volume * 3.0 / (4.0 * M_PI), 1.0 / 3);
#endif
  return cellrad;
}
#endif
 ...
 *  \param[out] geo Pointer to structure to which normal data is written.
 *
 *  \return 0 if success, -1 if interface can be ignored.
 */
int face_get_normals(tessellation *T, int i, struct geometry *geo)
{
  int li, ri;
  ...
  geo->nx = DP[VF[i].p2].x - DP[VF[i].p1].x;
  geo->ny = DP[VF[i].p2].y - DP[VF[i].p1].y;
  geo->nz = DP[VF[i].p2].z - DP[VF[i].p1].z;

  geo->nn = sqrt(geo->nx * geo->nx + geo->ny * geo->ny + geo->nz * geo->nz);
  geo->nx /= geo->nn;
  geo->ny /= geo->nn;
  geo->nz /= geo->nn;

  /* need an ortonormal basis */
  if(geo->nx != 0 || geo->ny != 0)
    {
      geo->mx = -geo->ny;
      geo->my = geo->nx;
      geo->mz = 0;
  ...
  mm = sqrt(geo->mx * geo->mx + geo->my * geo->my + geo->mz * geo->mz);
  geo->mx /= mm;
  geo->my /= mm;
  geo->mz /= mm;

  geo->px = geo->ny * geo->mz - geo->nz * geo->my;
  geo->py = geo->nz * geo->mx - geo->nx * geo->mz;
  geo->pz = geo->nx * geo->my - geo->ny * geo->mx;
...
void voronoi_make_new_tessellation(void)
{
  ...
  SaveTimeBin = (short int *)mymalloc_movable(&SaveTimeBin, "SaveTimeBin", NumPart * sizeof(short int));

  for(i = 0; i < NumPart; i++)
    {
      SaveTimeBin[i] = P[i].TimeBinHydro;
#ifdef DVR_RENDER
      if(P[i].Type == 0 && SphP[i].DoMesh == 1)
#endif
        P[i].TimeBinHydro = 0;
    }
  ...
  for(i = 0; i < TIMEBINS; i++)
    {
      SaveTimeBinActive[i] = TimeBinSynchronized[i];
#ifdef DVR_RENDER
      if(P[i].Type == 0 && SphP[i].DoMesh == 1)
#endif
...
void voronoi_restore_old_tessellation(void)
{
  ...
  for(i = 0; i < TIMEBINS; i++)
    TimeBinSynchronized[i] = SaveTimeBinActive[i];

  for(i = 0; i < NumPart; i++)
    P[i].TimeBinHydro = SaveTimeBin[i];

  reconstruct_timebins();

  myfree_movable(SaveTimeBin);
}
