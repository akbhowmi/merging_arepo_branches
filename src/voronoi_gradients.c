 ...
 * \date        MM/YYYY
 * \author
 * \brief       Algorithms to calculate the gradients simulations.
 * \details      For all except 1d simulations. This uses Green-Gauss
 *               gradients, which are outdated and give a worse than second
 *               order convergence rate for smooth flows. For the new gradient
 *               estimate, see voronoi_gradients_lsf.c
 *
 *
...
#if((defined(VORONOI) || (defined(AMR) && !defined(AMR_GRADIENTS))) && defined(GRADIENTS_GREEN_GAUSS)) || \
    (defined(VORONOI) && !defined(VORONOI_DYNAMIC_UPDATE))
#ifndef ONEDIMS

static double atime, hubble_a;

/*! \brief Calculates gradients.
 *
 *  Using the Green-Gauss method described in Springel et al. (2010).
 *
 *  \return void
 */
void calculate_gradients(void)
{
  ...
  double M0 = 0, M1 = 0;
#endif /* #ifdef USE_ENTROPY_FOR_COLD_FLOWS */
  ...
  for(k = 0; k < N_Grad; k++)
    {
      grad_elements[k].min_value = (double *)mymalloc("gradmin", NumGas * sizeof(double));
      grad_elements[k].max_value = (double *)mymalloc("gradmax", NumGas * sizeof(double));
    }

  curl = (double *)mymalloc("curl", NumGas * 3 * sizeof(double));
    ...
    defined(BOUNDARY_REFL_SOLIDSIDE_MAXID)
      {
        ...
        if((id0 >= BOUNDARY_REFL_FLUIDSIDE_MINID && id0 < BOUNDARY_REFL_FLUIDSIDE_MAXID && id1 >= BOUNDARY_REFL_SOLIDSIDE_MINID &&
            id1 < BOUNDARY_REFL_SOLIDSIDE_MAXID) ||
           (id1 >= BOUNDARY_REFL_FLUIDSIDE_MINID && id1 < BOUNDARY_REFL_FLUIDSIDE_MAXID && id0 >= BOUNDARY_REFL_SOLIDSIDE_MINID &&
            id0 < BOUNDARY_REFL_SOLIDSIDE_MAXID))
          {
            m[0] = 0.5 * (p1->x + p0->x);
            m[1] = 0.5 * (p1->y + p0->y);
            m[2] = 0.5 * (p1->z + p0->z);
            ...
            if(id1 >= BOUNDARY_REFL_FLUIDSIDE_MINID && id1 < BOUNDARY_REFL_FLUIDSIDE_MAXID && id0 >= BOUNDARY_REFL_SOLIDSIDE_MINID &&
               id0 < BOUNDARY_REFL_SOLIDSIDE_MAXID)
              {
                for(k = 0; k < N_Grad; k++)
                  {
...
#endif /* #if defined(BOUNDARY_REFL_FLUIDSIDE_MINID) && defined(BOUNDARY_REFL_FLUIDSIDE_MAXID) && \
          ...
          if(TimeBinSynchronized[P[q0].TimeBinHydro])
            {
...
#ifdef REFLECTIVE_X
                  if(((*image_flags0 & REFL_X_FLAGS) && (*image_flags0 & OUTFLOW_X)) ||
                     ((*image_flags1 & REFL_X_FLAGS) && (*image_flags1 & OUTFLOW_X)))
                    data[0] = 0;
#endif /* #ifdef REFLECTIVE_X */
#ifdef REFLECTIVE_Y
                  if(((*image_flags0 & REFL_Y_FLAGS) && (*image_flags0 & OUTFLOW_Y)) ||
                     ((*image_flags1 & REFL_Y_FLAGS) && (*image_flags1 & OUTFLOW_Y)))
                    data[1] = 0;
#endif /* #ifdef REFLECTIVE_Y */
#ifdef REFLECTIVE_Z
                  if(((*image_flags0 & REFL_Z_FLAGS) && (*image_flags0 & OUTFLOW_Z)) ||
                     ((*image_flags1 & REFL_Z_FLAGS) && (*image_flags1 & OUTFLOW_Z)))
                    data[2] = 0;
              ...
              SphP[q0].DivVel += facA * (c[0] * (GVelx->value0 - GVelx->value1) + c[1] * (GVely->value0 - GVely->value1) +
                                         c[2] * (GVelz->value0 - GVelz->value1));
#endif /* #if defined (OUTPUT_DIVVEL) || defined(TGCHEM) || defined(SGCHEM) */
            }
        }
          ...
          if(TimeBinSynchronized[P[q1].TimeBinHydro])
            {
              ...
              SphP[q1].DivVel += facB * (c[0] * (GVelx->value1 - GVelx->value0) + c[1] * (GVely->value1 - GVely->value0) +
                                         c[2] * (GVelz->value1 - GVelz->value0));
#endif /* #if defined (OUTPUT_DIVVEL) || defined(TGCHEM) || defined(SGCHEM) */
            }
        }
    }
  ...
  if(SfVars.limit_gradients)
    {
#endif /* #if defined(SHOCK_FINDER_BEFORE_OUTPUT) || defined(SHOCK_FINDER_ON_THE_FLY) */
                          ...
                          for(k = 0; k < N_Grad; k++)
                            {
                              ...
                              // if(p->ID != -1 && p->ID != -2)
#endif /* #ifdef SPECIAL_BOUNDARY */
                              // if((grad_elements[k].type != GRADIENT_TYPE_AX) && (grad_elements[k].type != GRADIENT_TYPE_AY) &&
                              // (grad_elements[k].type != GRADIENT_TYPE_AZ) && (grad_elements[k].type != GRADIENT_TYPE_ASHIFT))
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      ...
      if(i < 0)
        continue;
      SphP[i].CurlVel =
          sqrt(curl[3 * i + 0] * curl[3 * i + 0] + curl[3 * i + 1] * curl[3 * i + 1] + curl[3 * i + 2] * curl[3 * i + 2]);
    }
...
#endif /* #if defined(VS_TURB) || defined(AB_TURB) || defined(CALCULATE_QUANTITIES_IN_POSTPROCESS) */
...
void calculate_vertex_velocity_divergence(void)
{
  ...
  double fac0, fac1, n[3], nn, velGas0[3] = {0, 0, 0}, velGas1[3] = {0, 0, 0};
          ...
          if(TimeBinSynchronized[P[q0].TimeBinHydro])
            {
              ...
              SphP[q0].DivVelVertex +=
                  facA * (c[0] * (velGas0[0] - velGas1[0]) + c[1] * (velGas0[1] - velGas1[1]) + c[2] * (velGas0[2] - velGas1[2]));
            }
        }
          ...
          if(TimeBinSynchronized[P[q1].TimeBinHydro])
            {
              ...
              SphP[q1].DivVelVertex +=
                  facB * (c[0] * (velGas1[0] - velGas0[0]) + c[1] * (velGas1[1] - velGas0[1]) + c[2] * (velGas1[2] - velGas0[2]));
            }
        }
    }
}
