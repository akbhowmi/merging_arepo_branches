          ...
          if(Todo[i])
            {
              if((PSIDM[i].NumNgb < (All.SIDMDesNumNgb - All.SIDMMaxNumNgbDeviation)) ||
                 (PSIDM[i].NumNgb > (All.SIDMDesNumNgb + All.SIDMMaxNumNgbDeviation)))
                {
                  /* need to redo this particle */
                  npleft++;

                  if(PSIDM[i].NumNgb < All.SIDMDesNumNgb - All.SIDMMaxNumNgbDeviation)
                    Left[i] = fmax(PSIDM[i].Hsml, Left[i]);
                  else
                    {
                      if(Right[i] != 0)
                        {
                  ...
                  if(iter >= MAXITER - 10)
                    {
                      printf("SIDM: i=%d task=%d Hsml=%g Left=%g Right=%g Ngbs=%g Right-Left=%g\n", i, ThisTask, PSIDM[i].Hsml,
                             Left[i], Right[i], (double)PSIDM[i].NumNgb, Right[i] - Left[i]);
                      fflush(stdout);
                    }
              ...
              if((r2 < h2) && (r2 > 0) && ((1 << P[no].Type) & (SIDM)))
                {
                  ...
                  Ekin = SIDM_avel * SIDM_avel *
                         (0.5 * in_mass_1 * (vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2]) +
                          0.5 * in_mass_2 * (P[no].Vel[0] * P[no].Vel[0] + P[no].Vel[1] * P[no].Vel[1] + P[no].Vel[2] * P[no].Vel[2]));
              ...
              if((r2 < h2) && (r2 > 0) && ((1 << (Tree_Points[n].Type)) & (SIDM)))
                {
                  ...
                  out.VelDisp[state] += Tree_Points[n].Vel[0] * Tree_Points[n].Vel[0] + Tree_Points[n].Vel[1] * Tree_Points[n].Vel[1] +
                                        Tree_Points[n].Vel[2] * Tree_Points[n].Vel[2];
                  ...
                  Ekin = SIDM_avel * SIDM_avel *
                         (0.5 * in_mass_1 * (vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2]) +
                          0.5 * in_mass_2 *
                              (Tree_Points[n].Vel[0] * Tree_Points[n].Vel[0] + Tree_Points[n].Vel[1] * Tree_Points[n].Vel[1] +
                               Tree_Points[n].Vel[2] * Tree_Points[n].Vel[2]));
