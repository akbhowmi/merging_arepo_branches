 ...
 * \details     Star formation rates are calculated in sfr_eEOS for the multiphase model
 *              and in SMUGGLE/sfr.c for stellar feedback ISM.
 *
 *
...
#if !defined(SMUGGLE_SFR) && !defined(SFR_MCS)
  init_clouds();
#else
  init_star_formation();
#endif
...
#if defined(METALS) && !defined(SFR_MCS)
  double w = 0;
#endif
      ...
      if(i >= 0)
        {
...
#ifdef METALS
              SphP[i].Metallicity =
                  SphP[i].MassMetallicity / P[i].Mass; /* update this here because primitive variables have not been updated */
#ifndef SFR_MCS
              w = get_random_number();
          ...
          if(p_decide < p / pall) /* ok, it is decided to consider star formation */
            {
#ifndef SF_STELLAR_MASS_TO_GAS_MASS_RATIO
              /* default mode */
                      ...
                      for(int istar = 0; istar < NStars; istar++)
                        {
                          ...
                          int idx_of_star = make_star(idx, i, 1., mass_of_star, &sum_mass_stars);
                          for(int k = 0; k < 3; k++)
                            P[idx_of_star].Vel[k] += gsl_ran_gaussian(random_generator, VelDisp);
                        }
                    }
                }
#endif
...
#ifdef GFM_RPROCESS_CHANNELS_NS_KICKS
  if(out[cnt - 1] > 0)
    mpi_printf("NSNS KICKS: Spawned %d new NSNS particles.\n", out[cnt - 1]);
#endif
...
void spawn_star_from_cell(int igas, double birthtime, int istar, MyDouble mass_of_star)
{
...
#ifdef MAXSCALARS
  /* Note, the changes in MATERIALS, HIGHRESGASMASS, etc., are treated as part of the Scalars */
  ...
  /* new star particle starts with no tracer children */
  P[istar].TracerHead      = -1;
...
#if defined(GFM_RPROCESS_CHANNELS_NS_KICKS) && defined(REFINEMENT_HIGH_RES_GAS)
  int StarIsHighRes = SphP[i].HighResMass >= HIGHRESMASSFAC * P[i].Mass;
#endif
  ...
  if(get_random_number() < prob)
    {
      ...
      if(StarIsHighRes)
#endif
        /* make NSNS pseudo star particles */
        for(int ich = 0; ich < GFM_RPROCESS_NSNS; ich++)
          {
            double dtime = get_time_difference_in_Gyr(All.Time, 1);

            /* calculate number of NSNS mergers this particle will do until z = 0 */
            MyFloat NSNS_number = get_NSNS_number_dtd_powerlaw(ich, 0, dtime);
            NSNS_number *= P[idx_of_star].Mass * All.UnitMass_in_g / SOLAR_MASS / All.HubbleParam;
            int NSNS_number_true = gsl_ran_poisson(random_generator, NSNS_number);

            /* for each of them, make a star particle */
            for(int iNSNS = 0; iNSNS < NSNS_number_true; iNSNS++)
              {
                altogether_spawned = stars_spawned;
#ifdef GFM_WINDS
                altogether_spawned += wind_spawned;
                ...
                P[j]      = P[idx_of_star];
                P[j].Mass = 0;
                timebin_add_particle(&TimeBinsGravity, j, i, P[j].TimeBinGrav, TimeBinSynchronized[P[j].TimeBinGrav]);
#if defined(TRACER_MC)
                /* new star particle starts with no tracer children */
                P[j].TracerHead      = -1;
                P[j].NumberOfTracers = 0;
#endif
                gfm_add_star(j, i, 0., All.Time, 1.5 * get_cell_radius(i));

                /* draw a delay time for this NSNS mergers */
                StarP[P[j].AuxDataID].NSNS_channel = ich;
                StarP[P[j].AuxDataID].DelayTime    = get_NSNS_delay_time_single_event(ich, dtime);

                /* draw a kick velocity for this NSNS pair */
                double kickvel = (All.rp[ich].NSNS_KickAverage + gsl_ran_gaussian(random_generator, All.rp[ich].NSNS_KickSigma)) *
                                 All.cf_atime / All.UnitVelocity_in_cm_per_s * 1e5;
                while(kickvel < 0)
                  {
                    /* cut off distribution at zero! */
                    kickvel = (All.rp[ich].NSNS_KickAverage + gsl_ran_gaussian(random_generator, All.rp[ich].NSNS_KickSigma)) *
                              All.cf_atime / All.UnitVelocity_in_cm_per_s * 1e5;
                  }
                ...
                P[j].Vel[0] += kickvel * sin(theta) * cos(phi);
                P[j].Vel[1] += kickvel * sin(theta) * sin(phi);
                P[j].Vel[2] += kickvel * cos(theta);
