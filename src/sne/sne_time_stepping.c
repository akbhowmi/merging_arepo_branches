 ...
 *
 * \file        src/sne/sne_time_stepping.c
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
...
int sne_timestep_evaluate(int target, int mode, int threadid)
{
  ...
  double dx, dy, dz, dist, csnd, cellrad;
...
static void kernel_local(int local_n, int indices[])
{
  int idx;
#ifdef GENERIC_ASYNC
  int flag = 0;
        ...
        if(threadid == 0)
          {
            if((count & POLLINGINTERVAL) == 0)
              if(generic_polling_primary(count, local_n))
                flag = 1;
...
#pragma omp atomic capture
        ...
        sne_timestep_evaluate(i, MODE_LOCAL_PARTICLES, threadid);
      }
  }
}
...
#pragma omp atomic capture
        ...
        sne_timestep_evaluate(i, MODE_IMPORTED_PARTICLES, threadid);
      }
  }
}

void sne_tree_based_timesteps_setsoundspeeds(int local_n, int indices[])
{
  int idx, i;
  double rad, csnd;
...
{
  /*find particles that have to be recontrolled*/
  int local_n, total_n, *indices;
  double radius = 2. * injectionradius;  // 500.*PARSEC/All.UnitLength_in_cm;
  find_particles_within_a_sphere(sne_pos, radius, &local_n, &total_n, &indices);

  /*find tree based timesteps*/

  sne_tree_based_timesteps_setsoundspeeds(local_n, indices);
  generic_set_MaxNexport();
  generic_comm_pattern_for_given_particles(local_n, indices, kernel_local, kernel_imported);

  /*update time-steps of the injection cells*/
  for(int i = 0; i < local_n; i++)
    {
      const int idx = indices[i];

      const integertime ti_step = get_timestep_hydro(idx);
      const int bin             = timebins_get_bin_and_do_validity_checks(ti_step, P[idx].TimeBinHydro);
      const int binoldHydro     = P[idx].TimeBinHydro;
      const int binoldGravity   = P[idx].TimeBinGrav;

      if(bin < binoldHydro)
        {
          timebin_move_particle(&TimeBinsHydro, idx, binoldHydro, bin);
          timebin_move_particle(&TimeBinsGravity, idx, binoldGravity, bin);

          P[idx].TimeBinHydro = bin;
          P[idx].TimeBinGrav  = bin;
        }
    }
