...
int ngb_treebuild(int npart)
{
  ...
  for(int i = 0; i < NTopnodes; i++)
    Ngb_DomainTask[i + Ngb_MaxPart] = -1;

  for(int i = 0; i < NTopleaves; i++)
    {
      int index             = Ngb_DomainNodeIndex[i];
      Ngb_DomainTask[index] = DomainTask[i];
    }

  Mesh.LastDP = Mesh.Ndp;

  Ngb_Nodes[Ngb_MaxPart].Center[0] = DomainCenter[0];
  Ngb_Nodes[Ngb_MaxPart].Center[1] = DomainCenter[1];
  Ngb_Nodes[Ngb_MaxPart].Center[2] = DomainCenter[2];

  amr_set_node_data(Ngb_MaxPart, -1, 0, 1, &Mesh);
 ...
 *  `Ngb_Nodes' is shifted such that Ngb_Nodes[Ngb_MaxPart] gives the first tree node.
 *
 */
int ngb_treebuild_construct(int npart)
{
  ...
  Ngb_DomainNodeIndex[0] = Ngb_NextFreeNode;

#ifdef AMR
  Ngb_Nodes[Ngb_NextFreeNode].level  = 0;
  Ngb_Nodes[Ngb_NextFreeNode].father = -1;
  for(int j = 0; j < 2 * NUMDIMS; j++)
    {
      Ngb_Nodes[Ngb_NextFreeNode].neighbors[j] = Ngb_NextFreeNode;
    }
#endif
...
#if(NUM_THREADS > 1)
  omp_lock_t *node_lock = (omp_lock_t *)mymalloc("node_lock", NTopleaves * sizeof(omp_lock_t));
...
#pragma omp barrier
#pragma omp flush(Ngb_NextFreeNode)
...
#ifdef AMR
                nfreep->level = Ngb_Nodes[parent].level + 1;
                for(int n = 0; n < 2 * NUMDIMS; n++)
                  {
                    nfreep->neighbors[n] = -1;
                  }
#endif
...
#ifdef AMR
              Ngb_Nodes[Ngb_NextFreeNode].level = Ngb_Nodes[no].level + 1;
              for(int n = 0; n < 2 * NUMDIMS; n++)
                {
                  Ngb_Nodes[Ngb_NextFreeNode].neighbors[n] = -1;
                }
#endif
                      ...
                      if(p >= Ngb_MaxPart)
                        {
                          if(p < Ngb_MaxPart + Ngb_MaxNodes)  // not a pseudo particle
                            {
                              amr_set_node_data(p, no, j, 1, &Mesh);
                                  ...
                                  for(d = 0; d < 2 * NUMDIMS; d++)
                                    {
                                      ...
                                      if(ngb >= Ngb_MaxPart &&
                                         ngb < Ngb_FirstNonTopLevelNode)  // a top level node, must be on an other task
                          ...
                          for(d = 0; d < 2 * NUMDIMS; d++)
                            {
                              ...
                              if(ngb >= Ngb_MaxPart && ngb < Ngb_FirstNonTopLevelNode &&
                                 Ngb_DomainTask[ngb] != ThisTask)  // a top level node on an other task
                                {
                                  if(Mesh.NexpList == Mesh.MaxNexpList)
                                    {
                                      Mesh.MaxNexpList *= 1.2 + 1;  // FIXME better estiamte
                                      Mesh.expList = (list_export_data *)myrealloc_movable(
                                          Mesh.expList, Mesh.MaxNexpList * sizeof(list_export_data));
                                    }
...
#ifdef AMR
  amr_treemodifylength(delta_NgbMaxPart);
#endif
...
#ifdef AMR
      if(Ngb_Nodes[i + Ngb_MaxPart].nextinlevel >= Ngb_MaxPart)
        Ngb_Nodes[i + Ngb_MaxPart].nextinlevel += delta_NgbMaxPart;

      if(Ngb_Nodes[i + Ngb_MaxPart].previnlevel >= Ngb_MaxPart)
        Ngb_Nodes[i + Ngb_MaxPart].previnlevel += delta_NgbMaxPart;

      for(int k = 0; k < 8; k++)
        {
          if(Ngb_Nodes[i + Ngb_MaxPart].u.suns[k] >= Ngb_MaxPart)
            Ngb_Nodes[i + Ngb_MaxPart].u.suns[k] += delta_NgbMaxPart;
        }
      for(int k = 0; k < 2 * NUMDIMS; k++)
        {
          if(Ngb_Nodes[i + Ngb_MaxPart].neighbors[k] >= Ngb_MaxPart)
            Ngb_Nodes[i + Ngb_MaxPart].neighbors[k] += delta_NgbMaxPart;
        }
#endif
    }
...
#ifdef AMR
  Ngb_DomainTask -= delta_NgbMaxPart;
  Ngb_NextFreeNode += delta_NgbMaxPart;
#endif
#ifdef TREE_BASED_TIMESTEPS
...
{
  mpi_printf("ALLOCATE: Need to adjust NgbTree because we need %d additional nodes\n", delta_Nodes);

  int i;
  for(i = 0; i < Ngb_MaxPart + NTopleaves; i++)       /* check for pseudo particles */
    if(Ngb_Nextnode[i] >= Ngb_MaxPart + Ngb_MaxNodes) /* pseudo particle */
      Ngb_Nextnode[i] += delta_Nodes;

  for(i = 0; i < Ngb_MaxNodes; i++)
    {
      if(Ngb_Nodes[i + Ngb_MaxPart].u.d.nextnode >= Ngb_MaxPart + Ngb_MaxNodes) /* internal node or pseudo particle */
        Ngb_Nodes[i + Ngb_MaxPart].u.d.nextnode += delta_Nodes;

      if(Ngb_Nodes[i + Ngb_MaxPart].u.d.sibling >= Ngb_MaxPart + Ngb_MaxNodes) /* internal node or pseudo particle */
        Ngb_Nodes[i + Ngb_MaxPart].u.d.sibling += delta_Nodes;
    }

  Ngb_MaxNodes += delta_Nodes;
  Ngb_Nodes = (struct NgbNODE *)myrealloc_movable(Ngb_Nodes + Ngb_MaxPart, (Ngb_MaxNodes + 1) * sizeof(struct NgbNODE));
  Ngb_Nodes -= Ngb_MaxPart;

#ifdef TREE_BASED_TIMESTEPS
  ExtNgb_Nodes = (struct ExtNgbNODE *)myrealloc_movable(ExtNgb_Nodes + Ngb_MaxPart, (Ngb_MaxNodes + 1) * sizeof(struct ExtNgbNODE));
  ExtNgb_Nodes -= Ngb_MaxPart;
#endif

  Ngb_Marker = (int *)myrealloc_movable(Ngb_Marker, (Ngb_MaxNodes + Ngb_MaxPart) * sizeof(int));
  if(delta_Nodes > 0)
    memset(Ngb_Marker + Ngb_MaxNodes + Ngb_MaxPart - delta_Nodes, -1, delta_Nodes * sizeof(int));

#ifdef AMR
  amr_treerealloc(delta_Nodes);
#endif
}
...
#ifdef AMR
  Ngb_DomainTask = (int *)mymalloc_movable(&Ngb_DomainTask, "Ngb_DomainTask", NTopnodes * sizeof(int));
  Ngb_DomainTask -= Ngb_MaxPart;
#endif
...
#if(NUM_THREADS > 1)
  Ngb_NodeLocks = (omp_lock_t *)mymalloc("Ngb_NodeLocks", Ngb_MaxNodes * sizeof(omp_lock_t));
  int i;
  for(i = 0; i < Ngb_MaxNodes; i++)
    omp_init_lock(&Ngb_NodeLocks[i]);
  Ngb_NodeLocks -= Ngb_MaxPart;
#endif

#ifdef AMR
  amr_allocate_mesh(&Mesh);
#endif
}
...
void ngb_treefree(void)
{
  ...
  if(Ngb_Nodes)
    {
#if(NUM_THREADS > 1)
      myfree(Ngb_NodeLocks + Ngb_MaxPart);
      Ngb_NodeLocks = NULL;
#endif
...
#ifdef AMR
      myfree(Ngb_DomainTask + Ngb_MaxPart);
#endif
