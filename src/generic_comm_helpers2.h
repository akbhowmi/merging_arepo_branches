 ...
 *
 * \file        src/generic_comm_helpers.h
 * \date        MM/YYYY
 * \author
 * \brief
 * \details
...
#ifdef GENERIC_ASYNC
#include "generic_comm_helpers_async.h"
#else
...
static data_in *DataIn;
static data_out *DataOut;
...
#define generic_set_MaxNexport(...)                 \
  {                                                 \
    generic_set_info(__func__, __FILE__, __LINE__); \
  }
...
#ifdef VERBOSE
  mpi_printf(
      "GENERIC: function %s(), file %s, line %d: MinSpace = %g MB  NTopleaves = %d  ExportSpace = %g MB sizeof(data_in)=%zu "
      "sizeof(data_out)=%zu\n",
      func, file, line, MinSpace / (1024.0 * 1024.0), NTopleaves, ExportSpace / (1024.0 * 1024.0), sizeof(data_in), sizeof(data_out));
#endif

  if(ExportSpace < MinSpace)
    terminate(
        "Bummer. Can't even safely process a single particle for the available memory. FreeBytes=%zu  ExportSpace=%zu  MinSpace=%zu  "
        "MyNTask=%d  NTopleaves=%d",
        FreeBytes(), ExportSpace, MinSpace, MyNTask, NTopleaves);
}
      ...
      for(int j = 0, jj = 0; j < Thread[i].Nexport; j++)
        {
          ...
          while(jj < Thread[i].NexportNodes && Thread[i].PartList[j].Index == nodelist[-1 - jj].Index)
            {
              int task_ = nodelist[-1 - jj].Task;
              int off_  = Send_offset_nodes[task_] + Send[task_].CountNodes++;

              NodeDataIn[off_] = nodelist[-1 - jj].Node;

              rel_node_index[task_]++;
              jj++;
            }
        }
    }
  ...
  for(int ngrpstart = 1; ngrpstart < (1 << PTask); ngrpstart += ncycles)
    {
      ...
      if(ncycles == 0)
        terminate(
            "Seems like we can't even do one cycle: ncycles=%d  ngrpstart=%d  Nimport=%d  NimportNodes=%d  FreeBytes=%zu  needed "
            "storage=%zu",
            ncycles, ngrpstart, Nimport, NimportNodes, FreeBytes(), generic_calc_import_storage(Nimport, NimportNodes));
...
#ifndef OMIT_GENERIC_COMM_PATTERN
...
#ifndef OMIT_GENERIC_COMM_PATTERN_FOR_GIVEN_PARTICLES
...
#ifdef USE_INLINED_IBARRIER
/* can replace
 * MPI_Alltoall(Send, sizeof(struct send_recv_counts), MPI_INT, Recv, sizeof(struct send_recv_counts), MPI_INT, MYCOMMUNICATOR);
 * with a space communication pattern that effectively involves a home-grown
 * non-blocking barrier to establish that we can stop listening
 */
static void generic_prepare_import_counts_inlined_ibarrier(void)
{
