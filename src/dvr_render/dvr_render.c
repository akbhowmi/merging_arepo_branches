  ...
  for(i = 0; i < NumGas; i++)
    {
      ...
      delta_z =
          fabs(dvrdata->normalVec[0] * off_vector[0] + dvrdata->normalVec[1] * off_vector[1] + dvrdata->normalVec[2] * off_vector[2]);
...
void dvr_load_transfer_function(char *fname, int field)
{
  ...
  for(entries = 0; entries < transRGBEntries[field]; entries++)
    {
      fscanf(fdtrans, "%lf %lf %lf %lf", &(transRGBTable[field][entries].Val), &(transRGBTable[field][entries].Red),
             &(transRGBTable[field][entries].Green), &(transRGBTable[field][entries].Blue));
...
static void transferFunctions(double *rgba_voxel_data, double len, int field, color_data *color)
{
  ...
  color->Alpha =
      1.0 - exp(-All.DvrTauScaleFactor *
                tau);  // All.DvrTauScaleFactor * tau + All.DvrTauFloor;      /* ~ 1.0 - exp(-All.DvrTauScaleFactor * tau) */
  ...
  if(rgba_voxel_data[0] > 0)
    {
      ...
      if(entries >= 1 && entries < transRGBEntries[field])
        {
          double fac = (value - transRGBTable[field][entries - 1].Val) /
                       (transRGBTable[field][entries].Val - transRGBTable[field][entries - 1].Val);
          color->Red = transRGBTable[field][entries - 1].Red +
                       fac * (transRGBTable[field][entries].Red - transRGBTable[field][entries - 1].Red);
          color->Green = transRGBTable[field][entries - 1].Green +
                         fac * (transRGBTable[field][entries].Green - transRGBTable[field][entries - 1].Green);
          color->Blue = transRGBTable[field][entries - 1].Blue +
                        fac * (transRGBTable[field][entries].Blue - transRGBTable[field][entries - 1].Blue);
        }
    }
}
  ...
  if(fdcam == NULL)
    {
      ...
      for(i = 0; i < All.DvrFrameFileNum; i++)
        if(fscanf(fdcam, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", &dummy, &dummy, &dummy, &dummy, &dummy, &dummy, &dummy,
                  &dummy, &dummy, &dummy, &dummy, &dummy, &dummy) == EOF)
      ...
      if(field >= DVR_RENDER_ELEMENT_0 && field <= DVR_RENDER_ELEMENT_8)
        {
          ...
          file_path_sprintf(buf, "dvr_render/transRGB_element_%d.txt", off);
...
static int dvr_do_next_camera_setup(FILE *fdcam, dvr_cam_data *dvrdata)
{
  ...
  if(fscanf(fdcam, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", &(dvrdata->camPos[0]), &(dvrdata->camPos[1]),
            &(dvrdata->camPos[2]), &(dvrdata->camTo[0]), &(dvrdata->camTo[1]), &(dvrdata->camTo[2]), &(dvrdata->width),
            &(dvrdata->height), &(dvrdata->near), &(dvrdata->far), &(dvrdata->upVec[0]), &(dvrdata->upVec[1]),
            &(dvrdata->upVec[2])) == EOF)
    return -1;
  ...
  /* normalize up vector */
  double nn =
      sqrt(dvrdata->upVec[0] * dvrdata->upVec[0] + dvrdata->upVec[1] * dvrdata->upVec[1] + dvrdata->upVec[2] * dvrdata->upVec[2]);
  for(k = 0; k < 3; k++)
    dvrdata->upVec[k] /= nn;
  ...
  for(i = 0; i < NumGas; i++)
    {
...
#ifdef GFM_COOLING_METAL
      mu =
          (4. / (1. + 3. * SphP[i].MetalsFraction[element_index_Hydrogen] + 4. * SphP[i].MetalsFraction[element_index_Hydrogen] * ne) *
           PROTONMASS);
      ...
      if(temp > DVR_XRAY_TEMP)
        SphP[i].DvrFields[DVR_RENDER_XRAY] =
            1.2e-24 * P[i].Mass / All.HubbleParam * All.UnitMass_in_g * dens / (mu * mu) * sqrt(temp / 1.1e7);
      else
...
static int dvr_init(dvr_cam_data *dvrdata)
{
    ...
    for(y = 0; y < dvrdata->ny; y++)
      {
        ...
        for(k = 0; k < 3; k++)
          {
            ...
            dvrdata->Ray[dvrdata->Nray].pos[k] =
                dvrdata->camPos[k] +                                                        /* camera */
        ...
        double cos_angle =
            (dvrdata->dir[0] * dvrdata->Ray[dvrdata->Nray].dir[0] + dvrdata->dir[1] * dvrdata->Ray[dvrdata->Nray].dir[1] +
             dvrdata->dir[2] * dvrdata->Ray[dvrdata->Nray].dir[2]);
  ...
  mpi_distribute_items_from_search(searchdata, dvrdata->Ray, &dvrdata->Nray, &dvrdata->MaxNray, sizeof(dvr_ray_data), TAG_DENS_A,
                                   offsetof(dvr_ray_data, task), offsetof(dvr_ray_data, index));
  myfree(searchdata);
  return 0;
}
...
#if DVR_RENDER == 0
  file_path_sprintf(ext, "%03d_%04d", All.SnapshotFileCount, All.DvrFrameFileNum);
#endif
#if DVR_RENDER == 1
  file_path_sprintf(ext, "%04d", All.DvrFrameFileNum);
#endif

  mkdir(All.DvrOutputDir, MKDIR_MODE);

  if(field == DVR_RENDER_TEMP)
    file_path_sprintf(fname, "%s/dvr_render_temp_%s_%s", All.DvrOutputDir, name, ext);
  if(field == DVR_RENDER_MET)
    file_path_sprintf(fname, "%s/dvr_render_met_%s_%s", All.DvrOutputDir, name, ext);
  if(field == DVR_RENDER_RHO)
    file_path_sprintf(fname, "%s/dvr_render_rho_%s_%s", All.DvrOutputDir, name, ext);
  if(field == DVR_RENDER_XRAY)
    file_path_sprintf(fname, "%s/dvr_render_xray_%s_%s", All.DvrOutputDir, name, ext);
  if(field >= DVR_RENDER_ELEMENT_0 && field <= DVR_RENDER_ELEMENT_8)
    file_path_sprintf(fname, "%s/dvr_render_element_%d_%s_%s", All.DvrOutputDir, field - DVR_RENDER_ELEMENT_0, name, ext);
#ifdef GFM_DUST
  if(field == DVR_RENDER_DUST_RHO)
    file_path_sprintf(fname, "%s/dvr_render_dust_rho_%s_%s", All.DvrOutputDir, name, ext);
  if(field == DVR_RENDER_DUST_MET)
    file_path_sprintf(fname, "%s/dvr_render_dust_met_%s_%s", All.DvrOutputDir, name, ext);
#endif
...
static void dvr_exchange_rays(dvr_cam_data *dvrdata)
{
  ...
  mpi_distribute_items_to_tasks(dvrdata->Ray, offsetof(dvr_ray_data, task), &dvrdata->Nray, &dvrdata->MaxNray, sizeof(*dvrdata->Ray),
                                TAG_DENS_A);
}
  ...
  for(i = 0; i < dvrdata->Nray; i++)
    {
      ...
      if((dvrdata->Ray[i].pos[0] < 0.0) || (dvrdata->Ray[i].pos[0] > All.BoxSize) || (dvrdata->Ray[i].pos[1] < 0.0) ||
         (dvrdata->Ray[i].pos[1] > All.BoxSize) || (dvrdata->Ray[i].pos[2] < 0.0) || (dvrdata->Ray[i].pos[2] > All.BoxSize))
