...
void blackhole_do_bubbles(int num_activebh)
{
  ...
  if(total_num_activebh > 0)
    {
      radio_bubble = (struct localbubble_data *)mymalloc("radio_bubble", num_activebh * sizeof(struct localbubble_data));
      num_bubbles  = 0;
          ...
          if(BPP(n).BH_Mass_bubbles > 0 && BPP(n).BH_Mass_bubbles > All.BlackHoleRadioTriggeringFactor * BPP(n).BH_Mass_ini)
            {
              ...
              BPP(n).BH_CumEgy_RM += All.RadioFeedbackFactor * All.BlackHoleRadiativeEfficiency * radio_bubble[num_bubbles].bh_dmass *
                                     pow(CLIGHT / All.UnitVelocity_in_cm_per_s, 2);
              BPP(n).BH_MPB_CumEgyLow += BPP(n).BH_CumEgy_RM;
  ...
  for(i = 0; i < num_bubbles; i++)
    {
      ...
      radio_bubble[i].distance *=
          pow((radio_bubble[i].bubbleEnergy * All.DefaultICMDensity / (All.BubbleEnergy * ICMDensity)), 1. / 5.);
  ...
  for(i = 0; i < num_bubbles; i++)
    {
      ...
      radio_bubble[i].bubbleEnergy = All.RadioFeedbackFactor * All.BlackHoleRadiativeEfficiency * radio_bubble[i].bh_dmass *
                                     All.UnitMass_in_g / All.HubbleParam * pow(CLIGHT, 2); /*in cgs units */
  ...
  for(i = 0; i < num_bubbles; i++)
    {
      ...
      radio_bubble[i].mstrength = set_dipole_strength(radio_bubble[i].Volume_bubble / All.cf_a3inv, soft,
                                                      All.MagneticEnergyFraction * dE); /* in physical units */
