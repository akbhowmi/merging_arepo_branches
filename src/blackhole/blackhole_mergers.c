      ...
      if(i < NumPart)
        {
          ...
          BH_accreted[P[i].AuxDataID].BH_Mdot_radio  = fmax(BH_accreted[P[i].AuxDataID].BH_Mdot_radio, out->Prop.BH_Mdot_radio);
          BH_accreted[P[i].AuxDataID].BH_Mdot_quasar = fmax(BH_accreted[P[i].AuxDataID].BH_Mdot_quasar, out->Prop.BH_Mdot_quasar);
          BH_accreted[P[i].AuxDataID].BH_HaloVvir    = fmax(BH_accreted[P[i].AuxDataID].BH_HaloVvir, out->Prop.BH_HaloVvir);
          BH_accreted[P[i].AuxDataID].BH_XrayLum     = fmax(BH_accreted[P[i].AuxDataID].BH_XrayLum, out->Prop.BH_XrayLum);
          BH_accreted[P[i].AuxDataID].BH_RadioLum    = fmax(BH_accreted[P[i].AuxDataID].BH_RadioLum, out->Prop.BH_RadioLum);
#endif
      ...
      else
        {
          ...
          Tree_BhngbResultsImported[idx].Prop.BH_HaloVvir =
              fmax(Tree_BhngbResultsImported[idx].Prop.BH_HaloVvir, out->Prop.BH_HaloVvir);
          Tree_BhngbResultsImported[idx].Prop.BH_Mdot_radio =
              fmax(Tree_BhngbResultsImported[idx].Prop.BH_Mdot_radio, out->Prop.BH_Mdot_radio);
          Tree_BhngbResultsImported[idx].Prop.BH_Mdot_quasar =
              fmax(Tree_BhngbResultsImported[idx].Prop.BH_Mdot_quasar, out->Prop.BH_Mdot_quasar);
          Tree_BhngbResultsImported[idx].Prop.BH_XrayLum = fmax(Tree_BhngbResultsImported[idx].Prop.BH_XrayLum, out->Prop.BH_XrayLum);
          Tree_BhngbResultsImported[idx].Prop.BH_RadioLum =
              fmax(Tree_BhngbResultsImported[idx].Prop.BH_RadioLum, out->Prop.BH_RadioLum);
#endif
 ...
 */
void apply_recoil_kick(MyDouble Mass1, MyDouble Mass2, MyFloat *Pos1, MyFloat *Pos2, MyFloat *Vel1, MyFloat *Vel2, MyFloat *Spin1,
                       MyFloat *Spin2, MyFloat *RecoilVel, MyFloat *FinalSpin)
{
  int k;
  double Q, Eta;
  ...
  Lspin = s4 / pow(1 + Q * Q, 2) * (a2 * a2 + a1 * a1 * Q * Q * Q * Q + 2 * a1 * a2 * Q * Q * cosa1a2) +
          ((s5 * Eta + t0 + 2) / (1 + Q * Q)) * (a2 * cosa2l + a1 * Q * Q * cosa1l) + 2 * sqrt(3) + t2 * Eta + t3 * Eta * Eta;
  ...
  eccentricity = sqrt(1 + 2 * (0.5 * Vmag * Vmag - All.G * (Mass1 + Mass2) / Rmag) * Lmag * Lmag /
                              (All.G * All.G * (Mass1 + Mass2) * (Mass1 + Mass2)));
  for(k = 0; k < 3; k++)
...
void blackhole_do_mergers(void)
{
  ...
  treeBHexportflag = (char *)mymalloc_clear("treeBHexportflag", Tree_NumBHImported * sizeof(char));
...
#ifdef TRACER_MC
  treeBHexportRindex = (int *)mymalloc_clear("treeBHexportRindex", Tree_NumBHImported * sizeof(int));
  treeBHexportRtask  = (int *)mymalloc_clear("treeBHexportRtask", Tree_NumBHImported * sizeof(int));
  treeBHexportRID    = (MyIDType *)mymalloc_clear("treeBHexportRID", Tree_NumBHImported * sizeof(MyIDType));
#endif
  ...
  BH_accreted = (struct bh_properties *)mymalloc_clear("BH_accreted", NumBHs * sizeof(struct bh_properties));
  ...
  Tree_BhngbResultsImported =
      (struct bhresultsimported_data *)mymalloc_clear("Tree_BhngbResultsImported", ncount * sizeof(struct bhresultsimported_data));
  ...
  struct bhresultsimported_data *tmp_results =
      (struct bhresultsimported_data *)mymalloc("tmp_results", nexport * sizeof(struct bhresultsimported_data));
  ...
  for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&Tree_BhngbResultsImported[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct bhresultsimported_data),
                       MPI_BYTE, recvTask, TAG_FOF_A, &tmp_results[Send_offset[recvTask]],
                       Send_count[recvTask] * sizeof(struct bhresultsimported_data), MPI_BYTE, recvTask, TAG_FOF_A, MPI_COMM_WORLD,
                       MPI_STATUS_IGNORE);
    }
  ...
  Tree_BhngbDeleted = (struct bhdeleted_data *)mymalloc("Tree_BhngbDeleted", sizeof(struct bhdeleted_data) * N_BH_swallowed_imported);
        ...
        if(Tree_Points[n].Type == 5 && treeBHexportflag[Tree_Points[n].AuxDataIndex] == 1)
          {
            if(k >= N_BH_swallowed_imported)
              terminate("BLACK_HOLES: k >= N_BH_swallowed k=%d N_BH_swallowed_imported=%d n=%d i=%d Tree_NumPartImported=%d", k,
                        N_BH_swallowed_imported, n, i, Tree_NumPartImported);
  ...
  struct bhdeleted_data *tmp_deleted = (struct bhdeleted_data *)mymalloc("tmp_deleted", nexport * sizeof(struct bhdeleted_data));
  ...
  for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&Tree_BhngbDeleted[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct bhdeleted_data), MPI_BYTE,
                       recvTask, TAG_FOF_A, &tmp_deleted[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct bhdeleted_data),
      ...
      if(BH_accreted[P[n].AuxDataID].Mass > 0)
        {
          ...
          for(k = 0; k < 3; k++)
            P[n].Pos[k] = (P[n].Mass * P[n].Pos[k] + BH_accreted[P[n].AuxDataID].Mass * BH_accreted[P[n].AuxDataID].Pos[k]) /
          ...
          for(k = 0; k < 3; k++)
            P[n].Vel[k] =
                (P[n].Vel[k] * P[n].Mass + BH_accreted[P[n].AuxDataID].Momentum[k]) / (P[n].Mass + BH_accreted[P[n].AuxDataID].Mass);
          ...
          if(BH_accreted[P[n].AuxDataID].BHMass > BPP(n).BH_Mass)
            {
              BPP(n).BH_MPB_CumEgyHigh = BH_accreted[P[n].AuxDataID].BH_MPB_CumEgyHigh;
              BPP(n).BH_MPB_CumEgyLow  = BH_accreted[P[n].AuxDataID].BH_MPB_CumEgyLow;
            }
          ...
          BPP(n).BH_HaloVvir    = fmax(BPP(n).BH_HaloVvir, BH_accreted[P[n].AuxDataID].BH_HaloVvir);
          BPP(n).BH_Mdot_radio  = fmax(BPP(n).BH_Mdot_radio, BH_accreted[P[n].AuxDataID].BH_Mdot_radio);
          BPP(n).BH_Mdot_quasar = fmax(BPP(n).BH_Mdot_quasar, BH_accreted[P[n].AuxDataID].BH_Mdot_quasar);
          BPP(n).BH_XrayLum     = fmax(BPP(n).BH_XrayLum, BH_accreted[P[n].AuxDataID].BH_XrayLum);
          BPP(n).BH_RadioLum    = fmax(BPP(n).BH_RadioLum, BH_accreted[P[n].AuxDataID].BH_RadioLum);
#endif
        }
