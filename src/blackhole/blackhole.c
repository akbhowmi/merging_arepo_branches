  ...
  if(ThisTask == 0)
    {
      ...
      total_mdoteddington *=
          1.0 / ((4 * M_PI * GRAVITY * CLIGHT * PROTONMASS / (All.BlackHoleRadiativeEfficiency * CLIGHT * CLIGHT * THOMPSON)) *
                 All.UnitTime_in_s / All.HubbleParam);

      fprintf(FdBlackHoles, "%14e %6d  %14e %14e %14e %14e %14e\n", All.Time, All.TotNumBHs, total_mass_holes, total_mdot,
              mdot_in_msun_per_year, total_mass_real, total_mdoteddington);
...
void blackhole_energy_log_info(void)
{
  ...
  /* NOTE: the thermal energy per step is not expected to be balanced, because only active cells actually receive it in the same time
   * step. The cumulative should be exact. */
  mpi_printf(
      "BLACK_HOLES: AGN feedback channels (step)        Is      --> thermal = %g erg    mechanical = %g erg    electro-magnetic = "
      "(%g/%g) erg\n",
  ...
  mpi_printf("BLACK_HOLES: AGN feedback channels (step)        Ratio   --> thermal = %g        mechanical = %g \n",
             (totAGNEnergyT_Should > 0) ? (totAGNEnergyT_Is / totAGNEnergyT_Should) : (0),
             (totAGNEnergyM_Should > 0) ? (totAGNEnergyM_Is / totAGNEnergyM_Should) : (0));

  mpi_printf(
      "BLACK_HOLES: AGN feedback channels (cumulative)  Is      --> thermal = %g erg    mechanical = %g erg    electro-magnetic = "
      "(%g/%g) erg\n",
      All.CumAGNEnergyT_Is * All.UnitEnergy_in_cgs, All.CumAGNEnergyM_Is * All.UnitEnergy_in_cgs, All.CumAGNEnergyEMobs_Is,
      All.CumAGNEnergyEM_Is);
  ...
  mpi_printf("BLACK_HOLES: AGN feedback channels (cumulative)  Ratio   --> thermal = %g       mechanical = %g \n",
             (All.CumAGNEnergyT_Should > 0) ? (All.CumAGNEnergyT_Is / All.CumAGNEnergyT_Should) : (0),
             (All.CumAGNEnergyM_Should > 0) ? (All.CumAGNEnergyM_Is / All.CumAGNEnergyM_Should) : (0));
