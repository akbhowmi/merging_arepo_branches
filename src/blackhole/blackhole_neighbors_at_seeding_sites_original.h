...
void blackhole_neighbors_at_seeding_sites(void)
{
          ...
          if(blackhole_isactive(i))
            {
              if(BPP(i).BH_Density > 0)
                {
                  ...
                  printf("BLACK_HOLES: AM Direction = (%g,%g,%g)\n", BPP(i).BH_BipolarJ[0], BPP(i).BH_BipolarJ[1],
                         BPP(i).BH_BipolarJ[2]);
#endif
              ...
              if(BPP(i).BH_NumNgb < (All.DesNumNgbBlackHole - All.MaxNumNgbDeviationBlackHole) ||
                 (BPP(i).BH_NumNgb > (All.DesNumNgbBlackHole + All.MaxNumNgbDeviationBlackHole)))
                {
                  ...
                  if(iter >= MAXITER - 10)
                    {
                      printf("i=%d task=%d ID=%lld Hsml=%g Left=%g Right=%g Ngbs=%g Right-Left=%g\n   pos=(%g|%g|%g)\n", i, ThisTask,
                             (long long)P[i].ID, BPP(i).BH_Hsml, Left[i], Right[i], (double)BPP(i).BH_NumNgb, Right[i] - Left[i],
      ...
      if(blackhole_isactive(i))
        {
          if(BPP(i).NearestDist == MAX_REAL_NUMBER || BPP(i).DrainID == 0)
            terminate(
                "BLACK_HOLES DRAINGAS: no primary cell found: ID=%lld Left=%g Right=%g NearestDist=%g DrainID=%lld rho=%g hsml=%g "
                "NumNgb=%g Mdot=%g",
                (long long)P[i].ID, Left[i], Right[i], BPP(i).NearestDist, (long long)BPP(i).DrainID, BPP(i).BH_Density,
                BPP(i).BH_Hsml, BPP(i).BH_NumNgb, BPP(i).BH_Mdot);
        }
    }
#endif
     ...
     if(P[j].Mass > 0 && P[j].ID != 0)
        {
...
#ifdef BH_USE_ALFVEN_SPEED_IN_BONDI
          Bpress +=
              0.5 * (SphP[j].B[0] * SphP[j].B[0] + SphP[j].B[1] * SphP[j].B[1] + SphP[j].B[2] * SphP[j].B[2]) * SphP[j].Volume * wk;
#endif
          ...
          bipolar_j[0] += mass_j * mass_j *
                          (dr[1] * (SphP[j].Momentum[2] / mass_j - bh_vel[2]) - dr[2] * (SphP[j].Momentum[1] / mass_j - bh_vel[1]));
          bipolar_j[1] += mass_j * mass_j *
                          (dr[2] * (SphP[j].Momentum[0] / mass_j - bh_vel[0]) - dr[0] * (SphP[j].Momentum[2] / mass_j - bh_vel[2]));
          bipolar_j[2] += mass_j * mass_j *
                          (dr[0] * (SphP[j].Momentum[1] / mass_j - bh_vel[1]) - dr[1] * (SphP[j].Momentum[0] / mass_j - bh_vel[0]));
...
#else
          csnd += sqrt((P[j].Vel[0] - SphP[j].VelVertex[0]) * (P[j].Vel[0] - SphP[j].VelVertex[0]) +
                       (P[j].Vel[1] - SphP[j].VelVertex[1]) * (P[j].Vel[1] - SphP[j].VelVertex[1]) +
                       (P[j].Vel[2] - SphP[j].VelVertex[2]) * (P[j].Vel[2] - SphP[j].VelVertex[2]));
#endif
       ...
       if(P[j].Mass > 0 && P[j].ID != 0)
        {
          ...
          {
             // mpi_printf("!!!Assigning to the particle type %d, seed mass value of %g, at time %g \n",P[j].Type,All.SeedBlackHoleMass,in->Time_Of_Seeding);
