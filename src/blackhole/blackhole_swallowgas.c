      ...
      if(BH_accreted_Mass[P[n].AuxDataID] > 0)
        {
          ...
          int k;
          for(k = 0; k < 3; k++)
            P[n].Vel[k] = (P[n].Vel[k] * P[n].Mass + BH_accreted_momentum[3 * P[n].AuxDataID + k]) /
          ...
          if(r2 < h2 && P[j].Mass > 0 && P[j].ID != 0 && (bh_mass >= mass))
            {
              ...
              if(volsum <= 0.0)
                terminate(
                    "The BH seems not active, but it tries to accrete mass nevertheless. volsum=%g, BH ID=%llu, BH hsml=%g, BH "
                    "drainID=%llu, celld=%g",
              ...
              if(fac <= 0.01)
                {
                  warn("Trying to accrete more than the mass available from a cell: j=%d, fac=%g, P[j].Mass=%g, dmass=%g", j, fac,
                       P[j].Mass, dmass);
              ...
              if(dmass > 0.9 * P[j].Mass)
                {
                  ...
                  warn(
                      "BLACK_HOLES DRAINGAS (All.Time=%g): filling bucket for BH: id=%llu mass=%g/%g drainBucketMass=%g; from cell: "
                      "P[j].ID=%llu j=%d P[j].Mass=%g; dmass=%g diff_drainBucketMass=%g\n",
                      All.Time, (unsigned long long)id, mass, bh_mass, drainBucketMass, (unsigned long long)P[j].ID, j, P[j].Mass,
                      dmass, diff_drainBucketMass);
                  dmass = 0.9 * P[j].Mass;
                }
                  ...
                  if(drainBucketMass > 0)
                    {
                      if(0.9 * P[j].Mass > dmass + drainBucketMass)
                        {
                          warn(
                              "BLACK_HOLES DRAINGAS (All.Time=%g): fully emptying bucket of BH: id=%llu mass=%g/%g "
                              "drainBucketMass=%g; and cell: P[j].ID=%llu j=%d P[j].Mass=%g; dmass=%g\n",
                              All.Time, (unsigned long long)id, mass, bh_mass, drainBucketMass, (unsigned long long)P[j].ID, j,
                              P[j].Mass, dmass);
                      ...
                      else
                        {
                          ...
                          warn(
                              "BLACK_HOLES DRAINGAS (All.Time=%g): partially emptying bucket of BH: id=%llu mass=%g/%g "
                              "drainBucketMass=%g; with cell: P[j].ID=%llu j=%d P[j].Mass=%g; dmass=%g diff_drainBucketMass=%g\n",
                              All.Time, (unsigned long long)id, mass, bh_mass, drainBucketMass, (unsigned long long)P[j].ID, j,
                              P[j].Mass, dmass, diff_drainBucketMass);
                          dmass = 0.9 * P[j].Mass;
                        }
                    }
                }
