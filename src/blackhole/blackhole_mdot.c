          ...
          if(dt > bonditime)
            {
              ...
              accreted_mass +=
                  ((BPP(n).BH_Mass + (1. - All.BlackHoleRadiativeEfficiency) * accreted_mass) * (exp(eta * (dt - bonditime)) - 1.0));
            }
          else
      ...
      if(fraction_thermal > 0.0)
        {
          ...
          double egy = fraction_thermal * All.BlackHoleFeedbackFactor * All.BlackHoleRadiativeEfficiency * accreted_mass *
                       pow(CLIGHT / All.UnitVelocity_in_cm_per_s, 2);
#ifdef BH_THERMALFEEDBACK
          BPP(n).BH_ThermEnergy += egy;
          BPP(n).BH_CumEgy_QM += egy;
          BPP(n).BH_MPB_CumEgyHigh += egy;
#endif
      ...
      if(fraction_kinetic > 0.0 || fraction_quasar_wind > 0.0)
        {
          ...
          if(agn_mode == BH_RADIO_MODE)
            if((BPP(n).BH_Density * All.cf_a3inv) < All.RadioFeedbackMinDensityFactor * All.PhysDensThresh)
              {
                rad_efficiency *=
                    pow((BPP(n).BH_Density * All.cf_a3inv) / (All.RadioFeedbackMinDensityFactor * All.PhysDensThresh), 1);
              }
#endif
...
double blackhole_bondi_rate(int n, int type)
{
  ...
  bhvel = sqrt(pow(P[n].Vel[0] - BPP(n).BH_SurroundingGasVel[0], 2) + pow(P[n].Vel[1] - BPP(n).BH_SurroundingGasVel[1], 2) +
               pow(P[n].Vel[2] - BPP(n).BH_SurroundingGasVel[2], 2));
#else
  bhvel = 0;
#endif
  ...
  if(type == 0)
    {
      if(rho > 0)
        mdot = 4. * M_PI * All.BlackHoleAccretionFactor * All.G * All.G * BPP(n).BH_Mass * BPP(n).BH_Mass * rho /
               pow((pow(soundspeed, 2) + pow(bhvel, 2)), 1.5);
      flag++;
    }
#endif
  ...
  if(type == 1)
    {
      ...
      if(rho > 0)
        mdot = 4. * M_PI * alpha_accretion * All.G * All.G * BPP(n).BH_Mass * BPP(n).BH_Mass * rho /
               pow((pow(soundspeed, 2) + pow(bhvel, 2)), 1.5);
  ...
  if(type == 2)
    {
      ...
      if(rho > 0 && sigma > 0)
        mdot = 4. * M_PI * All.BlackHoleAccretionFactor * All.G * All.G * (BPP(n).BH_CaptureMass + BPP(n).BH_Mass) *
               (BPP(n).BH_CaptureMass + BPP(n).BH_Mass) * rho / pow((pow(soundspeed, 2) + pow(sigma, 2)), 1.5);
      flag++;
    }
#endif
  ...
  if(type == 3)
    {
      ...
      r_bondi = 50.0 * (PARSEC / All.UnitLength_in_cm) *
                ((BPP(n).BH_Mass * All.UnitMass_in_g / All.HubbleParam) / (1.e7 * SOLAR_MASS)) *
  ...
  if(BPP(n).BH_Mass > 0)
    {
      ...
      fprintf(FdBlackHolesBipolar, "BH=%llu %g %g %g %g %g %d %d %g\n", (long long)P[n].ID, All.Time, bj[0], bj[1], bj[2],
              BPP(n).BH_BipolarSum, (BPP(n).BH_BipolarColdFraction > All.BHBipolarColdFraction), BPP(n).BH_BipolarColdDisk,
              BPP(n).BH_BipolarColdFraction);
    }
#endif
...
{
  return (4 * M_PI * GRAVITY * CLIGHT * PROTONMASS / (All.BlackHoleRadiativeEfficiency * CLIGHT * CLIGHT * THOMPSON)) * bh_mass *
...
double blackhole_continous_mode_switch(double mdot, double mdot_adios, double meddington)
{
  ...
  if(mdot_max > meddington)
    mdot_max = meddington;
  f_k = pow(All.RadioFeedbackFactor * mdot_adios, 4.0) /
        (pow(All.BlackHoleFeedbackFactor * mdot, 4.0) + pow(All.RadioFeedbackFactor * mdot_adios, 4.0));
  ...
  for(idx = 0; idx < TimeBinsBHAccretion.NActiveParticles; idx++)
    {
      ...
      BPP(n).BH_RadioEgyFeedback +=
          All.BlackHoleRadiativeEfficiency * BPP(n).BH_Mdot_radio * dt * pow(CLIGHT / All.UnitVelocity_in_cm_per_s, 2);
