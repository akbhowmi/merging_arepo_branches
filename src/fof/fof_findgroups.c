...
double fof_find_groups(MyIDType *vMinID, int *vHead, int *vLen, int *vNext, int *vTail, int *vMinIDTask)
{
  ...
  Tree_Head = (int *)mymalloc("Tree_Head", Tree_NumNodes * sizeof(int));
...
/*! \brief This function finds the neighbors among the primary link types which
 *         are within a certain distance.
 *
 * \param[in] searchcenter
 * \param[in] hsml
 * \param[in] target
 * \param[in] numnodes
 * \param[in] firstnode
                        ...
                        if(flag_node_inside_linkinglength[nocur] & (1 << BITFLAG_INSIDE_LINKINGLENGTH)) /* already flagged */
                          {
                            ...
                            int p = fof_return_a_particle_in_cell_recursive(nocur);
