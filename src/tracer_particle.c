 ...
 *
 * Part of the generic communication pattern.
 *
 *  \param[out] in Data structure that is filled in this routine.
 *  \param[in] i Index of cell in P and SphP array.
 *  \param[in] firstnode Index of first node in communication.
 *
 *  \return void
 */
...
typedef struct
{
  ...
  MyFloat composition[EOS_NSPECIES]
...
/*! \brief Routine to store or combine result data.
 *
 *  Unused in this case, but part of the generic communication pattern.
 *
 *  \param[in] out Data that is to be set updated in global data structures on
 *             this task
 *  \param[in] i index in P and SphP array
 *  \param[in] mode Specifies whether out carries local or imported data
 *
 *  \return void
 */
...
/*! \brief Routine that specifies what to be done with local data
 *
 *  Part of the generic communication pattern
 *
 *  \return void
...
/*! \brief Routine that specifies what to be done with imported data
 *
 *  Part of the generic communication pattern
 *
 *  \return void
...
/*! \brief Find the host cell of the tracers
 *
 *  \return void
 */
              ...
              if(tracer_nearest_dist[i] == MAX_REAL_NUMBER)
                {
                  ...
                  if(iter >= MAXITER - 10)
                    {
                      printf("i=%d task=%d hsml=%g nearest dist=%g pos=(%g|%g|%g)\n", i, ThisTask, P[i].TracerHsml,
                             tracer_nearest_dist[i], P[i].Pos[0], P[i].Pos[1], P[i].Pos[2]);
                      myflush(stdout);
                    }
                  if(iter > MAXITER)
              ...
              else
                {
                  if(tracer_nearest_dist[i] > 0)
                    P[i].TracerHsml = 1.5 * fmax(tracer_nearest_dist[i], MinTracerHsml);
                  else
...
/*! \brief Inner function for find primary cell
 *
 *  This is called both on the local as well as on the imported particles
 *
 *  \param[in] target Index in particle data (local or imported)
 *  \param[in] mode Flag whether to analyze local or imported data
          ...
          if(SphP[j].Sfr == 0)
            {
              ...
              fluid_quantities[TracerPartTmaxIndex] = (SphP[j].GammaE - 1.0) / BOLTZMANN * SphP[j].Utherm * PROTONMASS * mu;
#else
...
#if(TRACER_PART_ENTMAX)
#ifdef VARIABLE_GAMMA
          fluid_quantities[TracerPartEntmaxIndex] =
              (SphP[j].Pressure * All.cf_a3inv) / pow(SphP[j].Density * All.cf_a3inv, SphP[j].GammaE);
#else
...
/*! \brief Put tracers into time integration
 *
 *  \return void
 */
...
/*! \brief Main function to assign tracer properties and timestep.
 *
 *  Called in main run loop.
 *
 *  \return void
 */
...
void set_tracer_part_indices(void)
{
  ...
  mpi_printf(
      "\nTRACER_PARTICLE: tracked quantity: \nTRACER_PARTICLE: TRACER_PART_TMAX=%d TRACER_PART_TMAX_TIME=%d TRACER_PART_TMAX_RHO=%d "
      "TRACER_PART_RHOMAX=%d TRACER_PART_RHOMAX_TIME=%d TRACER_PART_MACHMAX=%d TRACER_PART_ENTMAX=%d TRACER_PART_ENTMAX_TIME=%d\n",
      TRACER_PART_TMAX, TRACER_PART_TMAX_TIME, TRACER_PART_TMAX_RHO, TRACER_PART_RHOMAX, TRACER_PART_RHOMAX_TIME, TRACER_PART_MACHMAX,
      TRACER_PART_ENTMAX, TRACER_PART_ENTMAX_TIME);
  mpi_printf(
      "TRACER_PARTICLE: tracked quantity indices: \nTRACER_PARTICLE: TracerPartTmaxIndex=%d TracerPartTmaxTimeIndex=%d "
      "TracerPartTmaxRhoIndex=%d TracerPartRhomaxIndex=%d TracerPartRhomaxTimeIndex=%d TracerPartMachmaxIndex=%d "
      "TracerPartEntmaxIndex=%d TracerPartEntmaxTimeIndex=%d\n\n",
      TracerPartTmaxIndex, TracerPartTmaxTimeIndex, TracerPartTmaxRhoIndex, TracerPartRhomaxIndex, TracerPartRhomaxTimeIndex,
      TracerPartMachmaxIndex, TracerPartEntmaxIndex, TracerPartEntmaxTimeIndex);
#endif

#ifdef TRACER_MC_NUM_FLUID_QUANTITIES
  j = 0;

  TracerMCTmaxIndex = TracerMCTmaxTimeIndex = TracerMCTmaxRhoIndex = TracerMCRhomaxIndex = TracerMCRhomaxTimeIndex =
      TracerMCMachmaxIndex = TracerMCEntmaxIndex = TracerMCEntmaxTimeIndex = TracerMCLastStarTimeIndex = TracerMCWindCounterIndex =
          TracerMCExchangeCounterIndex = TracerMCExchangeDistanceIndex = TracerMCExchangeDistanceErrorIndex =
              TracerMCShockMachMaxIndex                                = -1;
  ...
  mpi_printf(
      "\nTRACER_MC: tracked quantity: \nTRACER_MC: TRACER_MC_TMAX=%d TRACER_MC_TMAX_TIME=%d TRACER_MC_TMAX_RHO=%d TRACER_MC_RHOMAX=%d "
      "TRACER_MC_RHOMAX_TIME=%d TRACER_MC_MACHMAX=%d TRACER_MC_ENTMAX=%d TRACER_MC_ENTMAX_TIME=%d TRACER_MC_LAST_STAR_TIME=%d "
      "TRACER_MC_WIND_COUNTER=%d TRACER_MC_EXCHANGE_COUNTER=%d TRACER_MC_EXCHANGE_DISTANCE=%d TRACER_MC_EXCHANGE_DISTANCE_ERROR=%d "
      "TRACER_MC_SHOCKMACHNUM_MAX=%d\n",
      TRACER_MC_TMAX, TRACER_MC_TMAX_TIME, TRACER_MC_TMAX_RHO, TRACER_MC_RHOMAX, TRACER_MC_RHOMAX_TIME, TRACER_MC_MACHMAX,
      TRACER_MC_ENTMAX, TRACER_MC_ENTMAX_TIME, TRACER_MC_LAST_STAR_TIME, TRACER_MC_WIND_COUNTER, TRACER_MC_EXCHANGE_COUNTER,
      TRACER_MC_EXCHANGE_DISTANCE, TRACER_MC_EXCHANGE_DISTANCE_ERROR, TRACER_MC_SHOCKMACHNUM_MAX);
  mpi_printf(
      "TRACER_MC: tracked quantity indices: \nTRACER_MC: TracerMCTmaxIndex=%d TracerMCTmaxTimeIndex=%d TracerMCTmaxRhoIndex=%d "
      "TracerMCRhomaxIndex=%d TracerMCRhomaxTimeIndex=%d TracerMCMachmaxIndex=%d TracerMCEntmaxIndex=%d TracerMCEntmaxTimeIndex=%d "
      "TracerMCLastStarTimeIndex=%d TracerMCWindCounterIndex=%d TracerMCExchangeCounterIndex=%d TracerMCExchangeDistanceIndex=%d "
      "TracerMCExchangeDistanceErrorIndex=%d TracerMCShockMachMaxIndex=%d\n\n",
      TracerMCTmaxIndex, TracerMCTmaxTimeIndex, TracerMCTmaxRhoIndex, TracerMCRhomaxIndex, TracerMCRhomaxTimeIndex,
      TracerMCMachmaxIndex, TracerMCEntmaxIndex, TracerMCEntmaxTimeIndex, TracerMCLastStarTimeIndex, TracerMCWindCounterIndex,
      TracerMCExchangeCounterIndex, TracerMCExchangeDistanceIndex, TracerMCExchangeDistanceErrorIndex, TracerMCShockMachMaxIndex);
#endif
}
