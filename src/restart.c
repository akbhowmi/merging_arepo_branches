 ...
 * \author
 * \brief
 * \details     Handling the loading/writing of restart files
 *
 *
...
/* need to define _GNU_SOURCE so that O_DIRECT will be defined in the headers */
#define _GNU_SOURCE
#endif
...
#define MODUS_CHECK RESTART_MODUS_CHECK
...
static void allocate_iobuf(void);
static void deallocate_iobuf(int modus);
...
#ifdef CHIMES
static double *ChimesAbundancesBuf;
#endif
...
void reread_params_after_loading_restart(void)
{
  ...
  if(ThisTask == 0 && All.MaxRMSDisplacementFac != all.MaxRMSDisplacementFac)
    warn("MaxRMSDisplacementFac modified from %g to %g while restarting at Time=%g", All.MaxRMSDisplacementFac,
         all.MaxRMSDisplacementFac, All.Time);
  ...
  if(ThisTask == 0 && All.MinimumTracerHsml != all.MinimumTracerHsml)
    warn("MinimumTracerHsml modified from %g to %g while restarting at Time=%g", All.MinimumTracerHsml, all.MinimumTracerHsml,
         All.Time);
  ...
  if(ThisTask == 0 && All.MinimumMetallicityOnStartUp != all.MinimumMetallicityOnStartUp)
    warn("MinimumMetallicityOnStartUp modified from %g to %g while restarting at Time=%g", All.MinimumMetallicityOnStartUp,
         all.MinimumMetallicityOnStartUp, All.Time);
...
#ifdef REFINEMENT
  if(ThisTask == 0 && All.RefinementCriterion != all.RefinementCriterion)
    warn("RefinementCriterion modified from %d to %d while restarting at Time=%g", All.RefinementCriterion, all.RefinementCriterion,
         All.Time);
  All.RefinementCriterion = all.RefinementCriterion;
  if(ThisTask == 0 && All.DerefinementCriterion != all.DerefinementCriterion)
    warn("DerefinementCriterion modified from %d to %d while restarting at Time=%g", All.DerefinementCriterion,
         all.DerefinementCriterion, All.Time);
  ...
  if(ThisTask == 0 && All.ConductionEfficiency != all.ConductionEfficiency)
    warn("ConductionEfficiency modified from %g to %g while restarting at Time=%g", All.ConductionEfficiency, all.ConductionEfficiency,
         All.Time);
  All.ConductionEfficiency = all.ConductionEfficiency;

  if(ThisTask == 0 && All.MaxConductionSubCycles != all.MaxConductionSubCycles)
    warn("MaxConductionSubCycles modified from %d to %d while restarting at Time=%g", All.MaxConductionSubCycles,
         all.MaxConductionSubCycles, All.Time);
...
#ifdef AURIGA_MOVIE
  if(ThisTask == 0 && strcmp(All.Auriga_Movie_Directory, all.Auriga_Movie_Directory) != 0)
    warn("Auriga_Movie_Directory modified from %s to %s while restarting at Time=%g", All.Auriga_Movie_Directory,
         all.Auriga_Movie_Directory, All.Time);
  strcpy(All.Auriga_Movie_Directory, all.Auriga_Movie_Directory);
#endif
  ...
  if(ThisTask == 0 && All.NetworkNSEThreshold != all.NetworkNSEThreshold)
    warn("NetworkNSEThreshold modified from %g to %g while restarting at Time=%g", All.NetworkNSEThreshold, all.NetworkNSEThreshold,
         All.Time);
...
#ifdef TGSET
  if(ThisTask == 0 && TGD.MaxTimeBinDiff != TGD_BAK.MaxTimeBinDiff)
    warn("TGD.MaxTimeBinDiff modified from %d to %d while restarting at Time=%g", TGD.MaxTimeBinDiff, TGD_BAK.MaxTimeBinDiff,
         All.Time);
  TGD.MaxTimeBinDiff = TGD_BAK.MaxTimeBinDiff;
  if(ThisTask == 0 && TGD.SnapFreeFallTimeFac != TGD_BAK.SnapFreeFallTimeFac)
    warn("TGD.SnapFreeFallTimeFac modified from %g to %g while restarting at Time=%g", TGD.SnapFreeFallTimeFac,
         TGD_BAK.SnapFreeFallTimeFac, All.Time);
  TGD.SnapFreeFallTimeFac = TGD_BAK.SnapFreeFallTimeFac;
  if(ThisTask == 0 && TGD.NHTerm != TGD_BAK.NHTerm)
    warn("TGD.NHTerm modified from %g to %g while restarting at Time=%g", TGD.NHTerm, TGD_BAK.NHTerm, All.Time);
  TGD.NHTerm = TGD_BAK.NHTerm;
  ...
  if(ThisTask == 0 && TGD.JeansNumberLow != TGD_BAK.JeansNumberLow)
    warn("TGD.JeansNumberLow modified from %g to %g while restarting at Time=%g", TGD.JeansNumberLow, TGD_BAK.JeansNumberLow,
         All.Time);
  TGD.JeansNumberLow = TGD_BAK.JeansNumberLow;
  if(ThisTask == 0 && TGD.JeansNumberHigh != TGD_BAK.JeansNumberHigh)
    warn("TGD.JeansNumberHigh modified from %g to %g while restarting at Time=%g", TGD.JeansNumberHigh, TGD_BAK.JeansNumberHigh,
         All.Time);
  ...
  if(ThisTask == 0 && HRD.RayMultiFreqMode != HRD_BAK.RayMultiFreqMode)
    warn("HRD.RayMultiFreqMode modified from %d to %d while restarting at Time=%g", HRD.RayMultiFreqMode, HRD_BAK.RayMultiFreqMode,
         All.Time);
  ...
  if(ThisTask == 0 && strcmp(All.SubboxCoordinatesPath, all.SubboxCoordinatesPath) != 0)
    warn("SubboxCoordinatesPath modified from %s to %s while restarting at Time=%g", All.SubboxCoordinatesPath,
         all.SubboxCoordinatesPath, All.Time);
  ...
  if(ThisTask == 0 && All.SubboxNumFilesPerSnapshot != all.SubboxNumFilesPerSnapshot)
    warn("SubboxNumFilesPerSnapshot modified from %d to %d while restarting at Time=%g", All.SubboxNumFilesPerSnapshot,
         all.SubboxNumFilesPerSnapshot, All.Time);
  ...
  if(ThisTask == 0 && All.SubboxNumFilesWrittenInParallel != all.SubboxNumFilesWrittenInParallel)
    warn("SubboxNumFilesWrittenInParallel modified from %d to %d while restarting at Time=%g", All.SubboxNumFilesWrittenInParallel,
         all.SubboxNumFilesWrittenInParallel, All.Time);
  ...
  if(ThisTask == 0 && All.MaxNumNgbDeviationEnrichment != all.MaxNumNgbDeviationEnrichment)
    warn("MaxNumNgbDeviationEnrichment modified from %g to %g while restarting at Time=%g", All.MaxNumNgbDeviationEnrichment,
         all.MaxNumNgbDeviationEnrichment, All.Time);
  ...
  if(ThisTask == 0 && All.WindEnergyIn1e51erg != all.WindEnergyIn1e51erg)
    warn("WindEnergyIn1e51erg modified from %g to %g while restarting at Time=%g", All.WindEnergyIn1e51erg, all.WindEnergyIn1e51erg,
         All.Time);
  All.WindEnergyIn1e51erg = all.WindEnergyIn1e51erg;
#else
  if(ThisTask == 0 && All.WindEnergyFraction != all.WindEnergyFraction)
    warn("WindEnergyFraction modified from %g to %g while restarting at Time=%g", All.WindEnergyFraction, all.WindEnergyFraction,
         All.Time);
  ...
  if(ThisTask == 0 && All.WindFreeTravelMaxTimeFactor != all.WindFreeTravelMaxTimeFactor)
    warn("WindFreeTravelMaxTimeFactor modified from %g to %g while restarting at Time=%g", All.WindFreeTravelMaxTimeFactor,
         all.WindFreeTravelMaxTimeFactor, All.Time);
  ...
  if(ThisTask == 0 && All.WindFreeTravelDensFac != all.WindFreeTravelDensFac)
    warn("WindFreeTravelDensFac modified from %g to %g while restarting at Time=%g", All.WindFreeTravelDensFac,
         all.WindFreeTravelDensFac, All.Time);
  ...
  if(ThisTask == 0 && All.VariableWindVelFactor != all.VariableWindVelFactor)
    warn("VariableWindVelFactor modified from %g to %g while restarting at Time=%g", All.VariableWindVelFactor,
         all.VariableWindVelFactor, All.Time);
  ...
  if(ThisTask == 0 && All.VariableWindSpecMomentum != all.VariableWindSpecMomentum)
    warn("VariableWindSpecMomentum modified from %g to %g while restarting at Time=%g", All.VariableWindSpecMomentum,
         all.VariableWindSpecMomentum, All.Time);
  ...
  if(ThisTask == 0 && All.VariableWindMassScale != all.VariableWindMassScale)
    warn("VariableWindMassScale modified from %g to %g while restarting at Time=%g", All.VariableWindMassScale,
         all.VariableWindMassScale, All.Time);
  ...
  if(ThisTask == 0 && All.WindSuppressionRedshift != all.WindSuppressionRedshift)
    warn("WindSuppressionRedshift modified from %g to %g while restarting at Time=%g", All.WindSuppressionRedshift,
         all.WindSuppressionRedshift, All.Time);
  ...
  if(ThisTask == 0 && All.HaloConcentrationNorm != all.HaloConcentrationNorm)
    warn("HaloConcentrationNorm modified from %g to %g while restarting at Time=%g", All.HaloConcentrationNorm,
         all.HaloConcentrationNorm, All.Time);
  All.HaloConcentrationNorm = all.HaloConcentrationNorm;

  if(ThisTask == 0 && All.HaloConcentrationSlope != all.HaloConcentrationSlope)
    warn("HaloConcentrationSlope modified from %g to %g while restarting at Time=%g", All.HaloConcentrationSlope,
         all.HaloConcentrationSlope, All.Time);
  ...
  if(ThisTask == 0 && All.AGB_Dust_Delta_Metal != all.AGB_Dust_Delta_Metal)
    warn("AGB_Dust_Delta_Metal modified from %g to %g while restarting at Time=%g", All.AGB_Dust_Delta_Metal, all.AGB_Dust_Delta_Metal,
         All.Time);
  ...
  if(ThisTask == 0 && All.SN_Dust_Delta_Metal != all.SN_Dust_Delta_Metal)
    warn("SN_Dust_Delta_Metal modified from %g to %g while restarting at Time=%g", All.SN_Dust_Delta_Metal, all.SN_Dust_Delta_Metal,
         All.Time);
  ...
  if(ThisTask == 0 && All.DustSingleGrainSize != all.DustSingleGrainSize)
    warn("DustSingleGrainSize modified from %g to %g while restarting at Time=%g", All.DustSingleGrainSize, all.DustSingleGrainSize,
         All.Time);
  ...
  if(ThisTask == 0 && All.Dust_Sputter_Tau_Fac != all.Dust_Sputter_Tau_Fac)
    warn("Dust_Sputter_Tau_Fac modified from %g to %g while restarting at Time=%g", All.Dust_Sputter_Tau_Fac, all.Dust_Sputter_Tau_Fac,
         All.Time);
  ...
  if(ThisTask == 0 && All.Dust_Destruction_Tau != all.Dust_Destruction_Tau)
    warn("Dust_Destruction_Tau modified from %g to %g while restarting at Time=%g", All.Dust_Destruction_Tau, all.Dust_Destruction_Tau,
         All.Time);
  All.Dust_Destruction_Tau = all.Dust_Destruction_Tau;
#endif
#endif

#if defined(DUST_LIVE) && defined(DL_GRAIN_BINS) && defined(DL_SUBCYCLE)
  if(ThisTask == 0 && All.DustSubcycleFac != all.DustSubcycleFac)
    warn("DustSubcycleFac modified from %g to %g while restarting at Time=%g", All.DustSubcycleFac, all.DustSubcycleFac, All.Time);
  All.DustSubcycleFac = all.DustSubcycleFac;
#endif

#ifdef BLACK_HOLES
  if(ThisTask == 0 && All.BlackHoleMaxAccretionRadius != all.BlackHoleMaxAccretionRadius)
    warn("BlackHoleMaxAccretionRadius modified from %g to %g while restarting at Time=%g", All.BlackHoleMaxAccretionRadius,
         all.BlackHoleMaxAccretionRadius, All.Time);
  ...
  if(ThisTask == 0 && All.BlackHoleRadioTriggeringFactor != all.BlackHoleRadioTriggeringFactor)
    warn("BlackHoleRadioTriggeringFactor modified from %g to %g while restarting at Time=%g", All.BlackHoleRadioTriggeringFactor,
         all.BlackHoleRadioTriggeringFactor, All.Time);
  ...
  if(ThisTask == 0 && All.DefaultICMDensity != all.DefaultICMDensity)
    warn("DefaultICMDensity modified from %g to %g while restarting at Time=%g", All.DefaultICMDensity, all.DefaultICMDensity,
         All.Time);
  All.DefaultICMDensity = all.DefaultICMDensity;

  if(ThisTask == 0 && All.RadioFeedbackFactor != all.RadioFeedbackFactor)
    warn("RadioFeedbackFactor modified from %g to %g while restarting at Time=%g", All.RadioFeedbackFactor, all.RadioFeedbackFactor,
         All.Time);
  ...
  if(ThisTask == 0 && All.SNEInjectionCriterion != all.SNEInjectionCriterion)
    warn("SNEInjectionCriterion modified from %ld to %ld while restarting at Time=%g", All.SNEInjectionCriterion,
         all.SNEInjectionCriterion, All.Time);
  ...
  if(ThisTask == 0 && All.SNEMinimalParticleNumber != all.SNEMinimalParticleNumber)
    warn("SNEMinimalParticleNumber modified from %d to %d while restarting at Time=%g", All.SNEMinimalParticleNumber,
         all.SNEMinimalParticleNumber, All.Time);
  All.SNEMinimalParticleNumber = all.SNEMinimalParticleNumber;

#if defined(SINK_PARTICLES) && defined(SINK_PARTICLES_FEEDBACK)
  if(ThisTask == 0 && All.SNEScatterAroundSink != all.SNEScatterAroundSink)
    warn("SNEScatterAroundSink modified from %g to %g while restarting at Time=%g", All.SNEScatterAroundSink, all.SNEScatterAroundSink,
         All.Time);
  All.SNEScatterAroundSink = all.SNEScatterAroundSink;
#endif

#endif

#if defined(SINK_PARTICLES) && defined(SINK_PARTICLES_FEEDBACK)
  if(ThisTask == 0 && All.SINKStarFormationEfficiency != all.SINKStarFormationEfficiency)
    warn("SINKStarFormationEfficiency modified from %g to %g while restarting at Time=%g", All.SINKStarFormationEfficiency,
         all.SINKStarFormationEfficiency, All.Time);
  All.SINKStarFormationEfficiency = all.SINKStarFormationEfficiency;
#endif

#ifdef SINK_PHOTOION_FEEDBACK
  if(ThisTask == 0 && All.StromgrenTemperature != all.StromgrenTemperature)
    warn("StromgrenTemperature modified from %g to %g while restarting at Time=%g", All.StromgrenTemperature, all.StromgrenTemperature,
         All.Time);
  All.StromgrenTemperature = all.StromgrenTemperature;
#endif
  ...
  if(ThisTask == 0 && strcmp(All.ExternalGravForcesFile, all.ExternalGravForcesFile) != 0)
    warn("ExternalGravForcesFile modified from %s to %s while restarting at Time=%g", All.ExternalGravForcesFile,
         all.ExternalGravForcesFile, All.Time);
  ...
  if(ThisTask == 0 && All.FeedbackRadiusLimiterFactor != all.FeedbackRadiusLimiterFactor)
    warn("FeedbackRadiusLimiterFactor modified from %g to %g while restarting at Time=%g", All.FeedbackRadiusLimiterFactor,
         all.FeedbackRadiusLimiterFactor, All.Time);
  All.FeedbackRadiusLimiterFactor = all.FeedbackRadiusLimiterFactor;
#endif

#ifdef SMUGGLE_RADIATION_FEEDBACK
  if(ThisTask == 0 && All.LumToMassRatioRadiationFeedback != all.LumToMassRatioRadiationFeedback)
    warn("LumToMassRatioRadiationFeedback modified from %g to %g while restarting at Time=%g", All.LumToMassRatioRadiationFeedback,
         all.LumToMassRatioRadiationFeedback, All.Time);
  All.LumToMassRatioRadiationFeedback = all.LumToMassRatioRadiationFeedback;
#endif
 ...
 *  \return void
 */
static void create_restartfiles_dir(void)
{
...
#endif
  printf(".\n");
  file_path_sprintf(buf, "%s/restartfiles", All.OutputDir);
#ifdef MULTIPLE_RESTARTS
  file_path_sprintf(buf, "%s/restartfiles_%03d", All.OutputDir, All.RestartFileCount);
#endif
  mkdir(buf, MKDIR_MODE);

#ifdef TOLERATE_WRITE_ERROR
  file_path_sprintf(buf, "%s/restartfiles", AlternativeOutputDir);
  mkdir(buf, MKDIR_MODE);
#endif
}
...
{
  file_path_sprintf(buf, "%s/restartfiles/restart.%d", All.OutputDir, task);

#ifdef MULTIPLE_RESTARTS
  if(modus == MODUS_WRITE)
    file_path_sprintf(buf, "%s/restartfiles_%03d/restart.%d", All.OutputDir, All.RestartFileCount++, task);
  if(modus == MODUS_READ || modus == MODUS_READCHECK || modus == MODUS_CHECK)
    file_path_sprintf(buf, "%s/restartfiles_%03d/restart.%d", All.OutputDir, All.RestartFileCount - 1, task);
#endif
}
...
static void backup_restartfiles(int task)
{
  ...
  file_path_sprintf(buf_bak, "%s/restartfiles/bak-restart.%d", All.OutputDir, ThisTask);
  ...
  file_path_sprintf(alternative_fname, "%s/restartfiles/restart.%d", AlternativeOutputDir, ThisTask);
  file_path_sprintf(buf_bak, "%s/restartfiles/bak-restart.%d", AlternativeOutputDir, ThisTask);
 ...
 *  \return void
 */
static void check_restart_files(const char *buf, struct check *ch, int *success)
{
  int oflag = O_RDONLY;
#ifdef USE_DIRECT_IO_FOR_RESTARTS
  struct stat st;
  if(stat(buf, &st) == 0)
          ...
          if(fd)
            {
              ...
              if(p == NULL)
                terminate("p == NULL");
              printf("RESTART: Topping of restart file '%s' by %zu bytes\n", buf, n);
              fwrite(p, n, 1, fd);
              fclose(fd);
              free(p);
            }
    ...
    terminate("Restart file '%s' not found.", buf);
  oflag |= O_DIRECT;
#endif
  ...
  if(memcmp(has_hash, ch->hash, 16) != 0 || memcmp(has_hash, written_hash, 16) != 0)
    {
      ...
      char newname[MAXLEN_PATH];
      file_path_sprintf(newname, "%s-damaged", buf);
      rename(buf, newname);
...
{
  if(ThisTask == 0)
    if(!(seq = (struct seq_data *)malloc(NTask * sizeof(struct seq_data))))
  ...
  if(ThisTask == 0)
  ...
  else
    {
      ...
      MPI_Recv(&type, 1, MPI_INT, 0, TAG_N, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void restart(int modus)
{
  ...
  if(NTask < All.NumFilesWrittenInParallel)
    {
      warn("Number of processors should be larger than or equal to `NumFilesWrittenInParallel'. We're adjusting the latter.\n");
  ...
  if(modus == MODUS_WRITE)
    if(ThisTask == 0)
      {
        if(!(checks = (struct check *)malloc(NTask * sizeof(struct check))))
          terminate("can't allocate checks");
        if(!(write_success = (char *)malloc(NTask)))
...
static void contents_restart_file(int modus)
{
...
#ifdef CHIMES
      ChimesAbundancesBuf = (double *)malloc(NumGas * ChimesGlobalVars.totalNumberOfSpecies * sizeof(double));
          ...
          for(partIndex = 0; partIndex < NumGas; partIndex++)
            {
              for(abunIndex = 0; abunIndex < ChimesGlobalVars.totalNumberOfSpecies; abunIndex++)
                ChimesAbundancesBuf[(partIndex * ChimesGlobalVars.totalNumberOfSpecies) + abunIndex] =
                    SphP[partIndex].ChimesGasVars.abundances[abunIndex];
            }
        }

      /* Abundance buffer */
      byten(&ChimesAbundancesBuf[0], NumGas * ChimesGlobalVars.totalNumberOfSpecies * sizeof(double), modus);
  ...
  if(modus == MODUS_READ) /* read */
    DC = (connection *)mymalloc_movable(&DC, "DC", MaxNvc * sizeof(connection));
...
#ifdef SFR_MCS_LOG
  byten(&(sf_dens_hist->n), sizeof(size_t), modus);
  byten(&(sf_dens_hist->range[0]), (sf_dens_hist->n + 1) * sizeof(double), modus);
  byten(&(sf_dens_hist->bin[0]), sf_dens_hist->n * sizeof(double), modus);
#endif

#ifdef SN_MCS_LOG
  byten(&(sn_dens_hist->n), sizeof(size_t), modus);
  byten(&(sn_dens_hist->range[0]), (sn_dens_hist->n + 1) * sizeof(double), modus);
  byten(&(sn_dens_hist->bin[0]), sn_dens_hist->n * sizeof(double), modus);
#endif
  ...
  if(NumSinks > 0)
    {
      /* SinkParticle data  */
      byten(&SinkP[0], NumSinks * sizeof(struct sink_particle_data), modus);
    }
#endif
#ifdef DUST_LIVE
  in(&N_dust, modus);
  ...
  if(modus)
    HRSL = (HRSL_struct *)mymalloc_movable(&HRSL, "HRSL", HRD.TotNumSources * sizeof(struct HRSL_struct));
  ...
  if(All.TotNumGas > 0)
    {
      if(modus == MODUS_READ)
        {
          Mesh.MaxNdp = maxndp;
          Mesh.DP     = (point *)myrealloc_movable(Mesh.DP, Mesh.MaxNdp * sizeof(point));
          Mesh.Ndp    = ndp;

          Mesh.NexpList    = nexp;
          Mesh.MaxNexpList = maxnexp;
          Mesh.expList     = (list_export_data *)myrealloc_movable(Mesh.expList, Mesh.MaxNexpList * sizeof(list_export_data));

          Mesh.NexpListNode    = nexpNode;
          Mesh.MaxNexpListNode = maxnexpNode;
          Mesh.expListNode = (list_export_data *)myrealloc_movable(Mesh.expListNode, Mesh.MaxNexpListNode * sizeof(list_export_data));
        }
      ...
      if(modus == MODUS_READ || modus == MODUS_READCHECK || modus == MODUS_CHECK) /* read */
        {
          ...
          while(n > 0)
            {
              if(iop != fillp)
              ...
              else
                {
                  ...
                  const ssize_t status = read(fdint, iobuf_aligned + fillp, nn);
                  if(status < 0 || (size_t)status != nn)
          ...
          if(hash) /* to prevent call if we write/load the checksum itself */
            MD5UpdateLong(&mysum, (unsigned char *)x, nin);
          ...
          while(n > 0)
            {
              if(iop < MAX_BLOCK_SIZE)
                ...
                {
                  const size_t nn      = MAX_BLOCK_SIZE;
                  const ssize_t status = write(fdint, iobuf_aligned, nn);
                  if(status < 0 || (size_t)status != nn)
...
void allocate_iobuf(void)
{
  ...
  if(!(io_buf = (char *)malloc(MAX_BLOCK_SIZE + PageSize)))
      ...
      if(iop > 0)
        {
          ...
          const ssize_t status = write(fdint, iobuf_aligned, iop);
          if(status < 0 || (size_t)status != iop)
            terminate("write error");
        }
    }
