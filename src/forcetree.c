 ...
 *              calculating the gravitational force. The type of tree
 *              implemented is a geometrical oct-tree, starting from a cube
 *              encompassing all particles. This cube is automatically found in
 *              the domain decomposition, which also splits up the global
 *              "top-level" tree along node boundaries, moving the particles of
 *              different parts of the tree to separate processors. In this
 *              version of the code, the tree construction may be repeated
 *              every time step without a renewed domain decomposition. If
 *              particles are on the "wrong" processor because a new domain
 *              decomposition has not been carried out, they are sent as
 *              temporary points to the right insertion processor according to
 *              the layout of the top-level nodes. In addition, the mapping of
 *              the top-level nodes to processors may be readjusted in order to
 *              improve work-load balance for the current time step.
 *
 *
  ...
  do
    {
       ...
       *
       * ToDo: Verify that this is actually working for an iterative tree construction with
       * insert_only_primary == 2; not sure this ever happens
       * */
      if(insert_only_primary != 2 || tree_num_nodes < 0)
        /* reallocate force tree structure */
      ...
      tree_num_nodes = force_treebuild(npart, optimized_domain_mapping, insert_only_primary, timebin);
    }
  while(tree_num_nodes < 0);

  return tree_num_nodes;
}
 ...
 */
int force_treebuild(
    int npart /*!< number of particles on local task */,
  ...
  if(last >= Tree_MaxPart)
    {
      if(last >= Tree_MaxPart + Tree_MaxNodes)
        /* a pseudo-particle or imported particle */
...
#endif
    {
      double locdata[2] = {(double)Tree_NumPartImported, (double)Tree_NumNodes}, sumdata[2];
 ...
 */
int force_treebuild_construct(
    int npart /*!< number of particles on local task */,
    int optimized_domain_mapping /*!< specifies if mapping of the top-level nodes to processors may be readjusted */,
    int insert_only_primary, int timebin)
{
  int i, j, no, flag = 0;
  int recvTask, count_ListNoData, *no_place = NULL;
  ...
  DomainNodeIndex[0] = Tree_NextFreeNode;
  ...
  level_list = (unsigned char *)mynew_movable(&level_list, "level_list", unsigned char, NumPart);
  Tree_IntPos_list =
...
#else
  if(flag)
    terminate("i=%d ID=%lld type=%d moved out of box. Pos[j=%d]=%g DomainCorner[%d]=%g DomainLen=%g", i, (long long)P[i].ID, P[i].Type,
              j, P[i].Pos[j], j, DomainCorner[j], DomainLen);
#endif
...
#pragma omp parallel for reduction(+ : mass_highres, mass_lowres)
  ...
  if(optimized_domain_mapping)
    {
      TaskCost    = (double *)mymalloc("TaskCost", NTask * sizeof(double));
      TaskCount   = (int *)mymalloc("TaskCount", NTask * sizeof(int));
      DomainCost  = (double *)mymalloc("DomainCost", NTopleaves * sizeof(double));
      DomainCount = (int *)mymalloc("DomainCount", NTopleaves * sizeof(int));
      ListNoData  = (struct no_list_data *)mymalloc("ListNoData", NTopleaves * sizeof(struct no_list_data));
      no_place    = (int *)mymalloc("no_place", NTopleaves * sizeof(int));
      ...
      if(All.Ti_Current > 0)
        {
          ...
          if(All.HighestActiveTimeBin <
            ...
            {
              mpi_printf("FORCETREE: we're not trying to optimize further because we just did a domain decomposition\n");
              memcpy(DomainNewTask, DomainTask, NTopleaves * sizeof(int));
            }
        }
      else
  ...
  /* make list of insertion top leaf and task for all particles */
  for(int idx = 0; idx < NTreeInsert; idx++)
    {
      i = INDEX(idx);
      if(i < 0)
  ...
  for(j = 0, Tree_NumSinksImported = 0, Tree_NumSinksExported = 0, Recv_offset[0] = 0, Send_offset[0] = 0; j < NTask; j++)
    {
      Tree_NumSinksImported += Recv_count[j];
      Tree_NumSinksExported += Send_count[j];
      if(j > 0)
        {
          Send_offset[j] = Send_offset[j - 1] + Send_count[j - 1];
          Recv_offset[j] = Recv_offset[j - 1] + Recv_count[j - 1];
...
#ifdef BLACK_HOLES
  Tree_AuxBH_Points = (struct treepoint_aux_bh_data *)mymalloc_movable(&Tree_AuxBH_Points, "Tree_AuxBH_Points",
                                                                       Tree_NumBHImported * sizeof(struct treepoint_aux_bh_data));
#endif

#ifdef SINKS
  Tree_AuxSinks_Points = (struct treepoint_aux_sinks_data *)mymalloc_movable(
      &Tree_AuxSinks_Points, "Tree_AuxSinks_Points", Tree_NumSinksImported * sizeof(struct treepoint_aux_sinks_data));
#endif
...
#ifdef BLACK_HOLES
  struct treepoint_aux_bh_data *export_Tree_AuxBH_Points =
      (struct treepoint_aux_bh_data *)mymalloc("export_Tree_AuxBH_Points", Tree_NumBHExported * sizeof(struct treepoint_aux_bh_data));
#endif

#ifdef SINKS
  struct treepoint_aux_sinks_data *export_Tree_AuxSinks_Points = (struct treepoint_aux_sinks_data *)mymalloc(
      "export_Tree_AuxSinks_Points", Tree_NumSinksExported * sizeof(struct treepoint_aux_sinks_data));
#endif
  ...
  /* prepare particle data to be copied to other tasks */
  for(int idx = 0; idx < NTreeInsert; idx++)
    {
      i = INDEX(idx);
      if(i < 0)
      ...
      if(task != ThisTask)
        {
...
#if defined(TREECOLV2_H2) || defined(TREECOLV2_CO) || defined(TREECOLV2_C)
          ...
          if(P[i].Type == DUST_LIVE)
            {
              ...
              export_Tree_Points[n].MinNumNgbDeviationDust = DTP(i).MinNumNgbDeviationDust;
              export_Tree_Points[n].MaxNumNgbDeviationDust = DTP(i).MaxNumNgbDeviationDust;
#endif
#ifdef DL_DEREFINEMENT
              export_Tree_Points[n].DustID           = P[i].ID;
              export_Tree_Points[n].BinMassChgTau    = DTP(i).BinMassChgTau;
              export_Tree_Points[n].OldBinMassChgTau = DTP(i).OldBinMassChgTau;
              for(int kk = 0; kk < DL_GRAIN_BINS; kk++)
                {
                  export_Tree_Points[n].NumGrains[kk] = DTP(i).NumGrains[kk];
#ifdef DL_GRAIN_BINS_PIECEWISE_LINEAR
                  export_Tree_Points[n].BinSlopes[kk] = DTP(i).BinSlopes[kk];
#endif
                }
              for(int kk = 0; kk < GFM_N_CHEM_ELEMENTS; kk++)
                {
                  export_Tree_Points[n].MetalFractions[kk] = DTP(i).MetalFractions[kk];
                }
              for(int kk = 0; kk < 3; kk++)
                {
                  export_Tree_Points[n].Vel[kk] = P[i].Vel[kk];
                }
#endif
#if defined(DL_DEREFINEMENT)
              export_Tree_Points[n].origTask = ThisTask;
#endif
            }
#endif
          ...
          if(P[i].Type == 5)
            {
...
#ifdef MRT_BH
              export_Tree_AuxBH_Points[k].BH_PhotonHsml = BPP(i).BH_PhotonHsml;
#endif
              ...
              export_Tree_AuxBH_Points[k].BH_MPB_CumEgyLow  = BPP(i).BH_MPB_CumEgyLow;
              export_Tree_AuxBH_Points[k].BH_MPB_CumEgyHigh = BPP(i).BH_MPB_CumEgyHigh;
          ...
          if(P[i].Type == 5)
            {
              int k                                           = Send_offset[task] + Send_count[task]++;
              export_Tree_AuxSinks_Points[k].ID               = P[i].ID;
              export_Tree_AuxSinks_Points[k].SwallowID        = SKP(i).SwallowID;
              export_Tree_AuxSinks_Points[k].Sinks_Hsml       = SKP(i).Sinks_Hsml;
              export_Tree_AuxSinks_Points[k].Vel[0]           = P[i].Vel[0];
              export_Tree_AuxSinks_Points[k].Vel[1]           = P[i].Vel[1];
              export_Tree_AuxSinks_Points[k].Vel[2]           = P[i].Vel[2];
              export_Tree_AuxSinks_Points[k].Sinks_CountProgs = SKP(i).Sinks_CountProgs;
            }
#endif
        }
    }
  ...
  for(int ngrp = 1; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&export_Tree_AuxBH_Points[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct treepoint_aux_bh_data),
                       MPI_BYTE, recvTask, TAG_DENS_B, &Tree_AuxBH_Points[Recv_offset[recvTask]],
                       Recv_count[recvTask] * sizeof(struct treepoint_aux_bh_data), MPI_BYTE, recvTask, TAG_DENS_B, MPI_COMM_WORLD,
                       MPI_STATUS_IGNORE);
    }
  ...
  for(int ngrp = 1; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&export_Tree_AuxSinks_Points[Send_offset[recvTask]],
                       Send_count[recvTask] * sizeof(struct treepoint_aux_sinks_data), MPI_BYTE, recvTask, TAG_DENS_B,
                       &Tree_AuxSinks_Points[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct treepoint_aux_sinks_data),
                       MPI_BYTE, recvTask, TAG_DENS_B, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }

  myfree(export_Tree_AuxSinks_Points);
...
int force_treebuild_insert_single_point(int i /*!< index of particle */,
                                        ...
                                        int th /*!< target node */, unsigned char levels /*!< level of target node */)
{
  int j, parent = -1;
  ...
  if(TakeLevel >= 0)
    {
      ...
      double *loc_cost  = (double *)mymalloc("loc_cost", NTopnodes * sizeof(double));
      double *glob_cost = (double *)mymalloc("glob_cost", NTopnodes * sizeof(double));
      ...
      gdata_export = (struct gravcost_data *)mymalloc("grav_data_export", Tree_NumPartExported * sizeof(struct gravcost_data));
      gdata_import = (struct gravcost_data *)mymalloc("grav_data_import", Tree_NumPartImported * sizeof(struct gravcost_data));
 ...
 *  \return void
 */
void force_update_node_recursive(int no /*!< node for which the moments shall be found */, int sib /*!< sibling of node no */,
                                 ...
                                 int *last /*!< last node for which this function was called, or -1 when called for root node */)
{
  ...
  double h2_mass_fraction;
#endif
#ifdef TREECOLV2_CO
  double co_mass_fraction;
#endif
#ifdef TREECOLV2_C
  double c_mass_fraction;
#endif
#ifdef TREECOLV2_VEL
  float v[3];
#endif
...
#endif
#ifdef TREECOLV2_C
                      c_mass_fraction = 12.0 * SphP[p].TracAbund[ICATOM] / (1.0 + 4.0 * ABHE);
                      cmass += P[p].Mass * c_mass_fraction;
#endif
                    }
#endif /*TREECOLV2*/
#ifdef TREECOLV2_VEL
...
#endif
#ifdef TREECOLV2_C
                      c_mass_fraction = 12.0 * Tree_Points[n].TracAbund[ICATOM] / (1.0 + 4.0 * ABHE);
                      cmass += Tree_Points[n].Mass * c_mass_fraction;
#endif
#ifdef TREECOLV2_VEL
...
void force_exchange_topleafdata(void)
{
  ...
  for(int n = 0, idx = 0; n < NTopleaves; n++)
    {
...
#ifdef TREECOLV2
          loc_DomainMoment[idx].gasmass = Nodes[no].u.d.gasmass;
#ifdef TREECOLV2_H2
...
#endif
#ifdef TREECOLV2_C
          loc_DomainMoment[idx].cmass = Nodes[no].u.d.cmass;
#endif
#endif /*TREECOLV2*/
...
#ifdef TREECOLV2
          Nodes[no].u.d.gasmass = DomainMoment[idx].gasmass;
#ifdef TREECOLV2_H2
...
#endif
#ifdef TREECOLV2_C
          Nodes[no].u.d.cmass = DomainMoment[idx].cmass;
#endif
#endif /*TREECOLV2*/
 ...
 *  \return void
 */
void force_treeupdate_toplevel(
    int no /*!< node to be updated */, int topnode /*!< index of the node no in the #TopNodes array */,
...
void forcetree_update_exported_potential_values(void)
{
  ...
  struct update_treepoint_data *export_Tree_Points =
      (struct update_treepoint_data *)mymalloc("export_Tree_Points", Tree_NumPartExported * sizeof(struct update_treepoint_data));
  struct update_treepoint_data *import_Tree_Points =
      (struct update_treepoint_data *)mymalloc("import_Tree_Points", Tree_NumPartImported * sizeof(struct update_treepoint_data));
  ...
  for(int ngrp = 1; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(recvTask < NTask)
        if(Force_Send_count[recvTask] > 0 || Force_Recv_count[recvTask] > 0)
          MPI_Sendrecv(&export_Tree_Points[Force_Send_offset[recvTask]],
                       Force_Send_count[recvTask] * sizeof(struct update_treepoint_data), MPI_BYTE, recvTask, TAG_DENS_A,
                       &import_Tree_Points[Force_Recv_offset[recvTask]],
                       Force_Recv_count[recvTask] * sizeof(struct update_treepoint_data), MPI_BYTE, recvTask, TAG_DENS_A,
                       MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
