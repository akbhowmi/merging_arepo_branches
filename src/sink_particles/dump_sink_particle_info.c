...
/*  Write all properties of all the sinks to a numbered file,
 *  this is needed if we are restarting from a snapshot
 */
void dump_sink_particle_snapshot(int snapNum)
{
  char buf[MAXLEN_PATH];

  file_path_sprintf(buf, "%s/sink_snap_%03d", All.OutputDir, snapNum);

  mpi_printf("SINK_PARTICLES: Writing sink particle snapshot %s \n", buf);
  ...
  fwrite(&NSinksAllTasks, sizeof(int), 1, fd);
  fwrite(&SinkP[0], sizeof(struct global_sink_particle_data), NSinksAllTasks, fd);
...
void dump_sink_particle_info(int success)
{
  ...
  if((get_time_difference_in_Gyr(SinksLastEvolutionDumpTime, All.Time) * 1.0e9 > SinkEvolutionDumpRateYears) || (success == 1))
    {
      SinksLastEvolutionDumpTime = All.Time;
...
int open_sink_particle_evolution_file(void)
{
  ...
  file_path_sprintf(sink_dir, "%s/sink_particle_info/", All.OutputDir);

  /* Make sure the sink directory is present */
  if(stat(sink_dir, &st) == -1)
    mkdir(sink_dir, MKDIR_MODE);
  else
  ...
  while(file_no < max_file_no)
    {
      file_path_sprintf(filename, "%s/sink_particle_info/sink_particle_evolution_%03d", All.OutputDir, file_no);
...
{
  mpi_printf("SINK_PARTICLES: Reading in the sink particle snapshot %d and initializing the SinkP structure with the gathered info\n",
             snapNum);

  char buf[MAXLEN_PATH];
  file_path_sprintf(buf, "%s/sink_snap_%03d", All.OutputDir, snapNum);
  mpi_printf("About to open the sink snapshot named %s \n", buf);
  FILE *fd;
  if(!(fd = fopen(buf, "r")))
    {
      mpi_printf("SINK_PARTICLES: Could not find the sink particles snapshot %s \n", buf);
      return 0;
    }
  ...
  my_fread(&ScanTime, sizeof(double), 1, fd);
  my_fread(&ScanNSinks, sizeof(int), 1, fd);
  my_fread(&SinkP[0], sizeof(struct global_sink_particle_data), ScanNSinks, fd);

  fclose(fd);

#ifdef DEBUG_SINK_PARTICLES
  for(int i = 0; i < ScanNSinks; i++)
    mpi_printf("SINK PARTICLES: sink %d ID %lld Mass %g Formation mass %g Task %d\n", i, SinkP[i].ID, SinkP[i].Mass,
               SinkP[i].FormationMass, SinkP[i].HomeTask);
#endif

  NSinksAllTasks = ScanNSinks;

  for(int i = 0; i < ScanNSinks; i++)
    if(SinkP[i].Mass == 0 && SinkP[i].ID == 0)
      ScanNSinks--;

  /* Check integrity of the freshly read sinks */
  int nsinks = get_all_sink_particle_info(1);
  if(nsinks != ScanNSinks)
    terminate("some sinks are missing\n");
  return 1;
}
...
int open_old_sink_file_and_read_sink_data(void)
{
  ...
  if(stat(sink_dir, &st) == -1)
    {
      terminate("SINK PARTICLES READ -- problem! Sinks present but can't find sink directory %s\n", sink_dir);
    }
  else
    {
