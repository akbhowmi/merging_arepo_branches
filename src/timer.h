    ...
    {                                                                              \
      ...
      for(int itimer = 0; itimer <= TimerStackPos; itimer++)                       \
        if(counter == TimerStack[itimer])                                          \
          terminate("Try to start timer %d, but it is already running.", counter); \
...
TIMER_CREATE(CPU_MAG_BATTERIES, "mag batteries", CPU_HYDRO, '|', '|')
#endif
#ifdef MHD_DEDNER
TIMER_CREATE(CPU_MHD_DEDNER, "mhd dedner", CPU_MHD, 'I', 'i')
#endif
...
TIMER_CREATE(CPU_DUST_RP, "rad press", CPU_DUST, 'r', 'R')
...
TIMER_CREATE(CPU_SINKS, "sinks", CPU_ALL, 's', 'S')
TIMER_CREATE(CPU_SINKS_CREATE, "creat", CPU_SINKS, 'c', 'C')
TIMER_CREATE(CPU_SINKS_ACCRETION, "accretion", CPU_SINKS, 'a', 'A')
#ifdef SINKS_MERGERS
TIMER_CREATE(CPU_SINKS_NGB, "ngb search", CPU_SINKS, 'n', 'N')
TIMER_CREATE(CPU_SINKS_MERGERS, "mergers", CPU_SINKS, 'm', 'M')
#endif
#endif
#if defined(MONOTONE_CONDUCTION) || defined(CONDUCTION)
TIMER_CREATE(CPU_CONDUCTION, "conduction", CPU_ALL, '{', '}')
#endif
#ifdef SIMPLEX
TIMER_CREATE(CPU_SIMPLEX, "simplex", CPU_ALL, '~', '^')
TIMER_CREATE(CPU_SX_EXCHANGE, "exchange", CPU_SIMPLEX, 'L', '1')
TIMER_CREATE(CPU_SX_DIFFUSE, "diffuse", CPU_SIMPLEX, 'M', '2')
TIMER_CREATE(CPU_SX_BALLISTIC, "ballistic", CPU_SIMPLEX, 'N', '3')
TIMER_CREATE(CPU_SX_CHEMISTRY, "chemistry", CPU_SIMPLEX, 'O', '4')
TIMER_CREATE(CPU_SX_SOURCES, "sources", CPU_SIMPLEX, 'P', '5')
#endif
#ifdef MRT
TIMER_CREATE(CPU_RT, "RT", CPU_ALL, '{', '}')
TIMER_CREATE(CPU_RT_FLUX, "fluxes", CPU_RT, '{', '}')
TIMER_CREATE(CPU_RT_COOLING, "cooling", CPU_RT, '{', '}')
TIMER_CREATE(CPU_RT_CHEM, "thermo-chemistry", CPU_RT, '{', '}')
TIMER_CREATE(CPU_RT_RP, "radpress", CPU_RT, '{', '}')
TIMER_CREATE(CPU_RT_EXCHANGE, "exchange", CPU_RT, '{', '}')
TIMER_CREATE(CPU_RT_GRADIENTS, "gradients", CPU_RT, '{', '}')
#endif
...
TIMER_CREATE(CPU_BRAGINSKII, "Braginskii viscosity", CPU_ALL, '{', '}')
TIMER_CREATE(CPU_BRAGINSKII_PREPARE, "Brag prepare", CPU_BRAGINSKII, '{', '}')
TIMER_CREATE(CPU_BRAGINSKII_LEAST_SQUARE_MATRIX, "Brag prepare matrix", CPU_BRAGINSKII_PREPARE, '{', '}')
TIMER_CREATE(CPU_BRAGINSKII_LEAST_SQUARE_MATRIX_GET_VALUES, "Get vals", CPU_BRAGINSKII_LEAST_SQUARE_MATRIX, '{', '}')
TIMER_CREATE(CPU_BRAGINSKII_EXCHANGE, "Brag exchange", CPU_BRAGINSKII, '{', '}')
TIMER_CREATE(CPU_BRAGINSKII_CALCULATE_AND_ADD_FLUXES, "Brag calculate and add fluxes", CPU_BRAGINSKII, '{', '}')
#endif
...
TIMER_CREATE(CPU_CR_STREAMING_PREPARE, "preparation", CPU_CR_STREAMING, '{', '}')
TIMER_CREATE(CPU_CR_STREAMING_SOLVE, "solver", CPU_CR_STREAMING, '{', '}')
TIMER_CREATE(CPU_CR_STREAMING_SOLVE_EXPLICIT, "explicit flux", CPU_CR_STREAMING_SOLVE, '{', '}')
TIMER_CREATE(CPU_CR_STREAMING_SOLVE_MATRIX, "matrix setup", CPU_CR_STREAMING_SOLVE, '{', '}')
TIMER_CREATE(CPU_CR_STREAMING_SET_COEFF, "set coeff", CPU_CR_STREAMING_SOLVE, '{', '}')
TIMER_CREATE(CPU_CR_STREAMING_SOLVE_IMPLICIT, "implicit solve", CPU_CR_STREAMING_SOLVE, '{', '}')
#endif
#ifdef FOF
