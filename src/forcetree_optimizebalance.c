...
#else
  double *inTaskCost = (double *)mymalloc("inTaskCost", NTask * sizeof(double));
  ;
...
void force_get_global_cost_for_leavenodes(int nexport)
{
  ...
  export_node_data = (struct node_data *)mymalloc("export_node_data", nexport * sizeof(struct node_data));
  import_node_data = (struct node_data *)mymalloc("import_node_data", nimport * sizeof(struct node_data));
...
void force_optimize_domain_mapping(void)
{
  ...
  force_domainAssign = (struct force_segments_data *)mymalloc("force_domainAssign", ncpu * sizeof(struct force_segments_data));
  ...
  struct mydata *ncost  = (struct mydata *)mymalloc("ncost", NTask * sizeof(struct mydata));
  struct mydata *ncount = (struct mydata *)mymalloc("ncount", NTask * sizeof(struct mydata));
  struct mydata *ncombi = (struct mydata *)mymalloc("ncombi", NTask * sizeof(struct mydata));
  ...
  int rep, *candidates = (int *)mymalloc("candidates", n_lowest * sizeof(int));
  struct mydata *np;
