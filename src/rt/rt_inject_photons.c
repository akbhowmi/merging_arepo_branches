...
void rt_inject_photons_spread(tessellation *T, double dt)
{
  ...
  All.BunchSize  = (int)((All.BufferSize * 1024 * 1024) /
                        (sizeof(data_index) + sizeof(struct data_nodelist) + sizeof(struct sourcedata_in) +
                         sizeof(struct sourcedata_in) + smax(sizeof(struct sourcedata_in), sizeof(struct sourcedata_in))));
          ...
          if(recvTask < NTask)
            {
              if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
                {
                  /* get the particles */
                  MPI_Sendrecv(&SourceDataIn[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct sourcedata_in), MPI_BYTE,
                               recvTask, TAG_DENS_A, &SourceDataGet[Recv_offset[recvTask]],
                               Recv_count[recvTask] * sizeof(struct sourcedata_in), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                               MPI_STATUS_IGNORE);
                }
            }
        }
