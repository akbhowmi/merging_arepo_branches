      ...
      if(recvTask < NTask)
        {
          if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&SourceList[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct source_list), MPI_BYTE, recvTask,
                           TAG_DENS_A, &SourceListGet[Recv_offset[recvTask]], Recv_count[recvTask] * sizeof(struct source_list),
