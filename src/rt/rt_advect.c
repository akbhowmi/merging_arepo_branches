          ...
          for(iside = 0; iside < 2; iside++) /* consider both sides of the face in turn */
            {
              ...
              if(densphot > 0)
                {
                  delta_densphot =
                      -dt_half / All.cf_atime *
                          (velPhot[0] * g->ddensphot[j][0] + velPhot[1] * g->ddensphot[j][1] + velPhot[2] * g->ddensphot[j][2]) +
                      g->ddensphot[j][0] * dx + g->ddensphot[j][1] * dy + g->ddensphot[j][2] * dz;
...
#define MAX_COUNT_DIFF 200
  ...
  struct radinflux_list_data *RadinFluxListGet =
      (struct radinflux_list_data *)mymalloc("RadinFluxListGet", nimport * sizeof(struct radinflux_list_data));
      ...
      if(recvTask < NTask)
        {
          if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
            {
              /* get the particles */
              MPI_Sendrecv(&RadinFluxList[Send_offset[recvTask]], Send_count[recvTask] * sizeof(struct radinflux_list_data), MPI_BYTE,
                           recvTask, TAG_DENS_A, &RadinFluxListGet[Recv_offset[recvTask]],
                           Recv_count[recvTask] * sizeof(struct radinflux_list_data), MPI_BYTE, recvTask, TAG_DENS_A, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
            }
        }
    }
