...
void rt_init_gradients(void)
{
  ...
  for(k = 0; k < RT_N_DIR; k++)
    rt_gradient_init(&SphP[0].DensPhot[k], &RTPrimExch[0].DensPhot[k], SphP[0].rt_Grad.ddensphot[k], &SphP[0].SourceID[k],
                     &RTPrimExch[0].SourceID[k], GRADIENT_TYPE_PHOTONS + k);
#else
  for(k = 0; k < RT_N_DIR; k++)
    rt_gradient_init(&SphP[0].DensPhot[k], &RTPrimExch[0].DensPhot[k], SphP[0].rt_Grad.ddensphot[k], NULL, NULL,
                     GRADIENT_TYPE_PHOTONS + k);
#endif
...
void rt_calculate_green_gauss_gradients(void)
{
  ...
  /* calculate a gradient for the total radiation field (without slope limiting), used for determining the direction of radiation
   * transport */
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    {
      i = TimeBinsHydro.ActiveParticleList[idx];
