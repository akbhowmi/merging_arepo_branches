 ...
 *  \return void
 */
void read_ic(const char *const fname, const int readTypes)
{
  int rest_files, ngroups, gr, filenr, masterTask, lastTask, groupMaster;
  double u_init, molecular_weight;
  char buf[MAXLEN_PATH];
  double t0, t1;

  if(All.ICFormat < SNAP_FORMAT_GADGET || All.ICFormat > SNAP_FORMAT_HDF5)
    mpi_terminate("ICFormat=%d not supported.", All.ICFormat);
  ...
  for(int rep = 0; rep < 2; rep++)
    {
      ...
      while(rest_files > NTask)
        {
          file_path_sprintf(buf, "%s.%d", fname, ThisTask + (rest_files - NTask));
          if(All.ICFormat == SNAP_FORMAT_HDF5)
            file_path_sprintf(buf, "%s.%d.hdf5", fname, ThisTask + (rest_files - NTask));
      ...
      if(rest_files > 0)
        {
          ...
          if(num_files > 1)
            {
              file_path_sprintf(buf, "%s.%d", fname, filenr);
              if(All.ICFormat == SNAP_FORMAT_HDF5)
                file_path_sprintf(buf, "%s.%d.hdf5", fname, filenr);
            }
          else
            {
              file_path_sprintf(buf, "%s", fname);
              if(All.ICFormat == SNAP_FORMAT_HDF5)
...
#ifdef SINKS
          int max_sinksload;
          MPI_Allreduce(&NumSinks, &max_sinksload, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
          All.MaxPartSinks = max_sinksload + ALLOC_SINKS_ROOM * (All.TotNumGas / NTask);
#endif
  ...
  /* All gas that is already present in the ICs is allowed to be (de-)refined */
      ...
      if(count)
        {
          ...
          for(int i = 0; i < NumGas - count; i++) /* make sure that AllowRefinement is shifted with the particles */
            SphP[i + count].AllowRefinement = SphP[i].AllowRefinement;
          for(int i = 0; i < count; i++) /* by default, new cells are not allowed to be refined */
          ...
          for(int i = count, j = 0; i < NumPart; i++)
#ifdef SPLIT_PARTICLE_TYPE
            if((1 << P[i].Type) & (SPLIT_PARTICLE_TYPE))
#else
            if(P[i].Type == PTYPE_HALO)
...
#ifdef REFINEMENT_HIGH_RES_GAS
                /* also allow gas which is produced by splitting a high-res DM particle to be (de-)refined */
                if(P[i].Type == PTYPE_HALO)
          ...
          for(int i = 0; i < count; i++)
            P[i].Vel[0] += TGD.StreamVel / sqrt(All.cf_atime);
#endif
        }
    }
#else
  ...
  if(RestartFlag == RESTART_IC)
    {
      ...
      for(int i = 0; i < NumGas; i++)
        {
          for(int j = 0; j < All.TracerMCPerCell; j++)
            {
              int itr = get_free_tracer_slot();

              TracerLinkedList[itr].ID = 1 + P[i].ID * All.TracerMCPerCell + j;
              if(TracerLinkedList[itr].ID == -1)
                terminate("GENERATE_TRACER_MC_IN_ICS: tracer id too large"); /* the IDs are of unsigned type, and -1 is a reserved
                                                                                value in some places in the code */
  ...
  if(RestartFlag == RESTART_IC)
    {
      ...
      for(int i = 0, j = NumPart; i < NumGas; i++)
        if(P[i].Type == PTYPE_GAS)
          {
            P[j].Mass   = 0;
  ...
  if(RestartFlag == RESTART_IC)
    {
      if(All.MassTable[PTYPE_STARS] == 0 && All.MassTable[PTYPE_GAS] > 0)
        {
          All.MassTable[PTYPE_GAS]   = 0;
          All.MassTable[PTYPE_STARS] = 0;
        }
    }
#endif
...
#ifdef BLACK_HOLES
  for(int i = N_BH_idx = 0; i < NumPart; i++)
...
#ifdef SINKS
  for(int i = N_Sinks_idx = 0; i < NumPart; i++)
    if(P[i].Type == PTYPE_BNDRY)
      {
        SinkP[N_Sinks_idx].PID = i;
        P[i].AuxDataID         = N_Sinks_idx;
        N_Sinks_idx++;
      }
#endif
  ...
  for(int i = N_dust_idx = 0; i < NumPart; i++)
  ...
  header.mass[PTYPE_GAS]   = 0; /* to make sure that the variable masses are stored in output file */
  All.MassTable[PTYPE_GAS] = 0;
  ...
  for(int i = 0; i < NumGas; i++)
    {
      SphP[i].Utherm = fmax(All.MinEgySpec, SphP[i].Utherm);
      if(SphP[i].Density > 0)
...
/* convert metal abundances to metal masses */
  ...
  long long glob_num;
  sumup_large_ints(1, &NumPart, &glob_num);
 ...
 * \param[in] type If blocknr == IO_VEL, P[n].Type is set to type.
 *
 * \return void
 */
void empty_read_buffer(const enum iofields blocknr, const int offset, const int pc, const int type)
{
#ifdef AUTO_SWAP_ENDIAN_READIC
  int vt, vpb;
  char *cp;
  ...
  MyInputFloat *fp = (MyInputFloat *)CommBuffer;
 ...
 *  \return void
 */
void share_particle_number_in_file(const char *const fname, const int filenr, const int readTask, const int lastTask, int readTypes)
{
  ...
  FILE *fd = 0;
  int type;
  char label[IO_LABEL_SIZE], buf[50];
  int nextblock;
#ifdef HAVE_HDF5
      ...
      if(All.ICFormat == SNAP_FORMAT_GADGET || All.ICFormat == SNAP_FORMAT_GADGET_VARIANT)
        {
          ...
          if(All.ICFormat == SNAP_FORMAT_GADGET_VARIANT)
            {
              SKIP;
#ifdef AUTO_SWAP_ENDIAN_READIC
              swap_file = blksize1;
#endif
              my_fread(&label, sizeof(char), IO_LABEL_SIZE, fd);
          ...
          SKIP;
#ifdef AUTO_SWAP_ENDIAN_READIC
          if(All.ICFormat == SNAP_FORMAT_GADGET)
            {
              if(blksize1 != 256)
                swap_file = 1;
            }
  ...
  if(All.TotNumPart == 0)
    {
      if(num_files == 1)
        for(type = 0; type < NTYPES; type++)
          {
            if(header.npartTotal[type] != (unsigned int)header.npart[type])
              {
                warn("header.npartTotal[%d]=%u != header.npart[%d]=%d, setting header.npartTotal[%d] = header.npart[%d]\n", type,
...
#endif
#ifdef SINKS
      All.TotN_sinks = header.npartTotal[5] + (((long long)header.npartTotalHighWord[5]) << 32);
#endif
#ifdef DUST_LIVE
    ...
    !defined(EXTERNALGRAVITY)
      ...
      All.TotPartSources = header.npartTotal[4] + (((long long)header.npartTotalHighWord[4]) << 32) + header.npartTotal[5] +
                           (((long long)header.npartTotalHighWord[5]) << 32);
      mpi_printf("Tot Sources %d %d %d\n", All.TotPartSources, header.npart[4] + header.npart[5],
                 header.npartTotal[4] + header.npartTotal[5]);
#endif
...
#endif
#ifdef SINKS
      All.TotN_sinks *= All.TileICsFactor * All.TileICsFactor * All.TileICsFactor;
#endif
#ifdef DUST_LIVE
 ...
 *  \return void
 */
void read_file(const char *const fname, const int filenr, const int readTask, const int lastTask, int readTypes)
{
  int n_in_file, n_for_this_task, ntask, pc;
  int blksize1, blksize2;
  MPI_Status status;
  FILE *fd = NULL;
  char label[IO_LABEL_SIZE], expected_label[IO_LABEL_SIZE];
  char buf[IO_DATASET_NAME_SIZE];
  int nextblock;
  ...
  for(int bnr = 0; bnr < IO_LASTENTRY + 1; bnr++)
    {
      ...
      if(blockpresent(blocknr, 0))
        {
          ...
          const int bytes_per_blockelement = get_bytes_per_blockelement(blocknr, 1);
                  ...
                  if(All.ICFormat == SNAP_FORMAT_GADGET_VARIANT)
                    {
                      ...
                      if(strncmp(label, expected_label, IO_LABEL_SIZE) != 0)
                        terminate("incorrect block structure!\nexpected '%c%c%c%c' but found '%c%c%c%c'\n", expected_label[0],
...
#ifdef HAVE_HDF5
                                  if(All.ICFormat == SNAP_FORMAT_HDF5 && pc > 0)
                                    {
                  ...
                  if(All.ICFormat == SNAP_FORMAT_GADGET || All.ICFormat == SNAP_FORMAT_GADGET_VARIANT)
                    {
                      ...
                      if(blksize1 != blksize2)
                        {
                          char *msg_id_mismatch = "";
                          if(blocknr == IO_ID)
                            {
                              msg_id_mismatch = "Possible mismatch of 32bit and 64bit ID's in IC file and AREPO compilation!\n";
                            }
                          terminate("incorrect block sizes detected!\n Task=%d   blocknr=%d  blksize1=%d  blksize2=%d\n%s", ThisTask,
                                    (int)blocknr, blksize1, blksize2, msg_id_mismatch);
                        }
                    }
                }
 ...
 */
int find_files(const char *const fname)
{
  FILE *fd;
  char buf[MAXLEN_PATH], buf1[MAXLEN_PATH];
  int dummy;

  if(All.ICFormat == SNAP_FORMAT_HDF5)
    {
      file_path_sprintf(buf, "%s.%d.hdf5", fname, 0);
      file_path_sprintf(buf1, "%s.hdf5", fname);
    }
  else
    {
      file_path_sprintf(buf, "%s.%d", fname, 0);
      file_path_sprintf(buf1, "%s", fname);
    }
  ...
  mpi_terminate("Can't find initial conditions file, neither as '%s'\nnor as '%s'", buf, buf1);
  return EXIT_FAILURE;
}
 ...
 *  \param[out] filenr Contains the file number to which this task belongs.
 *  \param[out] master The number of the task responsible to read the file.
 *  \param[out] last Number of the last task belonging to the same file as this
 *                   task.
 *
 *  \return void
 */
void distribute_file(const int nfiles, int *const filenr, int *const master, int *const last)
{
  ...
  if(tasks_left == 0)
    {
      const int group = ThisTask / tasks_per_file;
      ...
      for(int i = 0; i < nfiles; i++)
        {
          ...
          *filenr = i;
          ...
          if(*master <= ThisTask && ThisTask <= *last)
            return;
        }
    }
}
 ...
 */
void read_header_attributes_in_hdf5(const char *const fname)
{
 ...
 */
void read_header_attributes(FILE *const fd)
{
#ifdef NTYPES_ICS
  int type;
  if(RestartFlag == RESTART_IC)
 ...
 * \return void
 */
void swap_Nbyte(char *const data, const int n, const int m)
{
  char old_data[16];
...
void tile_ics(void)
{
...
#endif
#ifdef SINKS
  domain_resize_storage_blackholes(NumSinks * (All.TileICsFactor * All.TileICsFactor * All.TileICsFactor - 1));
#endif
#ifdef DUST_LIVE
  ...
  for(ix = 0; ix < All.TileICsFactor; ix++)
    for(iy = 0; iy < All.TileICsFactor; iy++)
      for(iz = 0; iz < All.TileICsFactor; iz++)
        if(ix != 0 || iy != 0 || iz != 0)
          memcpy(&StarP[N_star * ix + N_star * All.TileICsFactor * iy + N_star * All.TileICsFactor * All.TileICsFactor * iz],
                 &StarP[0], N_star * sizeof(struct star_particle_data));
#endif
#ifdef BLACK_HOLES
  for(ix = 0; ix < All.TileICsFactor; ix++)
    for(iy = 0; iy < All.TileICsFactor; iy++)
      for(iz = 0; iz < All.TileICsFactor; iz++)
        if(ix != 0 || iy != 0 || iz != 0)
          memcpy(&BHP[NumBHs * ix + NumBHs * All.TileICsFactor * iy + NumBHs * All.TileICsFactor * All.TileICsFactor * iz], &BHP[0],
                 NumBHs * sizeof(struct bh_particle_data));
#endif
#ifdef SINKS
  for(ix = 0; ix < All.TileICsFactor; ix++)
    for(iy = 0; iy < All.TileICsFactor; iy++)
      for(iz = 0; iz < All.TileICsFactor; iz++)
        if(ix != 0 || iy != 0 || iz != 0)
          memcpy(&SinkP[NumSinks * ix + NumSinks * All.TileICsFactor * iy + NumSinks * All.TileICsFactor * All.TileICsFactor * iz],
                 &SinkP[0], NumSinks * sizeof(struct bh_particle_data));
#endif
#ifdef DUST_LIVE
  for(ix = 0; ix < All.TileICsFactor; ix++)
    for(iy = 0; iy < All.TileICsFactor; iy++)
      for(iz = 0; iz < All.TileICsFactor; iz++)
        if(ix != 0 || iy != 0 || iz != 0)
          memcpy(&DustP[N_dust * ix + N_dust * All.TileICsFactor * iy + N_dust * All.TileICsFactor * All.TileICsFactor * iz],
                 &DustP[0], N_dust * sizeof(struct dust_particle_data));
#endif
...
#endif
#ifdef SINKS
  NumSinks *= All.TileICsFactor * All.TileICsFactor * All.TileICsFactor;
#endif
#ifdef DUST_LIVE
