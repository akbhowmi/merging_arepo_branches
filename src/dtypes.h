...
#ifdef SIMPLEX
#ifdef SX_FLOAT_PRECISION
typedef float MySxDouble;
#else
typedef double MySxDouble;
#endif
#endif
...
/* Most AREPO code assumes that fftw_complex and fftwf_complex can be indexed like an
 * array. However, this is not possible if these types use C99's complex types. FFTW
 * automatically uses these types if complex.h is loaded before fftw3.h, unless
 * FFTW_NO_Complex is set. Thus, set it to avoid problems if complex.h is loaded
 * somewhere.
 */
