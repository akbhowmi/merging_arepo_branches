...
peanokey position_to_peanokey(MyDouble pos[3])
{
  ...
  peanokey key = peano_hilbert_key((int)((pos[0] - DomainCorner[0]) * DomainFac), (int)((pos[1] - DomainCorner[1]) * DomainFac),
                                   (int)((pos[2] - DomainCorner[2]) * DomainFac), BITS_PER_DIMENSION);
  return key;
}
...
#ifdef SINKS
              if(P[dest].Type == 5)
                SinkP[P[dest].AuxDataID].PID = dest;
#endif
#ifdef DUST_LIVE
