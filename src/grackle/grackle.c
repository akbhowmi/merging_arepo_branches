  ...
  for(int timebin = All.LowestActiveTimeBin; timebin <= All.HighestActiveTimeBin; timebin++)
    {
      ...
      for(int j = 0; j < ngas_active; j++)
        mpi_printf("rho: %g u: %g HI: %g HII %g HeI %g HeII %g HeIII %g e: %g\n", density[j], energy[j], HI_density[j], HII_density[j],
                   HeI_density[j], HeII_density[j], HeIII_density[j], e_density[j]);

#ifdef GRACKLE_TAB
      if(solve_chemistry_table(&my_grackle_units, All.cf_atime, dtime, grid_rank, grid_dimension, grid_start, grid_end, density,
                               energy, dummy_velocity, dummy_velocity, dummy_velocity, metal_density) == 0)
        terminate("GRACKLE: Error in solve_chemistry_table.\n");

        /* Pressure in tabular case calculated later */
#else
      if(solve_chemistry(&my_grackle_units, All.cf_atime, dtime, grid_rank, grid_dimension, grid_start, grid_end, density, energy,
                         dummy_velocity, dummy_velocity, dummy_velocity, HI_density, HII_density, HM_density, HeI_density,
                         HeII_density, HeIII_density, H2I_density, H2II_density, DI_density, DII_density, HDI_density, e_density,
                         metal_density) == 0)
        terminate("GRACKLE: Error in solve_chemistry.\n");

      if(calculate_pressure(&my_grackle_units, All.cf_atime, grid_rank, grid_dimension, grid_start, grid_end, density, energy,
                            HI_density, HII_density, HM_density, HeI_density, HeII_density, HeIII_density, H2I_density, H2II_density,
                            DI_density, DII_density, HDI_density, e_density, metal_density, pressure) == 0)
...
#ifdef GRACKLE_TAB
  if(calculate_temperature_table(&my_grackle_units, All.cf_atime, grid_rank, grid_dimension, grid_start, grid_end, &density, &energy,
                                 &metal_density, &temperature) == 0)
    terminate("GRACKLE: Error in calculate_temperature_table.\n");
#else
  if(calculate_temperature(&my_grackle_units, All.cf_atime, grid_rank, grid_dimension, grid_start, grid_end, &density, &energy,
                           &HI_density, &HII_density, &HM_density, &HeI_density, &HeII_density, &HeIII_density, &H2I_density,
                           &H2II_density, &DI_density, &DII_density, &HDI_density, &e_density, &metal_density, &temperature) == 0)
...
#ifdef GRACKLE_TAB
  if(calculate_cooling_time_table(&my_grackle_units, All.cf_atime, grid_rank, grid_dimension, grid_start, grid_end, &density, &energy,
                                  &dummy_velocity, &dummy_velocity, &dummy_velocity, &metal_density, &t_cool) == 0)
    terminate("GRACKLE: Error in calculate_cooling_time_table.\n");
#else
  if(calculate_cooling_time(&my_grackle_units, All.cf_atime, grid_rank, grid_dimension, grid_start, grid_end, &density, &energy,
                            &dummy_velocity, &dummy_velocity, &dummy_velocity, &HI_density, &HII_density, &HM_density, &HeI_density,
                            &HeII_density, &HeIII_density, &H2I_density, &H2II_density, &DI_density, &DII_density, &HDI_density,
                            &e_density, &metal_density, &t_cool) == 0)
  ...
  while(ngas_con < NumGas)
    {
      ...
      if(solve_chemistry(&my_grackle_units, All.cf_atime, dtime, grid_rank, grid_dimension, grid_start, grid_end, density, energy,
                         x_velocity, y_velocity, z_velocity, HI_density, HII_density, HM_density, HeI_density, HeII_density,
                         HeIII_density, H2I_density, H2II_density, DI_density, DII_density, HDI_density, e_density,
                         metal_density) == 0)
        terminate("GRACKLE: Error in solve_chemistry.\n");
